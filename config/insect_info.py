#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""害虫配置：从 `config/insect_info.json` 加载关键字段，并提供区域索引与兼容别名（c1/c2）。"""
...
@dataclass(frozen=True)
class StandardTestDiagonalPx:
    """标准测试集（dachong-标准测试集）标注框对角线长度统计（像素）。"""
    ...
    @staticmethod
    def from_obj(obj: Any) -> StandardTestDiagonalPx:
        if not isinstance(obj, dict):
            return StandardTestDiagonalPx(None, None, None, None, None)
        return StandardTestDiagonalPx(
            _f(obj.get("min_px")),
            _f(obj.get("max_px")),
            _f(obj.get("mean_px")),
            _i(obj.get("sample_count")),
            obj.get("source"),
        )
...
class InsectJsonRecord:
    ...
    - **``standard_test_diagonal_px``**：标准测试集标注框对角线长度（像素）的 min/max/mean，来源见 ``source``。
    """
    ...
    standard_test_diagonal_px: StandardTestDiagonalPx
    ...
    def from_dict(key: str, d: Dict[str, Any]) -> InsectJsonRecord:
        ...
        return InsectJsonRecord(
            ...
            standard_test_diagonal_px=StandardTestDiagonalPx.from_obj(
                d.get("standard_test_diagonal_px")
            ),
