...
def peek_composed_insect_alg_dict(path: str | Path | None = None) -> dict[str, Any]:
    """
    读取生效算法配置（含 shengchan/other overlay），不加载 torch、不改写 model 路径。

    仅用于启动前探测 ``run_count`` 等；compose 规则与 ``load_insect_alg_all`` 一致。
    """
    ...
    if is_insect_alg_profile_path(effective_path):
        ...
        if launcher:
            return compose_insect_alg_from_profile(launcher, data)
    return data


def peek_run_count(path: str | Path | None = None) -> int:
    """启动前探测 ``predict_cfg.run_count``，不 import torch。"""
    return resolve_run_count(peek_composed_insect_alg_dict(path))
...
DEFAULT_MASK_RATE_SWITCH = False


def resolve_mask_rate_switch(
    cfg: dict[str, Any] | None = None,
    *,
    default: bool = DEFAULT_MASK_RATE_SWITCH,
) -> bool:
    """
    是否启用 ``out.mask_rate`` 填充率过滤（``predict_cfg.mask_rate_switch``）。

    未配置时默认 ``False``（关闭）。
    """
    v = resolve_predict_cfg_value("mask_rate_switch", cfg, default=default)
    return bool(v)
...
def _scale_dia_list(dia: Any, rate: Any) -> list[int] | Any:
    """将 ``dia`` 区间按 ``rate`` 缩放；非法输入原样返回。"""
    ...
    if factor <= 0:
        return dia
    try:
        lo = round(float(dia[0]) * factor)
        hi = round(float(dia[1]) * factor)
    except (TypeError, ValueError):
        return dia
    return [lo, hi]


def _resolve_inherited_dia_rate(
    node: dict[str, Any],
    inherited_rate: Any,
) -> Any:
    """
    ...
    仅当 ``dia_switch`` 为 true 且本节点配置了 ``dia_rate`` 时更新继承值；
    ``dia_switch`` 关闭时不改变已继承的系数（子级 ``out`` 仍可按上层系数缩放）。
    """
    if not bool(node.get("dia_switch", True)):
        return inherited_rate
    if "dia_rate" in node:
        return node.get("dia_rate")
    return inherited_rate


def _apply_profile_dia_rate(
    base: dict[str, Any] | None,
    profile: dict[str, Any] | None,
    merged: dict[str, Any] | None,
    *,
    inherited_dia_rate: Any = None,
) -> None:
    """
    profile 合并后：对 ``out`` 路由项中**未在 profile 显式配置 ``dia``** 的条目，
    按 launcher 基线 ``dia`` × 生效 ``dia_rate`` 写入 merged。

    profile 中已写 ``dia`` 的条目以配置值为准，不做缩放。
    """
    if not isinstance(merged, dict):
        return
    active_rate = _resolve_inherited_dia_rate(merged, inherited_dia_rate)

    merged_out = merged.get("out")
    if active_rate is not None and isinstance(merged_out, dict):
        base_out = (base or {}).get("out") if isinstance(base, dict) else None
        profile_out = (profile or {}).get("out") if isinstance(profile, dict) else None
        if not isinstance(base_out, dict):
            base_out = {}
        if not isinstance(profile_out, dict):
            profile_out = {}
        for key, merged_entry in merged_out.items():
            if not isinstance(merged_entry, dict):
                continue
            profile_entry = profile_out.get(key)
            base_entry = base_out.get(key)
            if not isinstance(profile_entry, dict):
                profile_entry = {}
            if not isinstance(base_entry, dict):
                base_entry = {}
            if "dia" not in profile_entry:
                base_dia = base_entry.get("dia")
                if isinstance(base_dia, (list, tuple)):
                    merged_entry["dia"] = _scale_dia_list(base_dia, active_rate)
            _apply_profile_dia_rate(
                base_entry,
                profile_entry,
                merged_entry,
                inherited_dia_rate=active_rate,
            )

    merged_models = merged.get("models")
    if isinstance(merged_models, dict):
        base_models = (base or {}).get("models") if isinstance(base, dict) else None
        profile_models = (profile or {}).get("models") if isinstance(profile, dict) else None
        if not isinstance(base_models, dict):
            base_models = {}
        if not isinstance(profile_models, dict):
            profile_models = {}
        for model_key, merged_model in merged_models.items():
            if not isinstance(merged_model, dict):
                continue
            base_model = base_models.get(model_key)
            profile_model = profile_models.get(model_key)
            _apply_profile_dia_rate(
                base_model if isinstance(base_model, dict) else {},
                profile_model if isinstance(profile_model, dict) else {},
                merged_model,
                inherited_dia_rate=active_rate,
            )
...
def compose_insect_alg_from_profile(
    ...
    当某层 ``dia_switch`` 为 true 且配置了 ``dia_rate`` 时，该层及子级 ``out`` 路由中
    **未在 profile 显式写 ``dia``** 的虫类，其 ``dia`` 取 launcher 基线值 × ``dia_rate``；
    profile 已写 ``dia`` 的以配置值为准。
    """
    ...
    if isinstance(profile, dict):
        ...
        _apply_profile_dia_rate(launcher, profile, out)
    return out
