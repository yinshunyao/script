#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Detail  : 统一虫情推理入口：从 insect_alg_all.json 加载多根模型与递归 out/cls 路由，
#           封装 detect（PredictSize）与 segment（PredictSeg）及嵌套分类子图。
...
from script.predict.model_detect_factory import (
    is_detect_pipeline_type,
    is_onnx_detect_backend,
    pipeline_type_of,
)
...
from script.tools.roi_preprocess import apply_roi_preprocess, roi_circle_from_apply
...
_DETECT_DEFAULTS: dict[str, Any] = {
    ...
    # 以下四项对应 postprocess.geometry；>0 启用，<=0 关闭（摊平进根配置）
    "detect_iou_same_class": 0.0,
    "big_in_big_ior": 0.0,
    "small_in_small_ior": 0.0,
    "small_in_big_ior": 0.0,
...
def _predict_clip_kwargs_from_cfg(
    cfg: dict[str, Any],
    *,
    global_cfg: dict[str, Any] | None = None,
) -> tuple[list[ClipProfile], int, dict[str, Any]]:
    """
    组装 detect/seg ``predict`` 的切片 kwargs。

    多套 ``clip_profiles`` 时透传列表；单套时展开为 ``clip_size``/``overlap_size``/``clip_start``。
    返回 ``(profiles, batch_size, predict_kwargs)``。
    """
    profiles = resolve_clip_profiles_from_cfg(cfg, global_cfg=global_cfg)
    batch = resolve_clip_batch_size_from_cfg(cfg, global_cfg=global_cfg)
    if len(profiles) > 1:
        kwargs: dict[str, Any] = {
            "clip_profiles": profiles,
            "clip_batch_size": batch,
        }
    else:
        kwargs = {
            "clip_size": profiles[0].clip_size,
            "overlap_size": profiles[0].overlap_size,
            "clip_start": profiles[0].clip_start,
            "clip_batch_size": batch,
        }
    return profiles, batch, kwargs
...
_CLS_NODE_DEFAULTS: dict[str, Any] = {
    ...
    "to_gray": False,
    "crop_pad_ratio": 0.05,
    # dia_switch 仅当 cls JSON 未写时作底；_coalesce_cls_node 会改成根/postprocess 摊平值
    "dia_switch": True,
...
class ValidationFocusConfig:
    """验证/绘图关注类：tier 清单来自叶类 ``report_tier``；``report_classes`` 由报出开关决定。"""
...
def _expand_class_names_with_aliases(
    ...
    extra = _zh_names_for_tier_aliases(aliases if isinstance(aliases, dict) else None)
    return build_class_tier_equivalence(aliases, extra_names=extra).expand_names(names)
...
def _class_matches_report_allowed(
    ...
    *,
    ...
    keys = {name}
    if cn_index:
        zh = str(cn_index.get(name) or "").strip()
        if zh:
            keys.add(zh)
            if zh in allowed:
                return True
        if name not in cn_index:
            for py, cn in cn_index.items():
                if str(cn).strip() == name:
                    py_s = str(py).strip()
                    if py_s:
                        keys.add(py_s)
                        if py_s in allowed:
    ...
    return any(
        tier_equiv.are_equivalent(cand, item)
        for cand in keys
        for item in allowed
    )


def _filter_class_names_excluding_disabled(
    names: list[str],
    disabled: frozenset[str],
) -> list[str]:
    """从 tier 清单中剔除 ``out`` 树 ``enable=false``（或根模型关闭）的类。"""
    if not disabled:
        return names
    return [
        name
        for name in names
        if str(name or "").strip() and str(name).strip() not in disabled
    ]


def _collect_out_leaf_class_keys(node: Any) -> set[str]:
    """递归收集 ``out`` 树下全部叶路由 key / ``infer_name``（不检查 enable）。"""
    keys: set[str] = set()
    ...
    for cls_key, entry in out_table.items():
        ...
        if route_key and not _is_out_route_key(route_key):
            keys.add(route_key)
            ...
            if isinstance(cls_out, dict) and cls_out:
                for leaf_key, leaf_entry in cls_out.items():
                    ...
                    if lk and not _is_out_route_key(lk):
                        keys.add(lk)
                    leaf_infer = str(leaf_entry.get("infer_name") or "").strip()
                    if leaf_infer:
                        keys.add(leaf_infer)
                keys.update(_collect_out_leaf_class_keys(entry))
                continue
        keys.update(_collect_out_leaf_class_keys(entry))
    return keys


def _collect_enabled_out_leaf_class_keys(node: Any) -> set[str]:
    """
    递归收集 ``out`` 树中仍启用的叶路由 key / ``infer_name``。

    跳过路由项 ``enable=false``。嵌套 ``models.cls.enable=false`` 时仍收集
    ``cls.out`` 中未关闭的叶类（检测直出用物种表，分类器关闭不等于整表不报出）。
    """
    keys: set[str] = set()
    if not isinstance(node, dict):
        return keys
        ...
        if isinstance(cls_cfg, dict):
            ...
            if isinstance(cls_out, dict) and cls_out:
                for leaf_key, leaf_entry in cls_out.items():
                    ...
                    if leaf_infer:
                        keys.add(leaf_infer)
                keys.update(_collect_enabled_out_leaf_class_keys(entry))
                continue
        route_key = str(cls_key).strip()
        if route_key and not _is_out_route_key(route_key):
            keys.add(route_key)
...
def _collect_disabled_report_classes_from_out_node(node: Any) -> set[str]:
    """递归收集 ``out`` 树中 ``enable=false`` 的叶类名（含 ``infer_name``）。"""
    keys: set[str] = set()
    if not isinstance(node, dict):
        return keys
    out_table = node.get("out")
    ...
    for cls_key, entry in out_table.items():
        ...
        if not _out_entry_is_enabled(entry):
            ...
            if infer:
                keys.add(infer)
            keys.update(_collect_disabled_report_classes_from_out_node(entry))
            continue
        models = entry.get("models") or {}
        cls_cfg = models.get("cls") if isinstance(models, dict) else None
        if not isinstance(cls_cfg, dict):
            ...
            if isinstance(cls_out, dict) and cls_out:
                for leaf_key, leaf_entry in cls_out.items():
                    ...
                    if not _out_entry_is_enabled(leaf_entry):
                        ...
                        if lk and not _is_out_route_key(lk):
                            keys.add(lk)
                        infer = str(leaf_entry.get("infer_name") or "").strip()
                        if infer:
                            keys.add(infer)
            keys.update(_collect_disabled_report_classes_from_out_node(entry))
            continue
...
def resolve_disabled_report_class_names(
    alg_config: dict[str, Any] | None,
) -> frozenset[str]:
    """
    评估/报出应排除的类名集合。

    来源：根模型 ``enable=false`` 时其 ``out`` 下全部叶类；否则 ``out`` / ``cls.out``
    中叶节点 ``enable=false`` 的类（含 ``infer_name``）。再经 ``class_tier_aliases`` 扩展。
    嵌套 ``models.cls.enable=false`` 不把 ``cls.out`` 整表算作禁用（检测直出仍报出）。

    若某叶类同时出现在**已关闭根**与**仍启用根**的 ``out`` 中，以启用根为准，
    不因旁路根关闭而从报出白名单剔除（例如 ``detect_daofeishi`` 关闭时，
    ``detect_big`` 仍可报出 ``hefeishi`` / ``huifeishi``）。
    """
    if not isinstance(alg_config, dict):
        return frozenset()
    disabled_from_off_roots: set[str] = set()
    disabled_explicit: set[str] = set()
    enabled_alive: set[str] = set()
    models = alg_config.get("models")
    if not isinstance(models, dict):
        return frozenset()
    for _root_id, raw in models.items():
        if not isinstance(raw, dict):
            continue
        if not _is_enabled(raw):
            disabled_from_off_roots.update(_collect_out_leaf_class_keys(raw))
        else:
            enabled_alive.update(_collect_enabled_out_leaf_class_keys(raw))
            disabled_explicit.update(
                _collect_disabled_report_classes_from_out_node(raw)
            )
    disabled = (disabled_from_off_roots - enabled_alive) | disabled_explicit
    aliases = _postprocess_block(alg_config).get("class_tier_aliases")
    return _expand_class_names_with_aliases(tuple(disabled), aliases)
...
def resolve_report_allowed_class_names(
    ...
    - ``report_all_switch=true``：返回 ``None``（不过滤，全部报出）；
    - ``report_all_switch=false``：仅 ``top1`` + ``top2`` + ``top3`` + ``other``；
      ``background_classes`` 不报出。

    白名单由各类 ``out.report_tier``（``top1``/``top2``/``top3``/``other``）汇总，
    含路由 key 与 ``infer_name``。自动排除 ``out`` 树中叶节点 ``enable=false`` 的类
    （见 ``resolve_disabled_report_class_names``）。
    ``models.cls.enable=false`` 不把其 ``cls.out`` 整表排除（检测直出仍报这些物种）。
    """
    postprocess = _postprocess_block(alg_config)
    if bool(postprocess.get("report_all_switch", False)):
        return None
    top1, top2, top3, other = _report_tier_lists_excluding_disabled(alg_config)
...
def resolve_black_class_names(
    alg_config: dict[str, Any] | None,
) -> frozenset[str]:
    """
    ``postprocess.black_classes``：分类 top1 命中则递补 / 无法递补则不报出。

    经 ``class_tier_aliases`` 等价扩展。空配置返回空集合。
    """
    postprocess = _postprocess_block(alg_config)
    names = _parse_class_name_list(postprocess.get("black_classes"))
    if not names:
        return frozenset()
    aliases = postprocess.get("class_tier_aliases")
    return _expand_class_names_with_aliases(names, aliases)


def resolve_near_switch(alg_config: dict[str, Any] | None) -> bool:
    """
    ``postprocess.near_switch``：是否对分类 topn 做垃圾桶/近种 OOD。

    未写或 ``None`` 视为 ``true``（与现网一致）。``false`` 时不改 top1 报出。
    """
    pp = _postprocess_block(alg_config)
    if "near_switch" not in pp:
        return True
    v = pp.get("near_switch")
    if v is None:
        return True
    return bool(v)


def resolve_only_class_names(
    alg_config: dict[str, Any] | None,
) -> frozenset[str]:
    """
    ``postprocess.only_class``：分类候选白名单。

    - 仅当配置非空时生效；
    - 为保持兼容，也支持 ``only_classes`` 作为别名；
    - 生效时与 ``black_classes`` 互斥（black_classes 整段跳过）。
    """
    postprocess = _postprocess_block(alg_config)
    names = _parse_class_name_list(
        postprocess.get("only_class") or postprocess.get("only_classes")
    )
    if not names:
        return frozenset()
    aliases = postprocess.get("class_tier_aliases")
...
_REPORT_TIER_RANK = {"top1": 0, "top2": 1, "top3": 2, "other": 3}


def _normalize_report_tier(value: Any) -> str | None:
    s = str(value or "").strip().lower()
    return s if s in _REPORT_TIER_RANK else None


def _nested_cls_out_table(entry: dict[str, Any]) -> dict[str, Any] | None:
    models = entry.get("models") or {}
    cls_cfg = models.get("cls") if isinstance(models, dict) else None
    if not isinstance(cls_cfg, dict):
        cls_cfg = entry.get("cls")
...
def _iter_out_leaf_entries(node: Any):
    """遍历 ``out`` 叶类 ``(路由key, 配置dict)``。嵌套 ``cls.out`` 只产出叶子，不含路由项。"""
    if not isinstance(node, dict):
        return
    out_table = node.get("out")
    if isinstance(out_table, dict):
        for cls_key, entry in out_table.items():
            if not isinstance(entry, dict):
                continue
            nested = _nested_cls_out_table(entry)
            if nested is not None:
                for leaf_key, leaf_entry in nested.items():
                    if not isinstance(leaf_entry, dict):
                        continue
                    lk = str(leaf_key).strip()
                    if lk and not _is_out_route_key(lk):
                        yield lk, leaf_entry
                    yield from _iter_out_leaf_entries(leaf_entry)
                yield from _iter_out_leaf_entries(entry)
                continue
            route_key = str(cls_key).strip()
            if route_key and not _is_out_route_key(route_key):
                yield route_key, entry
            yield from _iter_out_leaf_entries(entry)
    models = node.get("models")
    if isinstance(models, dict):
        for child in models.values():
            yield from _iter_out_leaf_entries(child)


def collect_report_tier_class_lists(
    alg_config: dict[str, Any] | None,
) -> dict[str, list[str]]:
    """
    从全部根的 ``out`` 叶读取 ``report_tier``，汇总为 top1/top2/top3/other 名单。

    每个叶把路由 key 与非空且不同于 key 的 ``infer_name`` 一并放入对应档。
    同名多处出现时保留首次顺序；档位冲突取更高档（top1 > top2 > top3 > other）。
    """
    buckets: dict[str, list[str]] = {name: [] for name in _REPORT_TIER_RANK}
    ...
    for raw in models.values():
        if not isinstance(raw, dict):
            continue
        for key, entry in _iter_out_leaf_entries(raw):
            tier = _normalize_report_tier(entry.get("report_tier"))
            if not tier:
                continue
            names = [key]
            infer = str(entry.get("infer_name") or "").strip()
            if infer and infer != key:
                names.append(infer)
            for name in names:
                ...
                if prev is None:
                    assigned[name] = tier
                    order.append(name)
                elif _REPORT_TIER_RANK[tier] < _REPORT_TIER_RANK[prev]:
                    assigned[name] = tier
    for name in order:
        buckets[assigned[name]].append(name)
    return buckets


def _report_tier_lists_excluding_disabled(
    alg_config: dict[str, Any] | None,
) -> tuple[list[str], list[str], list[str], list[str]]:
    disabled = resolve_disabled_report_class_names(alg_config)
    tiers = collect_report_tier_class_lists(alg_config)
    return (
        _filter_class_names_excluding_disabled(tiers["top1"], disabled),
        _filter_class_names_excluding_disabled(tiers["top2"], disabled),
        _filter_class_names_excluding_disabled(tiers["top3"], disabled),
        _filter_class_names_excluding_disabled(tiers["other"], disabled),
    )
...
def resolve_validation_focus_config(
    ...
    *,
    ...
    """
    从叶类 ``report_tier`` 汇总档位清单，并从 ``postprocess`` 读取报出开关。
    ...
    tier 清单与 ``report_classes`` 均自动排除 ``out`` 树 ``enable=false`` 的类。
    """
    if not isinstance(alg_config, dict):
    ...
    if allowed is None:
        report = list(
            _expand_class_names_with_aliases(
                list(top1) + list(top2) + list(top3) + list(other),
                postprocess.get("class_tier_aliases"),
            )
        )
    else:
        report = list(allowed)
...
class CompetitionCountingSummary:
    ...
    min_insect_count: int = 0
    avg_accuracy_exclude_zero_gt_percent: float = 0.0
    avg_accuracy_include_zero_gt_percent: float = 0.0
...
def competition_counting_accuracy_percent(gt: int, pred: int) -> float:
    """
    单类识别计数准确率（%）= (1 - |识别-鉴定|/鉴定) × 100。

    鉴定数 gt=0 且识别数≥1 时按 0；公式结果为负时按 0。
    鉴定>0 时「识别」取正确匹配数（TP）；鉴定=0 误报时「识别」取报出数。
    """
    gt_n = int(gt)
    pred_n = int(pred)
    if gt_n <= 0:
...
def _competition_recognize_count(s: dict[str, int], *, gt_n: int, pred_n: int) -> int:
    """
    比赛表「识别数」：

    - 鉴定>0：报出框正确匹配数（优先 ``report_tp``）。无该字段时，用 ``min(tp, pred)``
      避免 filtered / 软匹配（name=其他但 top1 对上）把 ``tp`` 抬高超过报出数。
      无匹配统计时回退 ``pred``（兼容旧调用）。
    - 鉴定=0：报出误报数 ``pred``。
    """
    if gt_n > 0:
        if "report_tp" in s:
            return max(0, int(s.get("report_tp", 0)))
        if "tp" in s:
            return max(0, min(int(s.get("tp", 0)), int(pred_n)))
        return max(0, int(pred_n))
    return max(0, int(pred_n))
...
def compute_competition_counting_summary(
    ...
    *,
    ...
    ignore_classes: frozenset[str] | None = None,
    min_insect_count: int = 0,
) -> CompetitionCountingSummary | None:
    """
    按北京比赛规则汇总报出类 focus 内的计数准确率。

    单类公式中的「识别数」：鉴定>0 为报出框正确匹配（``report_tp`` / ``min(tp,pred)``）；
    鉴定=0 为报出误报数。filtered 几何匹配可计入 ``tp`` 以免假漏检，但不抬识别数。
    摆拍额外非标样惩罚仍按报出 ``pred`` 计。

    - **生产**（``run_model=shengchan``）：同时给出不含鉴定0项、含鉴定0项两种算术平均。
      不含鉴定0项只平均鉴定数达标的报出类；含鉴定0项再把鉴定=0 且识别≥1 的类按 0 分纳入。
      主分 ``avg_accuracy_percent`` 取含鉴定0项口径。无摆拍修正。
      ``ignore_classes`` 中的类不进入计分；``min_insect_count>0`` 时，鉴定数>0 且不足下限
      的类两种口径都不进平均。
    - **摆拍**（``run_model=baipai``）：仅 **标样类**（鉴定数>0）参与算术平均；鉴定数=0 的
      额外误报类不参与平均，其报出数计入修正惩罚（仅 focus 内 gt=0 且 pred≥1 的报出类；
      background / ``report_tier`` 过滤框不报出，不计入惩罚）。忽略清单与鉴定数下限不生效。
      两种口径仍同时写出，主分取不含鉴定0项。
    """
    ...
    ignore_set = frozenset(ignore_classes or ()) if not is_baipai else frozenset()
    min_count = max(0, int(min_insect_count or 0)) if not is_baipai else 0
    specimen_rows: list[tuple[str, int, int, float]] = []
    extra_rows: list[tuple[str, int, int, float]] = []
    specimen_acc_vals: list[float] = []
    extra_acc_vals: list[float] = []
    extra_from_stat = 0
    total_specimen_gt = 0
    for cls_name, s in stat_by_cls.items():
        ...
        if key not in focus:
            continue
        if ignore_set and is_eval_ignored_class(key, ignore_set, None):
            continue
        gt_n = int(s.get("gt", 0))
        pred_n = int(s.get("pred", 0))
        recognize_n = _competition_recognize_count(s, gt_n=gt_n, pred_n=pred_n)
        if gt_n <= 0 and pred_n <= 0:
            continue
        acc = competition_counting_accuracy_percent(gt_n, recognize_n)
        if gt_n > 0:
            specimen_rows.append((key, gt_n, recognize_n, acc))
            if min_count <= 0 or gt_n >= min_count:
                specimen_acc_vals.append(acc)
            total_specimen_gt += gt_n
        elif pred_n >= 1:
            extra_rows.append((key, gt_n, recognize_n, acc))
            extra_from_stat += pred_n
            extra_acc_vals.append(acc)
    class_rows = specimen_rows + extra_rows
    if not class_rows:
        return None
    avg_exclude = (
        float(sum(specimen_acc_vals) / len(specimen_acc_vals))
        if specimen_acc_vals
        else 0.0
    )
    include_vals = specimen_acc_vals + extra_acc_vals
    avg_include = (
        float(sum(include_vals) / len(include_vals)) if include_vals else 0.0
    )
    avg_acc = avg_exclude if is_baipai else avg_include
    penalty = 0.0
    ...
    return CompetitionCountingSummary(
        ...
        min_insect_count=min_count,
        avg_accuracy_exclude_zero_gt_percent=avg_exclude,
        avg_accuracy_include_zero_gt_percent=avg_include,
    )
...
def print_competition_counting_summary(
    ...
    *,
    ...
    min_n = int(summary.min_insect_count or 0)
    if summary.corrected_accuracy_percent is not None:
        formula_tail = (
            "平均=标样类(鉴定>0)算术平均；误报类不参与平均"
            "；摆拍修正=MAX(0,平均-(额外非标样数/标样总数)×100)"
        )
    elif min_n > 0:
        formula_tail = (
            f"不含鉴定0项=鉴定数≥{min_n}的报出类算术平均；"
            "含鉴定0项=上述类与鉴定0识别≥1的误报类算术平均"
        )
    else:
        formula_tail = (
            "不含鉴定0项=鉴定>0的报出类算术平均；"
    ...
    print("公式: 单类=(1-|识别-鉴定|/鉴定)×100；"
          "识别数=报出框正确匹配(鉴定>0)或报出误报数(鉴定=0)；" + formula_tail)
    ...
    if is_baipai:
        ...
        avg_rows = tuple(
            r for r in summary.class_rows if r[1] >= (min_n if min_n > 0 else 1)
        )
        extra_avg_rows = tuple(extra_rows)
        skip_rows = (
            tuple(r for r in summary.class_rows if 0 < r[1] < min_n)
            if min_n > 0
            else ()
        )
        if avg_rows:
            if min_n > 0:
                print(f"【参与算术平均 — 鉴定数≥{min_n}】")
            else:
                print("【参与算术平均 — 鉴定数>0】")
            _print_counting_rows(avg_rows)
        if extra_avg_rows:
            print("【无鉴定误报 — 含鉴定0项时按 0 分纳入平均】")
            _print_counting_rows(extra_avg_rows)
        if skip_rows:
            print("【鉴定数不足 — 不参与算术平均】")
            _print_counting_rows(skip_rows)
        if not avg_rows and not extra_avg_rows and not skip_rows:
            _print_counting_rows(summary.class_rows)
    if summary.corrected_accuracy_percent is not None:
        print(
    ...
    print(
        "各类算术平均准确率（不含鉴定0项）: "
        f"{summary.avg_accuracy_exclude_zero_gt_percent:.2f}%"
    )
    print(
        "各类算术平均准确率（含鉴定0项）: "
        f"{summary.avg_accuracy_include_zero_gt_percent:.2f}%"
    )
    if summary.corrected_accuracy_percent is not None:
        print(
...
def load_insect_alg_all(path: str | Path | None = None) -> dict[str, Any]:
    ...
    n_dia = apply_dia_mm_pixel_scale(data)
    ppm = resolve_px_per_mm(data)
    if n_dia and ppm > 0:
        logging.info("px_per_mm=%.3f：已按 dia_mm 换算 %d 类像素 dia", ppm, n_dia)
    return data
...
_POSTPROCESS_ROOT_SKIP_KEYS = frozenset(
    {
        ...
        "compitition",
        "competition",
        "relative_size_rerank",
        "relative_size_min_count",
        "relative_size_anchor_conf",
        "relative_size_min_anchors",
        "relative_size_dia_diff",
        "relative_size_ref_ratio",
        "relative_size_cand_conf_min",
        "relative_size_edge_px",
        "px_per_mm",
        "near_switch",
    }
)

COMPETITION_FILTER_REASON = "competition_single_class_count"
COMPETITION_OTHER_NAME = "other"
COMPETITION_ORIG_NAME_KEY = "competition_orig_name"


@dataclass(frozen=True)
class CompetitionModeCfg:
    """``postprocess.compitition``：目录批量竞赛模式（整批单类头数抑制）。"""

    enable: bool = False
    single_class_count: int = 3


def resolve_competition_mode_cfg(
    alg_config: dict[str, Any] | None,
) -> CompetitionModeCfg:
    """
    解析 ``postprocess.compitition``（兼容键名 ``competition``）。

    缺省 ``enable=false``；``single_class_count`` 缺省 3，归一为 ``>=1``。
    """
    pp = _postprocess_block(alg_config)
    block = pp.get("compitition")
    if not isinstance(block, dict):
        block = pp.get("competition")
    if not isinstance(block, dict):
        return CompetitionModeCfg()
    enable = bool(block.get("enable", False))
    try:
        thr = int(block.get("single_class_count", 3) or 3)
    except (TypeError, ValueError):
        thr = 3
    return CompetitionModeCfg(enable=enable, single_class_count=max(1, thr))


def _competition_count_class_name(r: dict[str, Any] | None) -> str:
    """
    竞赛头数只认报出 ``name``（及重跑用的 ``competition_orig_name``）。

    不看 ``cls_topk`` / ``viz_name``，避免与 XML ``<name>`` 口径不一致。
    """
    if not isinstance(r, dict):
        return ""
    orig = str(r.get(COMPETITION_ORIG_NAME_KEY) or "").strip()
    if orig and not _is_other_class_name(orig) and orig not in ("昆虫", "insect"):
        return orig
    name = str(r.get("name") or "").strip()
    if (
        not name
        or _is_other_class_name(name)
        or name in ("昆虫", "insect")
    ):
        return ""
    return name


def collect_competition_suppress_names(
    results_batches: Any,
    *,
    single_class_count: int,
    filtered_batches: Any = None,
) -> frozenset[str]:
    """整批报出 ``name`` 头数 ``< single_class_count`` 的类名集合（不含 filtered）。"""
    del filtered_batches  # 竞赛只按报出 name；filtered 不参与头数
    thr = max(1, int(single_class_count))
    counts: dict[str, int] = defaultdict(int)
    for results in results_batches or ():
        for r in results or ():
            name = _competition_count_class_name(r or {})
            if name:
                counts[name] += 1
    return frozenset(n for n, c in counts.items() if c < thr)


def collect_competition_suppress_names_from_xmls(
    xml_paths: Any,
    *,
    single_class_count: int,
) -> frozenset[str]:
    """
    扫描整批预测 XML：仅统计**无** ``filter_reason`` 的 object 的 ``<name>`` 头数，
    返回头数 ``< single_class_count`` 的类名集合。
    """
    thr = max(1, int(single_class_count))
    counts: dict[str, int] = defaultdict(int)
    for xp in xml_paths or ():
        ...
        if not path.is_file() or path.name.startswith("._"):
            continue
        try:
            rows = parse_pascal_voc_pred_objects(path)
        except Exception:
            logging.warning("竞赛汇总读 XML 失败: %s", path, exc_info=True)
            continue
        for r in rows:
            if str(r.get("filter_reason") or "").strip() or bool(r.get("filtered")):
                continue
            name = _competition_count_class_name(r)
            if name:
                counts[name] += 1
    return frozenset(n for n, c in counts.items() if c < thr)


def _rewrite_row_to_competition_other(r: dict[str, Any]) -> dict[str, Any]:
    """报出 ``name`` 改为 ``other``；保留 ``competition_orig_name`` 与 cls_topn 便于核对。"""
    out = dict(r)
    cur = _competition_count_class_name(out)
    if not str(out.get(COMPETITION_ORIG_NAME_KEY) or "").strip():
        if cur:
            out[COMPETITION_ORIG_NAME_KEY] = cur
    out["name"] = COMPETITION_OTHER_NAME
    out["cls_name"] = COMPETITION_OTHER_NAME
    out["cn_name"] = "其他"
    # 不清 cls_topn：图上/XML 仍可看原 top1；计分只认 name
    return out


def _row_hits_competition_suppress(
    r: dict[str, Any], suppress_names: frozenset[str]
) -> bool:
    """仅按报出 ``name`` / ``competition_orig_name`` 命中 suppress。"""
    if not suppress_names:
        return False
    key = _competition_count_class_name(r)
    return bool(key and key in suppress_names)


def apply_competition_rewrite_to_other(
    ...
    *,
    ...
    """
    将报出 ``results`` 中命中 suppress 的实例 ``name`` 改为 ``other``。

    ``filtered`` 原样保留（不参与竞赛改写）。返回 (results, filtered, 改写条数)。
    """
    if not suppress_names:
        return results, list(filtered or []), 0
    n_changed = 0
    kept: list[dict[str, Any]] = []
    for r in results:
        if _row_hits_competition_suppress(r, suppress_names):
            n_changed += 1
            kept.append(_rewrite_row_to_competition_other(r))
        else:
            kept.append(r)
    return kept, list(filtered or []), n_changed


def rewrite_competition_other_in_pred_xml(
    xml_path: Path | str,
    *,
    suppress_names: frozenset[str],
) -> int:
    """
    就地改写单个预测 XML：无 filter 的 object 若 ``name`` 在 suppress 中则改为 other，
    并写入 ``competition_orig_name``。返回改写条数。
    """
    if not suppress_names:
        return 0
    path = Path(xml_path)
    ...
    try:
        ...
        logging.warning("竞赛改写打开 XML 失败: %s", path, exc_info=True)
        return 0
    n_changed = 0
    for obj in root.findall("object"):
        fr = (obj.findtext("filter_reason") or "").strip()
        filtered_el = obj.find("filtered")
        is_filtered = bool(fr) or (
            filtered_el is not None
            and str(filtered_el.text or "").strip() in ("1", "true", "True")
        )
        if is_filtered:
            continue
        ...
        if name_el is None or not (name_el.text or "").strip():
            continue
        cur = str(name_el.text).strip()
        orig_el = obj.find("competition_orig_name")
        orig = (orig_el.text or "").strip() if orig_el is not None else ""
        count_key = orig if orig and not _is_other_class_name(orig) else cur
        if count_key in ("昆虫", "insect") or _is_other_class_name(count_key):
            continue
        if count_key not in suppress_names:
            continue
        if orig_el is None:
            orig_el = ET.SubElement(obj, "competition_orig_name")
        if not (orig_el.text or "").strip():
            orig_el.text = count_key
        name_el.text = COMPETITION_OTHER_NAME
        n_changed += 1
    if n_changed:
        try:
            ...
            logging.warning("竞赛改写写回 XML 失败: %s", path, exc_info=True)
            return 0
    return n_changed


def apply_competition_single_class_filter(
    results: list[dict[str, Any]],
    filtered: list[dict[str, Any]] | None,
    *,
    suppress_names: frozenset[str],
    ...
    """
    兼容旧名：等价于 ``apply_competition_rewrite_to_other``（改 other，不再移入 filtered）。
    """
    kept, out_filtered, _n = apply_competition_rewrite_to_other(
        results, filtered, suppress_names=suppress_names
    )
    return kept, out_filtered


def _is_competition_single_class_filtered(r: dict[str, Any]) -> bool:
    """历史：``filter_reason=competition_single_class_count``（现网改为改 other，仍识别旧 XML）。"""
    return str(r.get("filter_reason") or "").strip() == COMPETITION_FILTER_REASON
...
def _merge_root_cfg(
    ...
    ``prepare``：推理前预处理（ROI 等）；``postprocess``：推理后过滤（如 dia_switch、bin_dark_ratio_min、geometry、inner_boxes_fp_threshold、split_by_scale）。
    ``postprocess.geometry`` 摊平为 detect 几何门限（detect_iou_same_class / big_in_big_ior / small_in_small_ior / small_in_big_ior）。
    ``postprocess`` 中的 ``background_classes``、``black_classes``、
    ``class_tier_aliases``（类型等价、可传递）为全局报出/验证配置，不并入单根推理配置。
    报出档位在叶类 ``report_tier``，也不摊进单根。
    根模型内显式写的同名键覆盖公共段。
    """
    ...
    if isinstance(predict_block, dict):
        for k, v in predict_block.items():
            if str(k).endswith("_说明"):
                continue
            if k in ("run_count", "mask_rate_switch"):
                continue
            pipeline[k] = v
    out = dict(defaults)
    out.update(pipeline)
...
def _coalesce_cls_node(
    cls_raw: dict[str, Any],
    *,
    root_cfg: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """cls 未写 ``dia_switch`` 时继承根（已含 postprocess 摊平值）；写出则覆盖。"""
    out = _coalesce(cls_raw, _CLS_NODE_DEFAULTS)
    if "dia_switch" in cls_raw:
        return out
    if isinstance(root_cfg, dict) and "dia_switch" in root_cfg:
        out["dia_switch"] = bool(root_cfg.get("dia_switch"))
    return out
...
def _route_entry_cls_disabled(route_entry: dict[str, Any] | None) -> bool:
    ...
    跳过嵌套分类，按检测模型类名与 ``det_conf`` 直出（不用 ``infer_name`` 覆盖细类）。
    """
...
def _iter_cls_route_nodes(
    ...
    *,
    ...
    inherit = root_cfg if isinstance(root_cfg, dict) else cfg
    ...
    for key, entry in out_table.items():
        ...
        if cls_raw is not None:
            cls_merged = _coalesce_cls_node(cls_raw, root_cfg=inherit)
            cls_path = f"{entry_path}.models.cls"
            results.append((cls_path, cls_merged))
            results.extend(
                _iter_cls_route_nodes(cls_merged, cls_path, root_cfg=inherit)
            )
    return results
...
def _describe_cls_route_node(path: str, cls_cfg: dict[str, Any]) -> str:
    ...
    crop_mode = "poly_mask" if not resolve_from_bbox(cls_cfg) else "gray_bbox"
    return (
        f"  - {path}: model={model} infer={infer}{trt_note} backend={backend} "
        f"crop={crop_mode} to_square={bool(cls_cfg.get('to_square', True))} "
        f"bg={cls_cfg.get('cls_crop_background', '-')} pad={cls_cfg.get('cls_pad_color', '-')}"
    )
...
def resolve_model_square_options(
    ...
    *,
    ...
    """
    根模型 ``to_square``：整图与 ``clip_profiles`` 切片补方共用。

    未写时 detect 默认 False、segment 默认 True。嵌套分类补方读 ``models.cls.to_square``。
    """
    explicit = cfg.get("to_square")
    if explicit is None:
        to_sq = str(model_type or "").strip().lower() == "segment"
    else:
        to_sq = bool(explicit)
    return {"to_square": to_sq}
...
_PLACEHOLDER_CLASS_KEYS = frozenset({"unknown", "*"})


def _is_placeholder_class_key(name: str | None) -> bool:
    """检测/分类占位键：不能当作物种拼音去查 ``out.cn_name``。"""
    return str(name or "").strip().lower() in _PLACEHOLDER_CLASS_KEYS


def _detect_pinyin_from_row(row: dict[str, Any]) -> str:
    """检测模型 ``class_names`` 拼音；跳过 ``unknown``，避免盖住真实 ``class_name``。"""
    for k in ("class_name", "cls_name", "seg_cls_name"):
        v = str(row.get(k, "") or "").strip()
        if v and not _is_placeholder_class_key(v):
            return v
    return ""


def _iter_class_keys_for_cn_lookup(row: dict[str, Any]) -> list[str]:
    """
    中文展示查表键：定稿 ``name`` 优先（尺寸区间直出 ``insect`` 仍显示昆虫），
    占位 ``unknown`` 则回退检测拼音。
    """
    keys: list[str] = []
    seen: set[str] = set()
    for field in ("name", "class_name", "cls_name", "infer_name"):
        v = str(row.get(field) or "").strip()
        if not v or _is_placeholder_class_key(v) or v in seen:
            continue
        seen.add(v)
        keys.append(v)
    return keys
...
def _primary_class_name(row: dict[str, Any]) -> str:
    """
    路由/后处理主类名：已分类时优先 ``cls_name``。

    检测粗类常写在 ``class_name``（如 ``insect``）；若仍优先取检测名，
    分类后的 ``out`` 路由会整批 ``no_route``，报出为空、竞赛抑制失效。
    """
    cls = str(row.get("cls_name") or "").strip()
    if cls and not _is_placeholder_class_key(cls):
        return cls
    for k in ("class_name", "seg_cls_name"):
        v = str(row.get(k, "") or "").strip()
        if v and not _is_placeholder_class_key(v):
            return v
    return cls
...
def _is_other_class_name(name: str | None) -> bool:
    """``other`` 开头的类别（含 other_feishi / other_bai 等变体）及中文「其他/其它」。"""
    s = str(name or "").strip()
    if not s:
        return False
    low = s.lower()
    return low.startswith("other") or s in ("其他", "其它")
...
def _extract_top1_cls_conf(row: dict[str, Any]) -> float | None:
    """解析分类 top1 置信度（后置 other 时优先保留字段，其次 reason）。"""
    ...
    for item in topk:
        if not isinstance(item, dict):
            continue
        nm = str(item.get("class_name", "") or item.get("name", "") or "").strip()
        if not nm or _is_other_class_name(nm):
            continue
        try:
            c = item.get("conf")
...
def _format_cls_conf_other_reason(top1: str, conf: float, thr: float) -> str:
    ...
    # 低分场景用更多小数，避免写成 0.00 导致落盘回退到 det_conf
    conf_s = f"{float(conf):.6f}".rstrip("0").rstrip(".") or "0"
    thr_s = f"{float(thr):.6f}".rstrip("0").rstrip(".") or "0"
    return f"other_cls_conf:{nm}({conf_s}<={thr_s})"
...
def _row_detect_conf(row: dict[str, Any]) -> float:
    ...
    for k in ("conf", "seg_conf", "det_conf"):
        try:
            v = row.get(k)
            if v is not None:
                return float(v)
...
def _row_conf(row: dict[str, Any]) -> float:
    """输出展示用综合置信度（优先分类）。"""
    for k in ("cls_conf", "conf", "seg_conf", "det_conf"):
        try:
            v = row.get(k)
            if v is not None:
                return float(v)
...
def parse_open_unit_interval(value: Any, *, key: str) -> float:
    """键已出现时必须 ``0 < value < 1``，不能为 0 或 1。"""
    try:
        f = float(value)
    except (TypeError, ValueError) as e:
        raise ValueError(
            f"{key} 须为开区间 (0,1) 内的数，不能为 0 或 1，当前={value!r}"
        ) from e
    if not (0.0 < f < 1.0):
        raise ValueError(
            f"{key} 须为开区间 (0,1) 内的数，不能为 0 或 1，当前={value!r}"
...
def resolve_open_unit_interval_cfg(
    cfg: dict[str, Any] | None, key: str | None
) -> float | None:
    if not key or not isinstance(cfg, dict) or key not in cfg or cfg[key] is None:
        return None
    return parse_open_unit_interval(cfg[key], key=key)


def validate_size_conf_keys(config: dict[str, Any] | None) -> None:
    """加载时校验已出现的体型门限键。"""

    def _walk(node: Any) -> None:
        if isinstance(node, dict):
            for k in _SIZE_CONF_KEYS:
                if k in node and node[k] is not None:
                    parse_open_unit_interval(node[k], key=k)
...
def resolve_size_conf_threshold(
    ...
    *,
    ...
    if size_class == "small":
        return resolve_open_unit_interval_cfg(cfg, small_key)
    if size_class == "large":
        return resolve_open_unit_interval_cfg(cfg, big_key)
    return None


def resolve_size_conf_min_present(
    cfg: dict[str, Any] | None, *, small_key: str, big_key: str
) -> float | None:
    vals: list[float] = []
    for key in (small_key, big_key):
        parsed = resolve_open_unit_interval_cfg(cfg, key)
        if parsed is not None:
            vals.append(parsed)
    return min(vals) if vals else None


def try_resolve_threshold_size_class(
    row: dict[str, Any] | None,
    entry: dict[str, Any] | None,
    size_class_by_name: dict[str, str] | None = None,
) -> str | None:
    """解析用于门限的体型；未知返回 None，不默认 large。"""
    if isinstance(entry, dict):
        raw = str(entry.get("size_class") or "").strip().lower()
        if raw in ("small", "large"):
            return raw
    row = row if isinstance(row, dict) else {}
    raw_row = str(row.get("size_class") or "").strip().lower()
    if raw_row in ("small", "large"):
        return raw_row
    if size_class_by_name:
        for key in (
            _primary_class_name(row),
            str(row.get("infer_name") or "").strip(),
            str(row.get("name") or "").strip(),
            str(row.get("cn_name") or "").strip(),
            str(row.get("cls_name") or "").strip(),
        ):
            if not key:
                continue
            mapped = size_class_by_name.get(key)
            if mapped:
                m = str(mapped).strip().lower()
                if m in ("small", "large"):
                    return m
    return None


def resolve_effective_cls_conf_threshold(
    ...
    *,
    ...
    """体型键出现时强制用该门限；否则回退 ``resolve_cls_top1_threshold``。"""
    if size_class is None:
        alg_entry = lookup_insect_alg_entry(alg, predicted_cls_name)
        size_class = try_resolve_threshold_size_class(
            {"cls_name": predicted_cls_name, "name": predicted_cls_name},
            alg_entry,
            size_class_by_name,
        )
    size_thr = resolve_size_conf_threshold(
        cls_cfg,
        small_key="cls_conf_small",
        big_key="cls_conf_big",
        size_class=size_class,
    )
    if size_thr is not None:
        return size_thr
    return resolve_cls_top1_threshold(alg, predicted_cls_name, generic_cls_conf)
...
def resolve_infer_detect_conf_floor(cfg: dict[str, Any], *, default: float = 0.25) -> float:
    ...
    已配置的 ``detect_conf_small`` / ``detect_conf_big`` 一并纳入最小值。
    """
    floors = [resolve_root_detect_conf(cfg, default=default)]
    size_min = resolve_size_conf_min_present(
        cfg, small_key="detect_conf_small", big_key="detect_conf_big"
    )
    if size_min is not None:
        floors.append(size_min)
...
def resolve_entry_detect_conf(
    ...
    *,
    ...
    """单类路由项上的检测门限；体型键出现时按 small/large 强制覆盖。"""
    if entry.get("detect_conf") is not None:
        base = float(entry["detect_conf"])
    else:
        base = resolve_root_detect_conf(root_cfg, default=default)
    size_thr = resolve_size_conf_threshold(
        root_cfg,
        small_key="detect_conf_small",
        big_key="detect_conf_big",
        size_class=size_class,
    )
    if size_class in ("small", "large"):
        return size_thr if size_thr is not None else base
    size_min = resolve_size_conf_min_present(
        root_cfg, small_key="detect_conf_small", big_key="detect_conf_big"
    )
    if size_min is None:
        return base
    return min(base, size_min)


def _row_has_cls_conf(row: dict[str, Any]) -> bool:
    """是否已有分类模型写出的 ``cls_conf``（缺省表示检测直出，不套用分类过滤）。"""
    return row.get("cls_conf") is not None
...
def _parse_positive_float_cfg_value(value: Any) -> float | None:
    if value is None:
        return None
    try:
        f = float(value)
...
@dataclass(frozen=True)
class SkipDiaThresholds:
    """跳过路由 ``dia`` 过滤的双阈值：检测与分类须**同时**严格大于各自门限。"""

    detect: float | None = None
    cls: float | None = None

    def is_configured(self) -> bool:
        return self.detect is not None or self.cls is not None

    def to_dict(self) -> dict[str, float]:
        out: dict[str, float] = {}
        if self.detect is not None:
            out["detect"] = float(self.detect)
        if self.cls is not None:
            out["cls"] = float(self.cls)
        return out


def resolve_skip_dia_thresholds(cfg: dict[str, Any] | None) -> SkipDiaThresholds:
    """
    解析 ``models.cls`` 上跳过 dia 的双阈值。

    - ``skip_dia_detect_conf``：检测置信度门限
    - ``skip_dia_cls_conf``：分类置信度门限
    - 仅当**两者均已配置且实例同时超过**时才跳过 dia
    - 遗留 ``skip_dia_conf``：仅映射为 ``skip_dia_cls_conf``（兼容旧配置）
    """
    if not cfg:
        return SkipDiaThresholds()
    detect = _parse_positive_float_cfg_value(cfg.get("skip_dia_detect_conf"))
    cls = _parse_positive_float_cfg_value(cfg.get("skip_dia_cls_conf"))
    if detect is None and cls is None:
        legacy = _parse_positive_float_cfg_value(cfg.get("skip_dia_conf"))
        if legacy is not None:
            cls = legacy
    return SkipDiaThresholds(detect=detect, cls=cls)


def merge_skip_dia_thresholds(
    child: SkipDiaThresholds | None,
    parent: SkipDiaThresholds | None,
) -> SkipDiaThresholds:
    c = child or SkipDiaThresholds()
    p = parent or SkipDiaThresholds()
    return SkipDiaThresholds(
        detect=c.detect if c.detect is not None else p.detect,
        cls=c.cls if c.cls is not None else p.cls,
    )


def resolve_effective_skip_dia_thresholds(
    cls_cfg: dict[str, Any] | None,
    inherited: SkipDiaThresholds | None = None,
) -> SkipDiaThresholds:
    return merge_skip_dia_thresholds(
        resolve_skip_dia_thresholds(cls_cfg),
        inherited,
    )


def coerce_skip_dia_thresholds(value: Any) -> SkipDiaThresholds | None:
    """从 ``SkipDiaThresholds`` / dict / 遗留单 float 解析。"""
    if value is None:
        return None
    if isinstance(value, SkipDiaThresholds):
        return value
    if isinstance(value, (int, float)):
        cls = _parse_positive_float_cfg_value(value)
        return SkipDiaThresholds(cls=cls) if cls is not None else None
    if isinstance(value, dict):
        detect = _parse_positive_float_cfg_value(
            value.get("detect", value.get("skip_dia_detect_conf"))
        )
        cls = _parse_positive_float_cfg_value(
            value.get("cls", value.get("skip_dia_cls_conf", value.get("skip_dia_conf")))
        )
        out = SkipDiaThresholds(detect=detect, cls=cls)
        return out if out.is_configured() else None
    return None


def resolve_skip_dia_conf(cfg: dict[str, Any] | None) -> float | None:
    """遗留：返回 ``skip_dia_cls_conf`` 或 ``skip_dia_conf``（供旧调用方）。"""
    th = resolve_skip_dia_thresholds(cfg)
    return th.cls


def resolve_dia_diff(cfg: dict[str, Any] | None) -> float | None:
    """
    ``models.cls.dia_diff``：路由项 ``dia`` 区间的相对容差。

    有效区间为 ``[lo*(1-d), hi*(1+d)]``；未配置、非法或 ``<=0`` 时返回 None（不扩缩）。
    """
    if not cfg:
        return None
    v = cfg.get("dia_diff")
...
def resolve_effective_dia_bounds(
    dia: Any,
    dia_diff: float | None = None,
) -> tuple[float, float] | None:
    """从 ``dia=[lo,hi]`` 与可选 ``dia_diff`` 得到有效对角线上下界；非法返回 None。"""
    if not isinstance(dia, (list, tuple)) or len(dia) < 2:
        return None
    try:
    ...
    if lo > hi:
        lo, hi = hi, lo
    if dia_diff is not None and float(dia_diff) > 0:
        d = float(dia_diff)
        lo = lo * (1.0 - d)
        hi = hi * (1.0 + d)
    return lo, hi
...
def resolve_px_per_mm(alg_config: dict[str, Any] | None) -> float:
    """``postprocess.px_per_mm``：1mm 对应像素；未配置 / 非法 / ``<=0`` 返回 0（不换算）。"""
    pp = _postprocess_block(alg_config)
    v = pp.get("px_per_mm")
    if v is None:
        return 0.0
...
def apply_dia_mm_pixel_scale(config: dict[str, Any] | None) -> int:
    """
    按 ``dia_mm * px_per_mm`` 写回分类 ``out.dia``。

    无有效 ``px_per_mm`` 或条目无 ``dia_mm`` 时不改。返回改写条数。
    """
    if not isinstance(config, dict):
        return 0
    ppm = resolve_px_per_mm(config)
    if ppm <= 0.0:
        return 0
    changed = 0
    for out in _collect_cls_out_dicts_from_config(config):
        ...
        for entry in out.values():
            ...
            if not isinstance(mm, (list, tuple)) or len(mm) < 2:
                continue
            try:
                lo, hi = float(mm[0]) * ppm, float(mm[1]) * ppm
            except (TypeError, ValueError):
                continue
            if lo > hi:
                lo, hi = hi, lo
            entry["dia"] = [int(round(lo)), int(round(hi))]
            changed += 1
    return changed
...
@dataclass(frozen=True)
class ClassDiaMeta:
    """相对尺寸重排用的类体型先验（来自路由项 ``dia``）。"""

    key: str
    dia: tuple[float, float]
    ref_len: float
    infer_name: str
    cn_name: str


def _dia_ref_len(lo: float, hi: float) -> float:
    """
    几何平均作为相对体型参考长度。

    对 ``hi`` 极大的开放上界（如 9999）做跨度钳制，避免 ref 被拉到离谱中位。
    """
    if lo <= 0 or hi <= 0:
        return max(lo, hi, 1e-6)
    max_span = 3.0
    hi_eff = min(float(hi), float(lo) * max_span)
    if hi_eff < lo:
        hi_eff = float(hi)
    return float(math.sqrt(float(lo) * hi_eff))


def _collect_class_dia_meta_from_out(
    out: dict[str, Any],
    dest: dict[str, ClassDiaMeta],
) -> None:
    """从单个 ``out`` 表写入类 → ``ClassDiaMeta``（已有键不覆盖，先到先得）。"""
    for cls_key, entry in out.items():
        if not isinstance(entry, dict):
            continue
        key = str(cls_key).strip()
        if not key or key == "*" or key.startswith("["):
            continue
        bounds = resolve_effective_dia_bounds(entry.get("dia"), None)
        if bounds is None:
            continue
        lo, hi = bounds
        infer = str(entry.get("infer_name") or key).strip() or key
        cn = str(entry.get("cn_name") or "").strip()
        meta = ClassDiaMeta(
            key=key,
            dia=(lo, hi),
            ref_len=_dia_ref_len(lo, hi),
            infer_name=infer,
            cn_name=cn,
        )
        if key not in dest:
            dest[key] = meta
        if infer and infer not in dest:
            dest[infer] = meta
        if cn and cn not in dest:
            dest[cn] = meta


def build_class_dia_index(alg_config: dict[str, Any] | None) -> dict[str, ClassDiaMeta]:
    """从算法配置各 ``out`` 表构建类名/infer_name/cn_name → ``dia`` 体型索引。"""
    if not isinstance(alg_config, dict):
        return {}
    dest: dict[str, ClassDiaMeta] = {}
    for out in _collect_cls_out_dicts_from_config(alg_config):
        if isinstance(out, dict):
            _collect_class_dia_meta_from_out(out, dest)
    if not dest:
        for out in _collect_all_out_dicts_from_config(alg_config):
            if isinstance(out, dict):
                _collect_class_dia_meta_from_out(out, dest)
    return dest


def build_class_route_entry_index(
    alg_config: dict[str, Any] | None,
) -> dict[str, dict[str, Any]]:
    """分类 ``out`` 类名/infer_name/cn_name → 路由项（dia 改类取 detect_conf / dia）。"""
    ...
    for out in _collect_cls_out_dicts_from_config(alg_config):
        ...
        for cls_key, entry in out.items():
            ...
            infer = str(entry.get("infer_name") or key).strip() or key
            cn = str(entry.get("cn_name") or "").strip()
            dest.setdefault(key, entry)
            if infer:
                dest.setdefault(infer, entry)
            if cn:
                dest.setdefault(cn, entry)
    return dest


def build_class_size_class_index(alg_config: dict[str, Any] | None) -> dict[str, str]:
    """从分类 ``out`` 构建类名/infer_name/cn_name → ``size_class``（small/large）。"""
    dest: dict[str, str] = {}
    if not isinstance(alg_config, dict):
        return dest
    for out in _collect_cls_out_dicts_from_config(alg_config):
        ...
        for cls_key, entry in out.items():
            ...
            if not key or key == "*" or key.startswith("["):
                continue
            sc = _normalize_size_class(entry.get("size_class"))
            infer = str(entry.get("infer_name") or key).strip() or key
            cn = str(entry.get("cn_name") or "").strip()
            dest.setdefault(key, sc)
            if infer:
                dest.setdefault(infer, sc)
            if cn:
                dest.setdefault(cn, sc)
    return dest


def _lookup_dia_meta(
    class_key: str,
    dia_index: dict[str, ClassDiaMeta],
) -> ClassDiaMeta | None:
    key = str(class_key or "").strip()
    if not key:
        return None
    return dia_index.get(key)


def _size_match_cost(norm_d: float, meta: ClassDiaMeta) -> float:
    """相对参考长度的对数偏差；越小越贴合。"""
    ref = float(meta.ref_len)
    if ref <= 1e-9 or norm_d <= 1e-9:
        return float("inf")
    return abs(math.log(float(norm_d) / ref))


def _ref_ratio_mismatch(
    norm_d: float,
    meta: ClassDiaMeta,
    ref_ratio: float,
) -> bool:
    """
    归一化对角线相对 ``ref_len`` 是否明显偏离。

    ``ref_ratio=1.4`` 表示允许约 ±40%；超出则视为相对体型明显不对。
    """
    r = float(ref_ratio)
    if r <= 1.0:
        r = 1.0 + 1e-6
    ...
    if ref <= 1e-9 or norm_d <= 1e-9:
        return True
    ratio = float(norm_d) / ref
    return ratio < (1.0 / r) or ratio > r


def _first_cls_dia_diff_from_config(alg_config: dict[str, Any] | None) -> float | None:
    """取配置中首个嵌套 ``models.cls.dia_diff``（供相对尺寸默认容差）。"""
    if not isinstance(alg_config, dict):
        return None
    ...
    def _walk(node: Any) -> None:
        ...
        if not isinstance(node, dict):
            return
        if node.get("model") is not None and "cls_conf" in node:
            d = resolve_dia_diff(node)
            if d is not None:
                found.append(d)
                return
        for value in node.values():
...
def _relative_size_cfg_block(pp: dict[str, Any]) -> dict[str, Any]:
    block = pp.get("relative_size")
    return block if isinstance(block, dict) else {}


def _relative_size_cfg_value(
    pp: dict[str, Any],
    nested_key: str,
    flat_key: str,
    default: Any,
) -> Any:
    """嵌套 ``relative_size.<key>`` 优先，否则旧平铺 ``relative_size_*``。"""
    block = _relative_size_cfg_block(pp)
...
def resolve_relative_size_anchor_class_names(
    alg_config: dict[str, Any] | None,
) -> frozenset[str]:
    """
    相对尺寸锚点类：叶类 ``report_tier`` 为 ``top1`` / ``top2`` / ``top3``（不含 other）。

    经 ``class_tier_aliases`` 扩展，并排除 ``out`` 树 ``enable=false`` 的类。
    清单为空时返回空集合（调用方视为不限制锚点类）。
    """
    postprocess = _postprocess_block(alg_config)
    top1, top2, top3, _other = _report_tier_lists_excluding_disabled(alg_config)
    names = list(top1) + list(top2) + list(top3)
    if not names:
        return frozenset()
    aliases = postprocess.get("class_tier_aliases")
    return _expand_class_names_with_aliases(names, aliases)
...
def resolve_relative_size_rerank_cfg(
    alg_config: dict[str, Any] | None,
) -> dict[str, Any]:
    """
    解析 ``postprocess.relative_size``（兼容旧平铺 ``relative_size_*``）。

    返回：``enabled``, ``min_count``, ``anchor_conf``, ``min_anchors``,
    ``dia_diff``, ``ref_ratio``, ``cand_conf_min``, ``edge_px``,
    ``anchor_names``, ``aliases``。
    """
    pp = _postprocess_block(alg_config)
    enabled = bool(
        _relative_size_cfg_value(pp, "enable", "relative_size_rerank", False)
    )
    try:
        min_count = int(
            _relative_size_cfg_value(pp, "min_count", "relative_size_min_count", 3)
            or 3
        )
    except (TypeError, ValueError):
        min_count = 3
    ...
    try:
        anchor_conf = float(
            _relative_size_cfg_value(
                pp, "anchor_conf", "relative_size_anchor_conf", 0.7
            )
            or 0.7
        )
    except (TypeError, ValueError):
    ...
    try:
        min_anchors = int(
            _relative_size_cfg_value(
                pp, "min_anchors", "relative_size_min_anchors", 2
            )
            or 2
        )
    except (TypeError, ValueError):
        min_anchors = 2
    min_anchors = max(1, min_anchors)
    dia_diff = resolve_dia_diff(
        {
            "dia_diff": _relative_size_cfg_value(
                pp, "dia_diff", "relative_size_dia_diff", None
            )
        }
    )
    if dia_diff is None:
        dia_diff = _first_cls_dia_diff_from_config(alg_config)
    if dia_diff is None:
        dia_diff = 0.3
    try:
        ref_ratio = float(
            _relative_size_cfg_value(pp, "ref_ratio", "relative_size_ref_ratio", 1.4)
            or 1.4
        )
    except (TypeError, ValueError):
        ref_ratio = 1.4
    ...
    try:
        cand_conf_min = float(
            _relative_size_cfg_value(
                pp, "cand_conf_min", "relative_size_cand_conf_min", 0.05
            )
            or 0.05
        )
    except (TypeError, ValueError):
    ...
    try:
        edge_px = float(
            _relative_size_cfg_value(pp, "edge_px", "relative_size_edge_px", 2)
            or 0
        )
    except (TypeError, ValueError):
        edge_px = 2.0
    aliases = pp.get("class_tier_aliases")
    return {
        "enabled": enabled,
        "min_count": min_count,
        "anchor_conf": anchor_conf,
        "min_anchors": min_anchors,
        "dia_diff": float(dia_diff),
        "ref_ratio": float(ref_ratio),
        "cand_conf_min": float(cand_conf_min),
        "edge_px": float(edge_px),
        "anchor_names": resolve_relative_size_anchor_class_names(alg_config),
        "aliases": aliases if isinstance(aliases, dict) else None,
    }


def _result_class_key_for_dia(row: dict[str, Any]) -> str:
    for k in ("name", "infer_name", "cls_name", "cls_name_top1", "cn_name"):
        v = str(row.get(k) or "").strip()
        if v and not _is_other_class_name(v):
            return v
    return ""
...
def _relative_size_row_keys(row: dict[str, Any]) -> list[str]:
    keys: list[str] = []
    seen: set[str] = set()
    for k in ("name", "infer_name", "cls_name", "cls_name_top1", "cn_name"):
        v = str(row.get(k) or "").strip()
        if v and v not in seen and not _is_other_class_name(v):
            seen.add(v)
            keys.append(v)
    return keys


def _relative_size_polygon_xyxy(poly: Any) -> tuple[float, float, float, float] | None:
    """多边形点列的轴对齐外接框；无法解析时返回 None。"""
    try:
...
def _relative_size_row_xyxy_float(
    row: dict[str, Any],
) -> tuple[float, float, float, float] | None:
    """贴边用外接框：location / x1..y2 / polygon 取并集（更贴近可视化轮廓）。"""
    extents: list[tuple[float, float, float, float]] = []
    loc = xyxy_from_seq(row.get("location"))
    if loc is not None:
        extents.append(loc)
    if all(k in row for k in ("x1", "y1", "x2", "y2")):
        xy = xyxy_from_seq((row["x1"], row["y1"], row["x2"], row["y2"]))
        if xy is not None:
            extents.append(xy)
    poly_xy = _relative_size_polygon_xyxy(row.get("polygon"))
    if poly_xy is not None:
        extents.append(poly_xy)
    if not extents:
        return None
    return (
...
def _relative_size_image_edge_gap(
    x1: float,
    y1: float,
    x2: float,
    y2: float,
    ...
    ``x2/y2`` 与 ``cv2.rectangle`` / 评估画框一致，为**含端点**：最后一行像素下标是
    ``height-1``。旧写法 ``height - y2`` 会把贴底框 ``y2=height-1`` 算成距边 1px。
    """
    return min(
        float(x1),
        float(y1),
        float(img_w) - 1.0 - float(x2),
        float(img_h) - 1.0 - float(y2),
    )


def _relative_size_row_near_edge(
    row: dict[str, Any],
    image_size: tuple[int, int] | None,
    edge_px: float,
) -> bool:
    """框到图像四边最小距离 ``<= edge_px`` 视为贴边；``edge_px<=0`` 或无图幅则否。"""
    ...
    if img_w <= 0 or img_h <= 0:
        return False
    xy = _relative_size_row_xyxy_float(row)
    if xy is None:
        return False
    return _relative_size_image_edge_gap(*xy, img_w, img_h) <= edge


def _relative_size_row_is_anchor_class(
    row: dict[str, Any],
    *,
    ...
    """``anchor_names`` 为空表示不限制。"""
    if not anchor_names:
        return True
    keys = _relative_size_row_keys(row)
    for key in keys:
        if _class_matches_report_allowed(key, anchor_names, aliases):
            return True
        meta = _lookup_dia_meta(key, dia_index)
        if meta is None:
            continue
        for cand in (meta.key, meta.infer_name, meta.cn_name):
            if cand and _class_matches_report_allowed(
                cand, anchor_names, aliases
            ):
                return True
    return False


def _relative_size_row_can_anchor(
    row: dict[str, Any],
    *,
    cfg: dict[str, Any],
    dia_index: dict[str, ClassDiaMeta],
    ...
    if _relative_size_row_near_edge(
        row, image_size, float(cfg.get("edge_px", 2) or 0)
    ):
        return False
    names = cfg.get("anchor_names") or frozenset()
    if not isinstance(names, frozenset):
        names = frozenset(str(x) for x in names if str(x).strip())
    return _relative_size_row_is_anchor_class(
        row,
        anchor_names=names,
        aliases=cfg.get("aliases") if isinstance(cfg.get("aliases"), dict) else None,
        dia_index=dia_index,
    )


def _normalized_diag_fits_class(
    norm_d: float,
    class_key: str,
    dia_index: dict[str, ClassDiaMeta],
    dia_diff: float | None,
) -> bool:
    meta = _lookup_dia_meta(class_key, dia_index)
    if meta is None:
        return False
    bounds = resolve_effective_dia_bounds(list(meta.dia), dia_diff)
    if bounds is None:
        return False
    lo, hi = bounds
    return lo <= float(norm_d) <= hi


def _class_size_clearly_wrong(
    norm_d: float,
    class_key: str,
    dia_index: dict[str, ClassDiaMeta],
    *,
    ...
    """top1 尺寸是否明显不对：落出有效 ``dia``，或相对 ``ref_len`` 倍率越界。"""
    meta = _lookup_dia_meta(class_key, dia_index)
    if meta is None:
        return False
    if not _normalized_diag_fits_class(norm_d, class_key, dia_index, dia_diff):
        return True
    return _ref_ratio_mismatch(norm_d, meta, ref_ratio)


def _estimate_image_size_scale(
    results: list[dict[str, Any]],
    dia_index: dict[str, ClassDiaMeta],
    *,
    anchor_conf: float,
    ...
    """用高置信锚点估计同图尺度 ``s``（``d ≈ s * L_ref``）；失败返回 None。"""
    opts = cfg or {}
    candidates: list[tuple[float, float, str]] = []
    for idx, row in enumerate(results):
        if exclude_row_id is not None and idx == exclude_row_id:
            continue
        if not _relative_size_row_can_anchor(
            row, cfg=opts, dia_index=dia_index, image_size=image_size
        ):
            continue
        cls_key = _result_class_key_for_dia(row)
        if not cls_key:
            continue
        meta = _lookup_dia_meta(cls_key, dia_index)
        if meta is None or meta.ref_len <= 0:
            continue
        if not _row_has_cls_conf(row) or _row_cls_conf(row) < float(anchor_conf):
            continue
        d = _result_bbox_diag_px(row)
        if d is None or d <= 0:
            continue
        candidates.append((float(d), float(meta.ref_len), cls_key))
    if len(candidates) < int(min_anchors):
        return None
    scales0 = [d / L for d, L, _ in candidates]
    s0 = float(statistics.median(scales0))
    if s0 <= 1e-9:
        return None
    # 去掉相对中位数偏离过大的锚点（抑制错误 top1 把尺度拉偏）
    inlier_ratio = 1.45
    inliers = [
        (d, L, cls_key)
        for d, L, cls_key in candidates
        if (1.0 / inlier_ratio) * s0 <= (d / L) <= inlier_ratio * s0
    ]
    if len(inliers) >= int(min_anchors):
        s0 = float(statistics.median([d / L for d, L, _ in inliers]))
        pool = inliers
    else:
        pool = candidates
    if s0 <= 1e-9:
        return None
    refined: list[float] = []
    for d, L, cls_key in pool:
        if _normalized_diag_fits_class(d / s0, cls_key, dia_index, dia_diff):
            refined.append(d / L)
    if len(refined) >= int(min_anchors):
        s = float(statistics.median(refined))
        return s if s > 1e-9 else None
    return s0


def _parse_relative_size_runtime_opts(opts: dict[str, Any]) -> dict[str, Any]:
    """从 ``resolve_relative_size_rerank_cfg`` 结果解析运行期浮点门限。"""
    try:
        dia_diff_f = float(opts.get("dia_diff")) if opts.get("dia_diff") is not None else 0.3
    except (TypeError, ValueError):
        dia_diff_f = 0.3
    try:
        ref_ratio = float(opts.get("ref_ratio", 1.4) or 1.4)
    except (TypeError, ValueError):
        ref_ratio = 1.4
    try:
        cand_conf_min = float(opts.get("cand_conf_min", 0.05) or 0.05)
    except (TypeError, ValueError):
        cand_conf_min = 0.05
    try:
        edge_px = float(opts.get("edge_px", 2) or 0)
    except (TypeError, ValueError):
        edge_px = 2.0
    return {
        "min_count": int(opts.get("min_count", 3) or 3),
        "anchor_conf": float(opts.get("anchor_conf", 0.7) or 0.7),
        "min_anchors": int(opts.get("min_anchors", 2) or 2),
        "dia_diff": dia_diff_f,
        "ref_ratio": ref_ratio,
        "cand_conf_min": cand_conf_min,
        "edge_px": edge_px,
    }


def _relative_size_collect_fitting_topk(
    row: dict[str, Any],
    *,
    norm_d: float,
    dia_index: dict[str, ClassDiaMeta],
    dia_diff: float,
    cand_conf_min: float,
) -> list[tuple[dict[str, Any], ClassDiaMeta, float]]:
    """收集合尺寸（有效 dia）的 topk 候选 ``(item, meta, conf)``。"""
    fitting: list[tuple[dict[str, Any], ClassDiaMeta, float]] = []
    for item in _sorted_cls_topk_entries(row):
        cand = _cls_topk_item_name(item)
        if not cand or _is_other_class_name(cand):
            continue
        conf_i = _cls_topk_item_conf(item)
        if conf_i < cand_conf_min:
            continue
        meta = _lookup_dia_meta(cand, dia_index)
        if meta is None:
            continue
        if not _normalized_diag_fits_class(norm_d, cand, dia_index, dia_diff):
            continue
        fitting.append((dict(item), meta, float(conf_i)))
    return fitting


def _relative_size_pick_best_by_cost(
    renormed: list[dict[str, Any]],
    meta_by_name: dict[str, ClassDiaMeta],
    conf_by_name: dict[str, float],
    *,
    norm_d: float,
) -> tuple[ClassDiaMeta | None, float | None]:
    """在归一化 topk 内按尺寸贴合度选最优类。"""
    best_meta: ClassDiaMeta | None = None
    best_conf: float | None = None
    best_cost = float("inf")
    for item in renormed:
        cand = _cls_topk_item_name(item)
        meta = meta_by_name.get(cand)
        if meta is None:
            continue
        cost = _size_match_cost(norm_d, meta)
        if cost < best_cost:
            best_cost = cost
            best_meta = meta
            best_conf = float(conf_by_name.get(cand, 0.0))
    return best_meta, best_conf


def apply_relative_size_rerank(
    results: list[dict[str, Any]],
    *,
    dia_index: dict[str, ClassDiaMeta],
    cn_index: dict[str, str] | None = None,
    ...
    finalize_filtered: Any | None = None,
    image_size: tuple[int, int] | None = None,
) -> list[dict[str, Any]]:
    """
    同图相对尺寸重排：top1 相对尺度明显不符时，在 ``cls_topk`` 中按尺寸选取候选。

    「明显不符」= 落出有效 ``dia``，或相对 ``ref_len`` 倍率越界（``ref_ratio``）。
    候选选取按与 ``ref_len`` 的贴合度，不再用置信度决胜。
    尺寸不合格的高置信类被剔除后，对剩余候选做
    ``conf_new = conf_orig / sum(剩余)``，并写回 ``cls_topk`` / ``cls_conf``。
    明显不符且无合尺寸候选时，从报出结果中剔除（``filter_reason=relative_size_no_candidate``）。
    估尺度锚点仅 ``top1/top2/top3``；贴边框（``edge_px``）不参与计算也不被本步骤剔除。

    ``cfg`` 为 ``resolve_relative_size_rerank_cfg`` 的返回值；就地改写保留项并返回。
    ``image_size`` 为 ``(width, height)``，供贴边判定。
    """
    if not results or not dia_index:
        return results
    opts = cfg or {}
    if not bool(opts.get("enabled", False)):
        return results
    rt = _parse_relative_size_runtime_opts(opts)
    if len(results) < int(rt["min_count"]):
        return results
    dia_diff_f = float(rt["dia_diff"])
    ref_ratio = float(rt["ref_ratio"])
    cand_conf_min = float(rt["cand_conf_min"])
    edge_px = float(rt["edge_px"])

    scale_kw = {
        "anchor_conf": float(rt["anchor_conf"]),
        "min_anchors": int(rt["min_anchors"]),
        "dia_diff": dia_diff_f,
        "cfg": opts,
        "image_size": image_size,
    }
    base_scale = _estimate_image_size_scale(
        results,
        dia_index,
        **scale_kw,
    )
    ...
    kept: list[dict[str, Any]] = []
    for row in results:
        if _relative_size_row_near_edge(row, image_size, edge_px):
            kept.append(row)
            continue
        d = _result_bbox_diag_px(row)
        if d is None or d <= 0:
            kept.append(row)
            continue
        # 用整图内点尺度，避免排除大体型正确框后被残框/小票锚点带偏
        scale = base_scale
        norm_d = float(d) / scale
        cur_key = _result_class_key_for_dia(row)
        cur_meta = _lookup_dia_meta(cur_key, dia_index) if cur_key else None
        clearly_wrong = bool(
            cur_key
            and _class_size_clearly_wrong(
                norm_d,
                cur_key,
                dia_index,
                dia_diff=dia_diff_f,
        ...
        if cur_key and not clearly_wrong:
            kept.append(row)
            continue

        fitting = _relative_size_collect_fitting_topk(
            row,
            norm_d=norm_d,
            dia_index=dia_index,
            dia_diff=dia_diff_f,
        ...
        if not fitting:
            if clearly_wrong:
                filtered_n += 1
                out = dict(row)
                out["relative_size_from"] = cur_key or str(row.get("name") or "")
                out["relative_size_no_candidate"] = True
                if filtered_out is not None:
                    fin = finalize_filtered or _finalize_filtered
                    filtered_out.append(
                        fin(
                            out,
                            route_entry=None,
                            root_id=str(row.get("source") or ""),
                            reason="relative_size_no_candidate",
                            cn_index=cn_index,
                        )
                    )
                continue
        ...
        # conf_new = conf_orig / sum(剩余前 cls_top_n)；避免新 top1 仍带原始极低概率
        renormed = _renormalize_cls_topk_confs(
            [item for item, _meta, _c in fitting],
            top_n=_cls_top_n_for_renorm(row),
        )
        if not renormed:
            kept.append(row)
            continue
        conf_by_name = {
            _cls_topk_item_name(item): _cls_topk_item_conf(item) for item in renormed
        }
        meta_by_name = {
            _cls_topk_item_name(item): meta for item, meta, _c in fitting
        }

        best_meta, best_conf = _relative_size_pick_best_by_cost(
            renormed, meta_by_name, conf_by_name, norm_d=norm_d
        )
        if best_meta is None:
            kept.append(row)
            continue

        new_name = best_meta.infer_name or best_meta.key
        if cur_key and (
            cur_key == new_name
            or cur_key == best_meta.key
            or cur_meta is best_meta
            or (best_meta.cn_name and cur_key == best_meta.cn_name)
        ):
            # 类名未变，但仍写回归一化 topk，避免后续门限读到未归一化的残渣概率
            row["cls_topk"] = renormed
            row["cls_top3"] = renormed[:3]
            if best_conf is not None and best_conf > 0:
                row["cls_conf"] = float(best_conf)
            _sync_row_cls_conf_orig_from_topk(row)
            kept.append(row)
            continue

        row["relative_size_from"] = cur_key or str(row.get("name") or "")
        row["relative_size_to"] = new_name
        row["relative_size_rerank"] = True
        row["name"] = new_name
        row["cls_name"] = new_name
        if best_meta.cn_name:
            row["cn_name"] = best_meta.cn_name
        elif cn_index:
            cn = _resolve_cn_display_for_class(
                best_meta.key, cn_index=cn_index, cls_cfg=None
            )
            if cn:
                row["cn_name"] = cn
        row["cls_topk"] = renormed
        ...
        if best_conf is not None and best_conf > 0:
            row["cls_conf"] = float(best_conf)
            try:
                det = float(row.get("det_conf", 0.0) or 0.0)
            except (TypeError, ValueError):
                det = 0.0
            row["score"] = max(det, float(best_conf))
        _sync_row_cls_conf_orig_from_topk(row)
        changed += 1
        kept.append(row)

    if changed or filtered_n:
        logging.debug(
            "relative_size_rerank: scale=%.4f changed=%d filtered=%d / %d",
            base_scale,
            changed,
            filtered_n,
            len(results),
...
def _should_skip_dia(
    row: dict[str, Any],
    skip_dia: SkipDiaThresholds | None,
) -> bool:
    if skip_dia is None or not skip_dia.is_configured():
        return False
    if skip_dia.detect is not None and _row_detect_conf(row) <= float(skip_dia.detect):
        return False
    if skip_dia.cls is not None:
        if not _row_has_cls_conf(row) or _row_cls_conf(row) <= float(skip_dia.cls):
            return False
    return True
...
def _should_skip_dia_by_cls_conf(
    ...
    cls = _parse_positive_float_cfg_value(skip_dia_conf)
    if cls is None:
        return False
    return _should_skip_dia(row, SkipDiaThresholds(cls=cls))
...
def _passes_entry_thresholds(
    ...
    *,
    ...
    skip_dia: SkipDiaThresholds | None = None,
    ...
    size_class = try_resolve_threshold_size_class(
        row, entry, size_class_by_name
    )
    conf_thr = resolve_entry_detect_conf(
        entry, root_cfg, size_class=size_class
    )
    if _row_detect_conf(row) < conf_thr:
        return False
    effective_skip = skip_dia
    if effective_skip is None and skip_dia_conf is not None:
        effective_skip = SkipDiaThresholds(
            cls=_parse_positive_float_cfg_value(skip_dia_conf)
        )
    apply_dia = dia_enabled and not _should_skip_dia(row, effective_skip)
    if apply_dia:
        bounds = resolve_effective_dia_bounds(entry.get("dia"), dia_diff)
...
def _threshold_filter_reason(
    ...
    *,
    ...
    - ``threshold_dia``：bbox 对角线不在 dia 范围（含 ``dia_diff`` 扩缩后）
    """
    size_class = try_resolve_threshold_size_class(
        row, entry, size_class_by_name
    )
...
def try_promote_cls_topk_for_dia(
    row: dict[str, Any],
    *,
    root_cfg: dict[str, Any],
    dia_enabled: bool = True,
    skip_dia: SkipDiaThresholds | None = None,
    dia_diff: float | None = None,
    dia_index: dict[str, ClassDiaMeta] | None = None,
    entry_index: dict[str, dict[str, Any]] | None = None,
    cand_conf_min: float = 0.01,
    cn_index: dict[str, str] | None = None,
) -> dict[str, Any] | None:
    """
    top1 因绝对 ``dia`` 不合格时，收集尺寸合格 ``cls_topk`` 候选并归一化后改类。

    归一化：``conf_new = conf_orig / sum(合格候选)``（对齐 ``only_classes``）。
    成功返回新 top1 路由项；失败返回 ``None``（不改写）。
    """
    del skip_dia  # 改类阶段不套用 skip_dia；候选一律校验 dia
    if not dia_enabled:
        return None
    if not _row_top1_fails_absolute_dia(
        row,
        ...
        return None
    entries = _sorted_cls_topk_entries(row)
    if len(entries) < 2:
        return None
    d = bbox_diag_px_from_row(row)
    if d <= 0:
        return None
    cur_key = _result_class_key_for_dia(row)
    dia_index = dia_index or {}
    entry_index = entry_index or {}
    try:
        min_conf = float(cand_conf_min)
    except (TypeError, ValueError):
        min_conf = 0.01
    if min_conf < 0:
        min_conf = 0.0

    fitting: list[tuple[dict[str, Any], dict[str, Any], float]] = []
    for item in entries:
        cand = _cls_topk_item_name(item)
        if not cand or _is_other_class_name(cand):
            continue
        if cur_key and (
            cand == cur_key
            or (cand == str(row.get("name") or "").strip())
            or (cand == str(row.get("cn_name") or "").strip())
        ):
            continue
        conf_i = _cls_topk_item_conf(item)
        if conf_i < min_conf:
            continue
        cand_entry = entry_index.get(cand)
        meta = _lookup_dia_meta(cand, dia_index)
        if cand_entry is None and meta is None:
            continue
        if cand_entry is None and meta is not None:
            cand_entry = {
                "dia": [meta.dia[0], meta.dia[1]],
                "infer_name": meta.infer_name or meta.key,
                "cn_name": meta.cn_name or "",
            }
        assert isinstance(cand_entry, dict)
        probe = {
            "conf": _row_detect_conf(row),
            "cls_conf": float(conf_i),
            "x1": row.get("x1"),
            "y1": row.get("y1"),
            "x2": row.get("x2"),
            "y2": row.get("y2"),
        ...
        if not _passes_entry_thresholds(
            ...
            continue
        fitting.append((dict(item), cand_entry, float(conf_i)))

    if not fitting:
        return None

    # conf_new = conf_orig / sum(剩余前 cls_top_n)
    normalized = _renormalize_cls_topk_confs(
        [item for item, _entry, _c in fitting],
        top_n=_cls_top_n_for_renorm(row),
    )
    if not normalized:
        return None
    conf_by_name = {
        _cls_topk_item_name(item): _cls_topk_item_conf(item) for item in normalized
    }

    # 保持「按原 conf 排序的合格列表」首位作为改类目标（与历史行为一致）
    best_item, best_entry, _old_conf = fitting[0]
    from_name = cur_key or str(row.get("name") or row.get("cls_name") or "")
    new_name = str(best_entry.get("infer_name") or _cls_topk_item_name(best_item)).strip()
    if not new_name:
        new_name = _cls_topk_item_name(best_item)
    new_conf = float(conf_by_name.get(_cls_topk_item_name(best_item), 0.0))
    if new_conf <= 0.0 and normalized:
        new_conf = float(_cls_topk_item_conf(normalized[0]))
        new_name = _cls_topk_item_name(normalized[0]) or new_name

    row["dia_topk_from"] = from_name
    row["dia_topk_to"] = new_name
    row["dia_topk_promote"] = True
    row["cls_name"] = new_name
    row["name"] = new_name
    row["cls_conf"] = new_conf
    row["cls_topk"] = normalized
    row["cls_top3"] = normalized[:3]
    _sync_row_cls_conf_orig_from_topk(row)
    cn = str(best_entry.get("cn_name") or "").strip()
    if not cn and cn_index:
        cn = _resolve_cn_display_for_class(
            new_name, cn_index=cn_index, cls_cfg=None
        ) or ""
    if cn:
        row["cn_name"] = cn
    try:
        det = float(row.get("det_conf", row.get("conf", 0.0)) or 0.0)
    except (TypeError, ValueError):
        det = 0.0
    row["score"] = max(det, new_conf)
    return best_entry


def _row_top1_fails_absolute_dia(
    row: dict[str, Any],
    *,
    entry_index: dict[str, dict[str, Any]] | None = None,
    dia_index: dict[str, ClassDiaMeta] | None = None,
    ...
    """当前 top1 是否落出其配置有效 ``dia``（含 ``dia_diff``）。无 dia 配置视为不失败。"""
    key = _result_class_key_for_dia(row)
    if not key:
        return False
    entry = (entry_index or {}).get(key)
    if entry is None and dia_index:
        meta = _lookup_dia_meta(key, dia_index)
        if meta is not None:
            entry = {"dia": [meta.dia[0], meta.dia[1]]}
    if not isinstance(entry, dict):
        return False
    bounds = resolve_effective_dia_bounds(entry.get("dia"), dia_diff)
    if bounds is None:
...
_DETECT_PENDING_THR_KEY = "_detect_pending_thresholds"
_DETECT_ROUTE_ENTRY_KEY = "_detect_route_entry"
_DETECT_ROUTE_KEY_KEY = "_detect_route_key"


def _geometry_cfg_value(cfg: dict[str, Any] | None, key: str) -> Any:
    """嵌套 ``geometry.<key>`` 优先，否则旧平铺 ``key``。"""
...
def resolve_detect_iou_same_class(cfg: dict[str, Any] | None) -> float:
    """
    detect 同类框 IoU 合并门限（``geometry.detect_iou_same_class``）。

    ``>0`` 启用；未配置 / 非法 / ``<=0`` 返回 ``0``（关闭）。分割根应忽略该值。
    """
    return _resolve_geometry_positive_float(cfg, "detect_iou_same_class")
...
def _set_row_bbox_xyxy(row: dict[str, Any], x1: int, y1: int, x2: int, y2: int) -> None:
    row["x1"] = int(x1)
    row["y1"] = int(y1)
    row["x2"] = int(x2)
    row["y2"] = int(y2)
    row["location"] = [int(x1), int(y1), int(x2), int(y2)]


def _record_detect_pending_threshold(
    row: dict[str, Any],
    *,
    entry: dict[str, Any],
    skip_dia: SkipDiaThresholds | None,
    ...
    out = dict(row)
    pending = list(out.get(_DETECT_PENDING_THR_KEY) or [])
    effective_skip = skip_dia
    if effective_skip is None and skip_dia_conf is not None:
        effective_skip = SkipDiaThresholds(
            cls=_parse_positive_float_cfg_value(skip_dia_conf)
    ...
    pending.append(
        {
            "entry": entry,
            "skip_dia": effective_skip.to_dict() if effective_skip else None,
            "skip_dia_conf": effective_skip.cls if effective_skip else None,
            "dia_diff": dia_diff,
            "dia_enabled": dia_enabled,
            "route_key": route_key,
        }
    )
    out[_DETECT_PENDING_THR_KEY] = pending
    return out


def _strip_detect_defer_keys(row: dict[str, Any]) -> dict[str, Any]:
    out = dict(row)
    out.pop(_DETECT_PENDING_THR_KEY, None)
    out.pop(_DETECT_ROUTE_ENTRY_KEY, None)
    out.pop(_DETECT_ROUTE_KEY_KEY, None)
    return out


def apply_detect_same_class_iou_merge(
    rows: list[dict[str, Any]],
    iou_thr: float,
) -> list[dict[str, Any]]:
    """
    detect 分类后同类框 IoU 合并：IoU ≥ 门限则并入高置信框。

    bbox 取交集，``conf`` / ``cls_conf`` 取最大。跳过 ``other``、已 ``filter``、无有效框。
    ``iou_thr <= 0`` 时原样返回。被并入的框丢弃（不进入 filtered）。
    """
    if not rows or float(iou_thr) <= 0:
        return rows

    mergeable_idx: list[int] = []
    for i, row in enumerate(rows):
        if row.get("filter"):
            continue
        name = _primary_class_name(row)
        if not name or _is_other_class_name(name):
            continue
        if row_location_to_bbox(row) is None:
            continue
        mergeable_idx.append(i)
    if len(mergeable_idx) < 2:
        return rows

    by_cls: dict[str, list[int]] = {}
    for i in mergeable_idx:
        by_cls.setdefault(_primary_class_name(rows[i]), []).append(i)
    ...
    for idxs in by_cls.values():
        ...
        idxs_sorted = sorted(
            idxs,
            key=lambda ii: (
                -_row_cls_conf(rows[ii]),
                -_row_detect_conf(rows[ii]),
                ii,
            ),
        )
        kept: list[int] = []
        for ii in idxs_sorted:
            box_i = row_location_to_bbox(rows[ii])
            if box_i is None:
                kept.append(ii)
                continue
            merged = False
            for kj in kept:
                box_k = row_location_to_bbox(rows[kj])
                if box_k is None:
                    continue
                if float(box_iou(box_i, box_k)) < float(iou_thr):
                    continue
                inter = _bbox_intersection_xyxy(box_k, box_i)
                if inter is None:
                    continue
                _set_row_bbox_xyxy(rows[kj], *inter)
                rows[kj]["conf"] = max(
                    _row_detect_conf(rows[kj]), _row_detect_conf(rows[ii])
                )
                rows[kj]["cls_conf"] = max(
                    _row_cls_conf(rows[kj]), _row_cls_conf(rows[ii])
                )
                dropped.add(ii)
                merged = True
                break
...
def _resolve_positive_float_cfg(cfg: dict[str, Any] | None, key: str) -> float:
    """读取配置中 ``>0`` 的浮点门限；未配置 / 非法 / ``<=0`` 返回 ``0``（关闭）。"""
    if not isinstance(cfg, dict):
        return 0.0
    v = cfg.get(key)
    if v is None:
...
def _resolve_geometry_positive_float(cfg: dict[str, Any] | None, key: str) -> float:
    """读取 ``geometry.<key>``（或旧平铺）中 ``>0`` 的门限；否则 ``0``（关闭）。"""
    v = _geometry_cfg_value(cfg, key)
    if v is None:
        return 0.0
    try:
        f = float(v)
...
def resolve_big_in_big_ior(cfg: dict[str, Any] | None) -> float:
    """
    detect 大虫内含大虫 IoR 门限（``geometry.big_in_big_ior``）。

    ``>0`` 启用；未配置 / 非法 / ``<=0`` 返回 ``0``（关闭）。分割根应忽略该值。
    IoR = 交集面积 / 较小框面积。
    """
    return _resolve_geometry_positive_float(cfg, "big_in_big_ior")


def resolve_small_in_small_ior(cfg: dict[str, Any] | None) -> float:
    """
    detect 小虫内含小虫 IoR 门限（``geometry.small_in_small_ior``）。

    ``>0`` 启用；未配置 / 非法 / ``<=0`` 返回 ``0``（关闭）。分割根应忽略该值。
    IoR = 交集面积 / 较小框面积。
    """
    return _resolve_geometry_positive_float(cfg, "small_in_small_ior")


def resolve_small_in_big_ior(cfg: dict[str, Any] | None) -> float:
    """
    detect 小虫落入大虫 IoR 门限（``geometry.small_in_big_ior``）。

    ``>0`` 启用；未配置 / 非法 / ``<=0`` 返回 ``0``（关闭）。分割根应忽略该值。
    IoR = 交集面积 / 小虫框面积。
    """
    return _resolve_geometry_positive_float(cfg, "small_in_big_ior")


def _normalize_size_class(value: Any) -> str:
    return "small" if str(value or "").strip().lower() == "small" else "large"


def resolve_row_size_class(
    row: dict[str, Any],
    size_class_by_name: dict[str, str] | None = None,
) -> str:
    """解析行体型：路由项 / 行字段 / 类名索引；未配置默认 ``large``。"""
    entry = row.get(_DETECT_ROUTE_ENTRY_KEY)
    if isinstance(entry, dict) and entry.get("size_class") is not None:
        raw = str(entry.get("size_class") or "").strip().lower()
        if raw in ("small", "large"):
            return raw
    raw_row = row.get("size_class")
    if raw_row is not None and str(raw_row).strip():
        raw = str(raw_row).strip().lower()
        if raw in ("small", "large"):
            return raw
    if size_class_by_name:
        for key in (
            ...
            mapped = size_class_by_name.get(key)
            if mapped:
                return _normalize_size_class(mapped)
    return "large"


def _merge_row_conf_take_max(dst: dict[str, Any], src: dict[str, Any]) -> None:
    """将 ``src`` 并入 ``dst``：``conf`` / ``cls_conf`` 分别取最大；不改 bbox 与类别。"""
    det = max(_row_detect_conf(dst), _row_detect_conf(src))
    dst["conf"] = det
    if _row_has_cls_conf(dst) or _row_has_cls_conf(src):
        cls = max(_row_cls_conf(dst), _row_cls_conf(src))
        dst["cls_conf"] = cls
        if "score" in dst or "score" in src:
            dst["score"] = max(det, cls)
        return
    dst.pop("cls_conf", None)
    if "score" in dst or "score" in src:
        dst["score"] = det


def apply_size_class_in_ior_filter(
    rows: list[dict[str, Any]],
    ior_thr: float,
    *,
    size_class: str,
    ...
    """
    同体型套框：较小框与较大框交集 / 小框面积 ≥ 门限则并入较大框。

    bbox 仍用较大框；``conf`` / ``cls_conf`` 取两者最大。
    ``size_class`` 为 ``small`` 或 ``large``。``ior_thr <= 0`` 时原样返回。
    返回 ``(kept, filtered)``；filtered 为被并入的内框。
    """
    want = "small" if str(size_class).strip().lower() == "small" else "large"
    reason_s = str(reason or "").strip() or (
        "small_in_small_ior" if want == "small" else "big_in_big_ior"
    )
    if not rows or float(ior_thr) <= 0:
        return rows, []
    ...
    for i, row in enumerate(rows):
        ...
        if not name or _is_other_class_name(name):
            continue
        if resolve_row_size_class(row, size_class_by_name) != want:
            continue
        box = row_location_to_bbox(row)
        if box is None:
            continue
        area = _bbox_area_xyxy(box)
        if area <= 0:
            continue
        candidates.append((i, area, box))
    if len(candidates) < 2:
        return rows, []

    dropped: dict[int, float] = {}
    kept_containers: list[tuple[int, float, tuple[int, int, int, int]]] = []
    for i, area, box in sorted(candidates, key=lambda t: (-t[1], t[0])):
        hit_ior: float | None = None
        merge_into: int | None = None
        for kj, area_k, box_k in kept_containers:
            if area_k <= area:
                continue
            inter = _bbox_intersection_xyxy(box, box_k)
            if inter is None:
                continue
            ior = _bbox_area_xyxy(inter) / area
            if ior >= float(ior_thr):
                hit_ior = ior
                merge_into = kj
                break
        if hit_ior is not None and merge_into is not None:
            _merge_row_conf_take_max(rows[merge_into], rows[i])
            dropped[i] = hit_ior
        else:
            kept_containers.append((i, area, box))
    ...
    ratio_key = f"{reason_s}_ratio"
    for i, row in enumerate(rows):
        if i in dropped:
            out = dict(row)
            out["filter"] = True
            out["filter_reason"] = reason_s
            out[reason_s] = float(ior_thr)
            out[ratio_key] = float(dropped[i])
            filtered.append(out)
        else:
            kept.append(row)
...
def apply_big_in_big_ior_filter(
    rows: list[dict[str, Any]],
    ior_thr: float,
    *,
    size_class_by_name: dict[str, str] | None = None,
) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    """大虫套大虫：见 ``apply_size_class_in_ior_filter``（``size_class=large``）。"""
    return apply_size_class_in_ior_filter(
        rows,
        ior_thr,
        size_class="large",
        reason="big_in_big_ior",
        size_class_by_name=size_class_by_name,
    )


def apply_small_in_small_ior_filter(
    rows: list[dict[str, Any]],
    ior_thr: float,
    *,
    size_class_by_name: dict[str, str] | None = None,
) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    """小虫套小虫：见 ``apply_size_class_in_ior_filter``（``size_class=small``）。"""
    return apply_size_class_in_ior_filter(
        rows,
        ior_thr,
        size_class="small",
        reason="small_in_small_ior",
        size_class_by_name=size_class_by_name,
    )


def apply_small_in_big_ior_filter(
    rows: list[dict[str, Any]],
    ior_thr: float,
    *,
    size_class_by_name: dict[str, str] | None = None,
    ...
    """
    小虫落入大虫：小虫框相对更大面积大虫框的 IoR ≥ 门限则滤除小虫框。

    不并入大虫置信度（异类误报）。``ior_thr <= 0`` 时原样返回。
    返回 ``(kept, filtered)``。
    """
    if not rows or float(ior_thr) <= 0:
        return rows, []

    larges: list[tuple[int, float, tuple[int, int, int, int]]] = []
    smalls: list[tuple[int, float, tuple[int, int, int, int]]] = []
    for i, row in enumerate(rows):
        if row.get("filter"):
            continue
        name = _primary_class_name(row)
        ...
        if area <= 0:
            continue
        sc = resolve_row_size_class(row, size_class_by_name)
        if sc == "small":
            smalls.append((i, area, box))
        else:
            larges.append((i, area, box))
    if not larges or not smalls:
        return rows, []

    dropped: dict[int, float] = {}
    for i, area, box in smalls:
        hit_ior: float | None = None
        for _lj, area_l, box_l in larges:
            if area_l <= area:
                continue
            inter = _bbox_intersection_xyxy(box, box_l)
            if inter is None:
                continue
            ior = _bbox_area_xyxy(inter) / area
            if ior >= float(ior_thr):
    ...
    for i, row in enumerate(rows):
        if i in dropped:
            out = dict(row)
            out["filter"] = True
            out["filter_reason"] = "small_in_big_ior"
            out["small_in_big_ior"] = float(ior_thr)
            out["small_in_big_ior_ratio"] = float(dropped[i])
            filtered.append(out)
        else:
            kept.append(row)
    return kept, filtered
...
def resolve_cls_top_n(cfg: dict[str, Any] | None) -> int:
    """
    ``cls_top_n``：过滤后归一化与导出保留的 top-N（按置信度降序）。

    推理侧另由 ``CLS_INFER_KEEP_TOPK``（默认 10）保留更宽候选供过滤。
...
def _cls_topk_item_conf_orig(item: Any) -> float:
    """模型原始置信度；无 ``conf_orig`` 时回退到当前 ``conf``。"""
    if not isinstance(item, dict):
        return 0.0
    if item.get("conf_orig") is not None:
        try:
            return float(item.get("conf_orig") or 0.0)
        except (TypeError, ValueError):
            pass
    return _cls_topk_item_conf(item)


def _ensure_cls_topk_conf_orig(item: dict[str, Any]) -> float:
    """确保条目带有 ``conf_orig``（只写一次，不被归一化覆盖）。"""
    if item.get("conf_orig") is None:
        item["conf_orig"] = _cls_topk_item_conf(item)
    return float(item["conf_orig"] or 0.0)


def _sync_row_cls_conf_orig_from_topk(row: dict[str, Any]) -> None:
    """用当前 top1 候选的 ``conf_orig`` 回写行级 ``cls_conf_orig``。"""
    topk = row.get("cls_topk") or row.get("cls_top3") or []
    if topk and isinstance(topk[0], dict):
        row["cls_conf_orig"] = _cls_topk_item_conf_orig(topk[0])
    elif row.get("cls_conf_orig") is None and row.get("cls_conf") is not None:
        try:
            row["cls_conf_orig"] = float(row.get("cls_conf") or 0.0)
        except (TypeError, ValueError):
            pass


def _stamp_cls_topk_conf_orig_list(entries: list[Any] | None) -> None:
    """对 topk 列表就地写入 ``conf_orig``（已有则跳过）。"""
    if not entries:
        return
    for it in entries:
        if isinstance(it, dict):
            _ensure_cls_topk_conf_orig(it)
...
def _cls_top_n_for_renorm(
    row: dict[str, Any],
    cls_cfg: dict[str, Any] | None = None,
) -> int | None:
    """归一化用的 ``cls_top_n``：行字段优先，其次配置；均无则对全部剩余候选归一化。"""
    for src in (
        row.get("cls_top_n"),
        (cls_cfg or {}).get("cls_top_n") if isinstance(cls_cfg, dict) else None,
    ):
        if src is None:
            continue
        try:
...
def _renormalize_cls_topk_confs(
    entries: list[dict[str, Any]],
    *,
    top_n: int | None = None,
) -> list[dict[str, Any]]:
    """
    剔除部分候选后，对剩余 ``cls_topk`` 做 ``conf_new = conf_orig / sum(前 top_n)``。

    推理侧通常保留置信度前 10；经尺寸 / only_classes / dia 等过滤后，
    最多取剩余前 ``top_n``（``cls_top_n``）参与求和与输出。
    ``top_n`` 为空时对全部剩余候选归一化。
    """
    working = [dict(x) for x in entries if isinstance(x, dict) and _cls_topk_item_name(x)]
    if not working:
        return []
    for item in working:
        _ensure_cls_topk_conf_orig(item)
    working.sort(key=_cls_topk_item_conf_orig, reverse=True)
    if top_n is not None and int(top_n) > 0:
        working = working[: int(top_n)]
    sum_conf = sum(_cls_topk_item_conf_orig(item) for item in working)
    if sum_conf <= 1e-12:
        return []
    inv = 1.0 / sum_conf
    out: list[dict[str, Any]] = []
    for item in working:
        nm = _cls_topk_item_name(item)
        conf_i = _cls_topk_item_conf_orig(item) * inv
        item["conf"] = float(conf_i)
        if nm:
            item["name"] = nm
            item.setdefault("class_name", nm)
        out.append(item)
    out.sort(key=_cls_topk_item_conf, reverse=True)
    return out


def apply_only_classes_topk_filter_normalize(
    row: dict[str, Any],
    only_classes: frozenset[str] | None,
    *,
    cls_cfg: dict[str, Any] | None = None,
) -> bool:
    """
    仅保留 ``only_classes`` 中的分类候选，并对剩余候选 conf 做归一化重算。

    规则：
    - 从 ``cls_topk`` 中删除不在名单里的候选；
    - 若剩余为空：写回 ``cls_name=other``、``cls_conf=0``、``cls_topk/cls_top3=[]``；
    - 否则：取剩余前 ``cls_top_n``，``conf_new = conf_orig / sum(...)``，并同步更新
      ``cls_name/cls_conf`` 与 ``cls_topk/cls_top3``。
    """
    if not only_classes:
        return False
    entries = _sorted_cls_topk_entries(row)
    ...
    working = [
        dict(e)
        for e in entries
        if _cls_topk_item_name(e) in only_classes
    ]
    if not working:
        row["cls_other_reason"] = "other_cls_only_class_empty"
        row["cls_name"] = "other"
        row["cls_conf"] = 0.0
        row["cls_topk"] = []
        row["cls_top3"] = []
    ...
    normalized = _renormalize_cls_topk_confs(
        working, top_n=_cls_top_n_for_renorm(row, cls_cfg)
    )
    if not normalized:
        row["cls_other_reason"] = "other_cls_only_class_sum_conf_nonpos"
        row["cls_name"] = "other"
        row["cls_conf"] = 0.0
        row["cls_topk"] = []
        row["cls_top3"] = []
    ...
    row["cls_name"] = _cls_topk_item_name(normalized[0])
    row["cls_conf"] = float(_cls_topk_item_conf(normalized[0]))
    row["cls_topk"] = normalized
    row["cls_top3"] = normalized[:3]
    _sync_row_cls_conf_orig_from_topk(row)
    return True
...
def apply_black_classes_topk_promote(
    row: dict[str, Any],
    black_classes: frozenset[str] | None,
    *,
    cn_index: dict[str, str] | None = None,
    update_output_name: bool = False,
) -> bool:
    """
    分类 top1 落在 ``black_classes`` 时忽略 top1，从后续 topk 递补。

    置信度重算：以「最初黑名单 top1」置信度 `c_black` 作为分母；丢弃连续命中的黑名单档后，
    将剩余候选每档的置信度按 `conf_new = conf_orig / c_black` 重算。
    若 top1/top2 连续在黑名单内，则递补到更深候选（如 top3/top4/top5），分母仍取最初黑名单 top1。
    无可递补非黑名单类时不改写，返回 False。
    """
    if not black_classes:
        return False

    def _is_black(name: str) -> bool:
        nm = str(name or "").strip()
        return bool(nm) and nm in black_classes

    entries = _sorted_cls_topk_entries(row)
    if not entries:
        return False
    if not _is_black(_cls_topk_item_name(entries[0])):
        return False

    from_name = _cls_topk_item_name(entries[0])
    _ensure_cls_topk_conf_orig(entries[0])
    anchor_conf = _cls_topk_item_conf_orig(entries[0])
    working = [dict(e) for e in entries]
    promoted = False
    while working and _is_black(_cls_topk_item_name(working[0])):
        promoted = True
        working = working[1:]

    if not promoted or not working:
        return False
    if _is_black(_cls_topk_item_name(working[0])):
        return False

    # 重算置信度：按最初黑名单 top1 置信度作为分母
    if anchor_conf > 0:
        for item in working:
            item["conf"] = float(_cls_topk_item_conf(item)) / float(anchor_conf)

    to_name = _cls_topk_item_name(working[0])
    to_conf = _cls_topk_item_conf(working[0])
    row["cls_name"] = to_name
    row["cls_conf"] = float(to_conf)
    row["cls_topk"] = working
    row["cls_top3"] = working[:3]
    row["black_classes_skip"] = True
    row["black_classes_from"] = from_name
    row["black_classes_to"] = to_name

    try:
        det = float(row.get("det_conf", 0.0) or 0.0)
    except (TypeError, ValueError):
        det = 0.0
    if update_output_name or "score" in row:
        row["score"] = max(det, float(to_conf))

    if update_output_name:
        row["name"] = to_name
        if cn_index:
            cn = _resolve_cn_display_for_class(
                to_name, cn_index=cn_index, cls_cfg=None
            )
            if cn:
                row["cn_name"] = cn
    return True
...
def _ood_canon_class_key(
    raw: str,
    *,
    cn_to_key: dict[str, str] | None = None,
) -> str:
    s = str(raw or "").strip()
    if not s:
        return ""
...
def resolve_cls_ood_near_patterns(
    alg: dict[str, Any] | None,
    predicted_cls_name: str,
) -> tuple[str, ...]:
    """解析 ``near_cls``：精确名或 glob（``*jingui``）。未配置返回空（无近种托底）。"""
    entry = lookup_insect_alg_entry(alg, predicted_cls_name)
    if not entry or "near_cls" not in entry:
        return ()
    names = _parse_class_name_list(entry.get("near_cls"))
    return tuple(str(n).strip().lower() for n in names if str(n).strip())


def _cls_ood_is_near_name(nm: str, *, patterns: tuple[str, ...]) -> bool:
    if not nm or not patterns:
        return False
    n = nm.lower()
    for p in patterns:
        if not p:
        ...
        if n == p:
            return True
        if any(ch in p for ch in "*?[") and fnmatch.fnmatchcase(n, p):
            return True
    return False


def resolve_cls_ood_thresholds(
    alg: dict[str, Any] | None,
    predicted_cls_name: str,
) -> tuple[float, float]:
    """返回 ``(p_dump, p_near)``；未配置或 ``<=0`` 为 0。"""
    entry = lookup_insect_alg_entry(alg, predicted_cls_name)
    if not entry:
        return 0.0, 0.0
    return (
        _resolve_positive_float_cfg(entry, "p_dump"),
        _resolve_positive_float_cfg(entry, "p_near"),
    )


def resolve_cls_ood_min_near(
    alg: dict[str, Any] | None,
    predicted_cls_name: str,
) -> int:
    """返回 ``min_near``；未配置 / 非法 / ``<=0`` 为 0（关闭近种条数路径）。"""
    entry = lookup_insect_alg_entry(alg, predicted_cls_name)
    if not isinstance(entry, dict):
        return 0
    v = entry.get("min_near")
    if v is None:
        return 0
    try:
        n = int(v)
...
def _format_cls_ood_other_reason(
    top1: str,
    dump_mass: float,
    dump_thr: float,
    near_mass: float,
    ...
    *,
    ...
    near_count: int = 0,
    dump_hit: bool = True,
    min_near_hit: bool = False,
) -> str:
    nm = str(top1 or "").strip() or "?"
    parts: list[str] = []
    if min_near_hit:
        parts.append(f"min_near={near_count}<{min_near}")
    if dump_hit:
        parts.append(f"p_dump={dump_mass:.6f}>={dump_thr:.6f}")
        parts.append(f"p_near={near_mass:.6f}<{near_thr:.6f}")
    inner = ",".join(parts) if parts else "ood"
    return f"other_cls_ood:{nm}({inner})"


def try_demote_cls_ood_dump_near(
    row: dict[str, Any],
    *,
    alg: dict[str, Any] | None,
    dump_names: frozenset[str] | None,
    cn_to_key: dict[str, str] | None = None,
    near_switch: bool = True,
) -> bool:
    """
    高置信 + 垃圾桶且缺少近种支持时后置 other。

    ``near_switch=false`` 时直接跳过。``p_dump<=0`` 且 ``min_near<=0``（含未配置）的类直接跳过。质量用 ``conf_orig``。
    """
    if not near_switch:
        return False
    top1_raw = str(row.get("cls_name") or row.get("name") or "").strip()
    if not top1_raw or _is_other_class_name(top1_raw):
        return False
    top1 = _ood_canon_class_key(top1_raw, cn_to_key=cn_to_key)
    dump_thr, near_thr = resolve_cls_ood_thresholds(alg, top1)
    min_near = resolve_cls_ood_min_near(alg, top1)
    if dump_thr <= 0.0 and min_near <= 0:
        return False
    dump_names = dump_names or frozenset()
    entries = _sorted_cls_topk_entries(row)
    rest = entries[1:3] if len(entries) >= 2 else []
    dump_mass = 0.0
    near_mass = 0.0
    near_count = 0
    near_patterns = resolve_cls_ood_near_patterns(alg, top1)
    dump_l = {str(x).strip().lower() for x in dump_names if str(x).strip()}
    for item in rest:
        nm = _ood_canon_class_key(_cls_topk_item_name(item), cn_to_key=cn_to_key)
        if not nm:
            continue
        orig = _cls_topk_item_conf_orig(item)
        if nm in dump_names or nm.lower() in dump_l:
            dump_mass += orig
            continue
        if _cls_ood_is_near_name(nm, patterns=near_patterns):
            near_mass += orig
            near_count += 1
    dump_hit = (
        dump_thr > 0.0
        and dump_mass >= dump_thr
        and (near_thr <= 0.0 or near_mass < near_thr)
    )
    min_near_hit = min_near > 0 and near_count < min_near
    if not dump_hit and not min_near_hit:
        return False
    _stash_top1_cls_before_other(row)
    row["cls_other_reason"] = _format_cls_ood_other_reason(
        top1,
        dump_mass,
        dump_thr,
        near_mass,
...
def _apply_cls_top_n_to_row(
    ...
    """
    ``cls_top_n>1`` 时：取剩余候选前 N，按 ``conf_orig`` 归一化并写回。

    无论是否发生尺寸/名单过滤，门限判定与 XML 导出前都应调用，避免 raw topk
    概率极低（接近 0）却未重算。
    """
    ...
    if not entries:
        return
    normalized = _renormalize_cls_topk_confs(entries, top_n=n)
    if not normalized:
        return
    row["cls_top_n"] = n
    row["cls_topk"] = normalized
    row["cls_top3"] = normalized[: min(3, len(normalized))]

    cur = str(row.get("cls_name") or "").strip()
    if _is_other_class_name(cur):
        _sync_row_cls_conf_orig_from_topk(row)
        return
    match = next(
        (x for x in normalized if _cls_topk_item_name(x) == cur),
        None,
    )
    if match is None:
        row["cls_name"] = _cls_topk_item_name(normalized[0])
        ...
        row["cls_conf"] = float(_cls_topk_item_conf(match))
    _sync_row_cls_conf_orig_from_topk(row)
        ...
        payload.append(
            {
                ...
                "conf_orig": _cls_topk_item_conf_orig(item),
            }
        )
...
def resolve_in_big_fill(cfg: dict[str, Any] | None) -> bool:
    ...
    仅 ``in_big_by=mask`` 时生效；``bbox`` 模式忽略本项。
    """
    if not cfg:
        return False
...
def resolve_in_big_by(cfg: dict[str, Any] | None) -> str:
    """
    ``in_big_by``：in_big 相交占比计算方式。

    - ``mask``（默认）：小虫裁切前景 mask 与大虫 polygon mask 像素相交（现有实现）；
    - ``bbox``：小虫检测框矩形与大虫 polygon 几何相交，不提取前景 mask。
    """
    if not cfg:
        return "mask"
    return normalize_in_big_by(cfg.get("in_big_by"))
...
def _edge_reject_should_filter_row(
    ...
    if not _row_has_cls_conf(row):
        return det_conf < det_conf_thr
    cls_conf = _row_cls_conf(row)
...
def _detect_filter_reason(row: dict[str, Any]) -> str:
    ...
    # 1.5) 多尺度大框误检：apply_split_by_scale_filter
    if row.get("split_by_scale_count") is not None:
        try:
            return f"split_by_scale({int(row.get('split_by_scale_count'))})"
        except Exception:
            return "split_by_scale"
...
def _resolve_dia_enabled(
    ...
    *,
    ...
    - 各模型层级可独立配置 ``dia_switch``；cls 未写时已由 ``_coalesce_cls_node`` 继承根值。
...
def _finalize_result(
    ...
    *,
    ...
    detect_pinyin = _detect_pinyin_from_row(row)
    cls_disabled = _route_entry_cls_disabled(route_entry)
    has_cls = (not cls_disabled) and _row_has_cls_conf(row)
    infer = ""
    if route_entry and not cls_disabled:
        infer = str(route_entry.get("infer_name") or "").strip()
    if infer:
        name = infer
    else:
        name = str(cls_name or detect_pinyin or route_key or "").strip() or cls_name
        if name == "*" or _is_placeholder_class_key(name):
            name = detect_pinyin or cls_name or "insect"
    cn = "" if cls_disabled else str((route_entry or {}).get("cn_name") or "").strip()
    bbox = row_location_to_bbox(row)
    if bbox is None:
        raise KeyError("row missing bbox (location/x1..y2/polygon)")
    x1, y1, x2, y2 = bbox
    det_conf = float(_row_detect_conf(row))
    out: dict[str, Any] = {
        "name": name,
        "score": _row_conf(row) if has_cls else det_conf,
        "det_conf": det_conf,
    ...
    if detect_pinyin:
        out["class_name"] = detect_pinyin
    if has_cls:
        out["cls_conf"] = float(_effective_cls_conf_for_display(row))
    if infer and infer != name:
        out["infer_name"] = infer
    if cn:
        out["cn_name"] = cn
    elif cn_index or cls_cfg:
        lookup_key = detect_pinyin if cls_disabled else (
            _extract_top1_cls_name(row) or name
        )
        if not lookup_key or _is_placeholder_class_key(lookup_key):
            lookup_key = detect_pinyin or name
        cn_disp = _resolve_cn_display_for_class(
            lookup_key, cn_index=cn_index, cls_cfg=cls_cfg
        )
        if cn_disp and not (
            _is_placeholder_class_key(lookup_key) and cn_disp in ("未知", "昆虫")
        ):
            if cls_disabled or cn_disp != lookup_key:
                out["cn_name"] = cn_disp
    ...
    for k in (
        ...
        "dia_topk_from",
        "dia_topk_to",
        "big_in_big_ior_ratio",
        "big_in_big_ior",
        "small_in_small_ior_ratio",
        "small_in_small_ior",
        "small_in_big_ior_ratio",
        "small_in_big_ior",
    ):
        if k in row:
            if not has_cls and k.startswith("cls_"):
                continue
            out[k] = row[k]
    return out
...
class _ClsRunner:
    ...
    def __init__(
        ...
        *,
        ...
        black_classes: frozenset[str] | None = None,
        only_classes: frozenset[str] | None = None,
        dia_index: dict[str, ClassDiaMeta] | None = None,
        entry_index: dict[str, dict[str, Any]] | None = None,
        cn_index: dict[str, str] | None = None,
        dia_cand_conf_min: float = 0.01,
        size_class_by_name: dict[str, str] | None = None,
        near_switch: bool = True,
    ):
        self.cfg = cfg
        ...
        self._black_classes = black_classes or frozenset()
        self._only_classes = only_classes or frozenset()
        self._dia_index = dia_index or {}
        self._entry_index = entry_index or {}
        self._cn_index = cn_index or {}
        self._dia_cand_conf_min = float(dia_cand_conf_min)
        self._size_class_by_name = size_class_by_name or {}
        self._near_switch = bool(near_switch)
    ...
    def _finalize_cls_result(
        ...
        *,
        ...
        det["cls_conf_orig"] = float(cls_result["conf"])
        _tk = cls_result.get("topk") or cls_result.get("top3") or []
        det["cls_topk"] = list(_tk)
        det["cls_top3"] = det["cls_topk"][:3]
        _stamp_cls_topk_conf_orig_list(det["cls_topk"])
        det["cls_top_n"] = resolve_cls_top_n(self.cfg)
        if self._only_classes:
            apply_only_classes_topk_filter_normalize(
                det, self._only_classes, cls_cfg=self.cfg
            )
        else:
            apply_black_classes_topk_promote(det, self._black_classes)

        # 先按绝对 dia 用 topk 改类，再对前 cls_top_n 归一化，最后判 cls_conf
        if self._entry_index or self._dia_index:
            try_promote_cls_topk_for_dia(
                det,
                root_cfg=self._root_cfg,
                dia_enabled=_resolve_dia_enabled(
                    self.cfg, img_w, fallback_cfg=self._root_cfg
                ),
                dia_diff=resolve_dia_diff(self.cfg),
                dia_index=self._dia_index,
                entry_index=self._entry_index,
                cand_conf_min=self._dia_cand_conf_min,
                cn_index=self._cn_index,
            )

        _apply_cls_top_n_to_row(det, self.cfg)

        top1_name = str(det.get("cls_name", "") or "")
        size_class = try_resolve_threshold_size_class(
            det, None, self._size_class_by_name
        )
        if size_class is not None:
            det["size_class"] = size_class
        thr = resolve_effective_cls_conf_threshold(
            self.alg,
            top1_name,
            self.cfg.get("cls_conf"),
            cls_cfg=self.cfg,
            size_class=size_class,
            size_class_by_name=self._size_class_by_name,
        )
        if thr is not None and float(det.get("cls_conf", 0.0) or 0.0) <= thr:
            ...
            det["cls_conf"] = conf
            return det
        if try_demote_cls_ood_dump_near(
            det,
            alg=self.alg,
            dump_names=self._black_classes,
            near_switch=self._near_switch,
        ):
            if self.route_label:
                logging.info(
                    "[%s] 分类OOD后置other: %s",
                    self.route_label,
                    det.get("cls_other_reason"),
                )
        return det
    ...
    def classify_row(
        ...
        img_w = (
            ...
            if getattr(image_bgr, "shape", None) is not None
            else 0
        )
        ...
        if cache_key in self._prefill_cache:
            ...
            if cached is _PREFILL_CROP_EMPTY:
                ...
                det["cls_top3"] = []
                return det
            return self._finalize_cls_result(row, cached, img_w=img_w)
        ...
        return self._finalize_cls_result(row, cls_result, img_w=img_w)
...
class InsectPredictAll:
    ...
    def __init__(
        ...
        *,
        ...
        root_ids: list[str] | None = None,
        enable_mask_rate_filter: bool | None = None,
    ):
        if config is None or isinstance(config, (str, Path)):
            ...
            apply_dia_mm_pixel_scale(self.config)

        validate_size_conf_keys(self.config)
        ...
        self.device = device
        if enable_mask_rate_filter is None:
            self._enable_mask_rate_filter = resolve_mask_rate_switch(self.config)
        else:
            self._enable_mask_rate_filter = bool(enable_mask_rate_filter)
        logging.info(
            "mask_rate_switch=%s：%s out.mask_rate 填充率过滤",
            "true" if self._enable_mask_rate_filter else "false",
            "启用" if self._enable_mask_rate_filter else "跳过",
        )
        ...
        self._class_dia_index = build_class_dia_index(self.config)
        self._class_route_entry_index = build_class_route_entry_index(self.config)
        self._class_size_class_index = build_class_size_class_index(self.config)
        self._relative_size_rerank_cfg = resolve_relative_size_rerank_cfg(self.config)
        ...
        self._in_big_by_by_root: dict[str, str] = {}
        ...
        self._black_classes = resolve_black_class_names(self.config)
        self._only_classes = resolve_only_class_names(self.config)
        self._near_switch = resolve_near_switch(self.config)
    ...
    def _route_lookup_class_key(self, class_name: str) -> str:
        """
        路由查找用类键：中文展示名先反查配置拼音 key，再 resolve ``out``。

        分类模型若输出中文，直接查 ``out`` 会整批 ``no_route``，报出变空、图上无框。
        """
        key = str(class_name or "").strip()
        if not key:
            return key
        rev = _get_cn_to_class_key_index(self)
        hit = rev.get(key)
    ...
    def _build_roots(self) -> None:
        ...
        for root_id, raw in models_cfg.items():
            ...
            if in_big is not None:
                ...
                in_big_by = resolve_in_big_by(merged)
                if in_big_by != "mask":
                    self._in_big_by_by_root[root_id] = in_big_by
            ...
            sq = resolve_model_square_options(merged, model_type=mtype)
            if mtype == "segment":
                seg_sz = resolve_seg_imgsz(merged)
                if seg_sz > 0:
                    logging.info(
                        "已注册根模型: %s (segment), seg_imgsz=%d, to_square=%s%s",
                        root_id,
                        seg_sz,
                        sq["to_square"],
                        nms_suffix,
                    ...
                    logging.info(
                        "已注册根模型: %s (segment), to_square=%s%s",
                        root_id,
                        sq["to_square"],
                        nms_suffix,
                    )
        ...
        if self._in_big_conf_by_root:
            ...
            by_parts = [
                f"{rid}={mode}"
                for rid, mode in sorted(self._in_big_by_by_root.items())
            ]
            by_msg = (
                f"；in_big_by: {', '.join(by_parts)}"
                if by_parts
                else ""
            )
            skip_det_parts = [
            ...
            logging.info(
                "in_big_conf 小虫与大虫相交过滤: %s%s%s%s%s%s%s",
                ", ".join(
                    f"{rid}={thr:.4f}"
    ...
    def _log_report_tier_mode(self) -> None:
        ...
        if self._only_classes:
            logging.info(
                "分类 only_class 白名单: %d 类（生效时跳过 black_classes）",
                len(self._only_classes),
            )
        if self._black_classes:
            logging.info(
                "分类黑名单 black_classes: %d 类（top1 命中则递补）",
                len(self._black_classes),
            )
        logging.info(
            "near_switch=%s：%s分类 topn 垃圾桶/近种 OOD",
            "true" if self._near_switch else "false",
            "启用" if self._near_switch else "跳过",
        )

    def _apply_black_classes_post(
        self,
        results: list[dict[str, Any]],
        *,
        filtered_acc: list[dict[str, Any]] | None = None,
    ) -> list[dict[str, Any]]:
        """递补 black_classes top1；仍命中则过滤不报出。"""
        if self._only_classes:
            # only_class 作为最高优先级限制时，black_classes 整段跳过
            return results
        black = self._black_classes
        if not black:
            return results
        kept: list[dict[str, Any]] = []
        skipped = 0
        filtered_n = 0
        for row in results:
            if apply_black_classes_topk_promote(
                row,
                black,
                cn_index=self._cn_display_index,
                update_output_name=True,
            ):
                skipped += 1
            name = str(row.get("name") or row.get("cls_name") or "").strip()
            if name and name in black:
                # 相对尺寸等后处理可能把 name 改回黑名单；再从 topk 取首个非黑
                rescued = False
                for item in _sorted_cls_topk_entries(row):
                    cand = _cls_topk_item_name(item)
                    if not cand or cand in black:
                        continue
                    row["name"] = cand
                    row["cls_name"] = cand
                    conf_i = _cls_topk_item_conf(item)
                    if conf_i > 0:
                        row["cls_conf"] = float(conf_i)
                    cn = _resolve_cn_display_for_class(
                        cand, cn_index=self._cn_display_index, cls_cfg=None
                    )
                    if cn:
                        row["cn_name"] = cn
                    rescued = True
                ...
                if not rescued:
                    ...
                    if filtered_acc is not None:
                        fr = dict(row)
                        fr["filtered"] = True
                        fr["filter_reason"] = "black_classes"
                        filtered_acc.append(fr)
                    continue
            kept.append(row)
        if skipped or filtered_n:
            logging.debug(
                "black_classes: 递补 %d 条，过滤 %d 条（保留 %d）",
                skipped,
                filtered_n,
                len(kept),
            )
    ...
    def _apply_cls_ood_post(
        self,
        results: list[dict[str, Any]],
        *,
        filtered_acc: list[dict[str, Any]] | None = None,
    ) -> list[dict[str, Any]]:
        """相对尺寸/黑名单之后再判 OOD，避免改类后漏网。"""
        if not self._near_switch:
            return results
        dump = self._black_classes
        alg = _get_merged_postprocess_alg(self)
        cn_to_key = _get_cn_to_class_key_index(self)
        kept: list[dict[str, Any]] = []
        n_hit = 0
        for row in results:
            if not _row_has_cls_conf(row):
                ...
                if filtered_acc is not None:
                    filtered_acc.append(
                        _finalize_filtered(
                            ...
                            route_entry=None,
                            root_id=str(row.get("source") or ""),
                            reason=str(row.get("cls_other_reason") or "other_cls_ood"),
                            cn_index=self._cn_display_index,
                        )
                    )
                continue
            kept.append(row)
        if n_hit:
            logging.debug("cls_ood: 过滤 %d 条（保留 %d）", n_hit, len(kept))
        return kept
    ...
    def _apply_report_tier_filter(
        ...
        *,
        ...
        for row in results:
            name = str(row.get("name") or row.get("cls_name") or "").strip()
            if _class_matches_report_allowed(
                name,
                allowed,
                aliases,
                cn_index=self._cn_display_index,
    ...
    def _filter_small_in_big_masks(
        ...
        *,
        ...
        for row in results:
            ...
            in_big_by = self._in_big_by_by_root.get(src, "mask")
    ...
    def _collect_trt_cls_jobs_for_rows(
        ...
        *,
        ...
        orig_rows: list[dict[str, Any]] | None = None,
        force_cls: bool = False,
    ) -> list[ClsBatchJob]:
        ...
        for i, row in enumerate(rows):
            if orig_rows is not None:
                orig = orig_rows[i] if i < len(orig_rows) else row
            elif orig_row is not None:
                orig = orig_row
            else:
                orig = row
            if str(row.get("cls_name", "") or "") == "other":
            ...
            if _route_table_wants_direct_output(route_table):
                continue
            cls_key = self._route_lookup_class_key(_primary_class_name(row))
            ...
            if cls_raw is None:
                continue
            cls_merged = _coalesce_cls_node(cls_raw, root_cfg=root_cfg)
            ...
            effective_skip_dia = resolve_effective_skip_dia_thresholds(
                cls_merged, skip_dia
            )
            effective_dia_diff = resolve_dia_diff(cls_merged) or dia_diff
            cls_first = effective_skip_dia.is_configured()
            if cls_first or force_cls:
                need_cls = True
            else:
                need_cls = _passes_entry_thresholds(
                    ...
                    root_cfg,
                    dia_enabled=dia_enabled,
                    skip_dia=effective_skip_dia,
                    dia_diff=effective_dia_diff,
                    size_class_by_name=self._class_size_class_index,
                )
            if not need_cls:
                continue
    ...
    def _batch_collect_trt_cls_jobs_from_pending(
        self,
        pending: list[
        ...
        *,
        ...
        """按 (route_table, route_path, dia) 分组，一次收集整层 pending 的 cls job。"""
        if not pending:
            return []
        groups: dict[tuple[int, str, bool], list[tuple]] = defaultdict(list)
        for item in pending:
            _sim_row, rt, rpath, _orig_row, layer_dia = item
            groups[(id(rt), rpath, layer_dia)].append(item)
        jobs: list[ClsBatchJob] = []
        for items in groups.values():
            rt = items[0][1]
            rpath = items[0][2]
            layer_dia = items[0][4]
            sim_rows = [it[0] for it in items]
            orig_rows = [it[3] for it in items]
            jobs.extend(
    ...
    def _prefill_trt_cls_for_rows(
        ...
        *,
        ...
        with self._phase_timer("cls_batch"):
            ...
            for layer_idx in range(max(1, int(max_layers))):
                t_collect0 = time.perf_counter()
                jobs = self._batch_collect_trt_cls_jobs_from_pending(
                    pending,
                    image_bgr,
                    root_cfg=root_cfg,
                    skip_dia=skip_dia,
                ...
                if not jobs:
                    break
                logging.info(
                    "YOLO 分类 job 批量收集完成: root=%s layer=%d pending=%d jobs=%d 耗时=%.3fs",
                    root_id,
                    layer_idx,
                    len(pending),
                    len(jobs),
                ...
                for sim_row, rt, rpath, orig_row, _layer_dia in pending:
                    cls_key = self._route_lookup_class_key(_primary_class_name(sim_row))
                    _pat, entry = resolve_out_route_entry(
                        rt,
                        cls_key,
                    ...
                    row2["cls_conf_orig"] = float(raw["conf"])
                    _tk = raw.get("topk") or raw.get("top3") or []
                    row2["cls_topk"] = list(_tk)
                    row2["cls_top3"] = row2["cls_topk"][:3]
                    _stamp_cls_topk_conf_orig_list(row2["cls_topk"])
                    row2["cls_top_n"] = resolve_cls_top_n(cls_merged)
                    if self._only_classes:
                        apply_only_classes_topk_filter_normalize(
                            row2, self._only_classes, cls_cfg=cls_merged
                        )
                    else:
                        apply_black_classes_topk_promote(
                            row2, self._black_classes
                        )
    ...
    def _make_cls_runner(
        ...
        *,
        ...
        return _ClsRunner(
            ...
            phase_add=phase_add,
            cache_lock=self._cls_cache_lock,
            black_classes=self._black_classes,
            only_classes=self._only_classes,
            dia_index=self._class_dia_index,
            entry_index=self._class_route_entry_index,
            cn_index=self._cn_display_index,
            dia_cand_conf_min=float(
                self._relative_size_rerank_cfg.get("cand_conf_min", 0.01) or 0.01
            ),
            size_class_by_name=self._class_size_class_index,
            near_switch=self._near_switch,
        )
    ...
    def _create_detect_predictor(self, root_id: str, cfg: dict[str, Any]) -> PredictSize:
        ...
        if is_onnx_detect_backend(mtype, model_raw):
            detect_path = model_raw
        else:
            detect_path = resolve_inference_model_path(cfg, log_label=f"{root_id}/detect")
        class_names = cfg.get("class_names")
        if isinstance(class_names, str):
            class_names = class_names.strip() or None
    ...
    def _emit_detect_size_class_ior_filtered(
        self,
        ...
        *,
        ...
        if not filtered_rows:
            return
        logging.info(
            "%s: root=%s thr=%.3f before=%d after=%d",
            log_label,
            root_id,
            thr,
            n_before,
        ...
        for frow in filtered_rows:
            entry = frow.get(_DETECT_ROUTE_ENTRY_KEY)
            work = _strip_detect_defer_keys(frow)
            filtered_out.append(
                _finalize_filtered(
                    work,
    ...
    @staticmethod
    def _stamp_detect_row_edge_dia_meta(
        rows: list[dict[str, Any]],
        *,
        edge_rule_enabled: bool,
        dia_enabled: bool,
        edge_reject_distance: int,
        edge_reject_conf_threshold: Any,
        edge_reject_cls_conf_threshold: Any,
        bin_dark_ratio_min: float,
    ) -> None:
        """把 detect 根实际生效的边缘/dia 元数据写回 row，供 filter_reason 推断。"""
        for r in rows:
            if not isinstance(r, dict):
                continue
            r["edge_rule_enabled"] = edge_rule_enabled
            r["dia_enabled"] = dia_enabled
            r["edge_reject_distance"] = edge_reject_distance
            r["edge_reject_conf_threshold"] = edge_reject_conf_threshold
            r["edge_reject_cls_conf_threshold"] = edge_reject_cls_conf_threshold
            r["bin_dark_ratio_min"] = bin_dark_ratio_min

    def _apply_detect_geometry_post_filters(
        self,
        ...
        *,
        ...
        big_ior_thr: float,
        small_ior_thr: float,
        small_in_big_ior_thr: float,
        filtered_out: list[dict[str, Any]] | None,
    ) -> list[dict[str, Any]]:
        """同类 IoU 合并 + 大小虫 IoR 过滤（defer 门限路径）。"""
        n_before = len(pending)
        pending = apply_detect_same_class_iou_merge(pending, iou_thr)
        if n_before != len(pending):
            logging.info(
                "detect 同类框 IoU 合并: root=%s thr=%.3f before=%d after=%d",
                root_id,
                iou_thr,
                n_before,
                len(pending),
            )
        n_before_big = len(pending)
        pending, big_filtered = apply_big_in_big_ior_filter(
            pending,
            big_ior_thr,
            size_class_by_name=self._class_size_class_index,
        )
        self._emit_detect_size_class_ior_filtered(
            big_filtered,
            root_id=root_id,
            reason="big_in_big_ior",
            filtered_out=filtered_out,
            log_label="detect 大虫内含大虫 IoR 过滤",
            thr=big_ior_thr,
            n_before=n_before_big,
            n_after=len(pending),
        )
        n_before_small = len(pending)
        pending, small_filtered = apply_small_in_small_ior_filter(
            pending,
            small_ior_thr,
            size_class_by_name=self._class_size_class_index,
        )
        self._emit_detect_size_class_ior_filtered(
            ...
            reason="small_in_small_ior",
            filtered_out=filtered_out,
            log_label="detect 小虫内含小虫 IoR 过滤",
            thr=small_ior_thr,
            n_before=n_before_small,
            n_after=len(pending),
        )
        n_before_sib = len(pending)
        pending, sib_filtered = apply_small_in_big_ior_filter(
            pending,
            small_in_big_ior_thr,
            size_class_by_name=self._class_size_class_index,
        )
        self._emit_detect_size_class_ior_filtered(
            sib_filtered,
            root_id=root_id,
            reason="small_in_big_ior",
            filtered_out=filtered_out,
            log_label="detect 小虫落入大虫 IoR 过滤",
            thr=small_in_big_ior_thr,
            n_before=n_before_sib,
            n_after=len(pending),
        )
        return pending
    ...
    def _emit_detect_route_result(
        ...
        *,
        ...
        cls_cfg = (
            _resolve_cls_cfg_from_route_entry(route_entry)
            if isinstance(route_entry, dict)
            else None
        )
        if _route_entry_cls_disabled(route_entry):
            row = dict(row)
            row.pop("cls_conf", None)
            row.pop("cls_topk", None)
            row.pop("cls_top3", None)
            row.pop("cls_conf_orig", None)
        if (
    ...
    def _finish_detect_rows_after_cls_merge(
        self,
        rows: list[dict[str, Any]],
        *,
        root_id: str,
        ...
        """分类 + 同类 IoU 合并 + 大虫内含过滤之后：补跑暂存的 dia/detect_conf，再叶节点输出。"""
        root_cfg = self._roots.get(root_id, {})
        results: list[dict[str, Any]] = []
        for row in rows:
            pending = list(row.get(_DETECT_PENDING_THR_KEY) or [])
            route_entry = row.get(_DETECT_ROUTE_ENTRY_KEY)
            route_key = row.get(_DETECT_ROUTE_KEY_KEY)
            if isinstance(route_key, str) or route_key is None:
                route_key_s = route_key
            else:
                route_key_s = str(route_key)
            work = _strip_detect_defer_keys(row)
            ...
            for spec in pending:
                ...
                if not isinstance(entry, dict):
                    continue
                skip_dia = coerce_skip_dia_thresholds(
                    spec.get("skip_dia") or spec.get("skip_dia_conf")
                )
                dia_diff = spec.get("dia_diff")
                hop_dia = bool(spec.get("dia_enabled", True))
                if _passes_entry_thresholds(
                    work,
                    entry,
                    root_cfg,
                    dia_enabled=hop_dia,
                ...
                if reason == "threshold_dia":
                    new_entry = try_promote_cls_topk_for_dia(
                        work,
                        root_cfg=root_cfg,
                        dia_enabled=hop_dia,
                        skip_dia=skip_dia,
                        ...
                        entry_index=self._class_route_entry_index,
                        cand_conf_min=float(
                            self._relative_size_rerank_cfg.get("cand_conf_min", 0.01)
                            or 0.01
                        ),
                        cn_index=self._cn_display_index,
                    )
                    if new_entry is not None and _passes_entry_thresholds(
                        work,
                        new_entry,
                        root_cfg,
                        dia_enabled=hop_dia,
                        ...
                        route_entry = new_entry
                        continue
                failed_spec = spec
                break
            if failed_spec is not None:
                if filtered_out is not None:
                    entry = failed_spec.get("entry")
                    entry_d = entry if isinstance(entry, dict) else None
                    filtered_out.append(
                        _finalize_filtered(
                            work,
                            route_entry=entry_d,
                            ...
                            reason=_threshold_filter_reason(
                                ...
                                entry_d or {},
                                root_cfg,
                                dia_enabled=bool(failed_spec.get("dia_enabled", True)),
                                skip_dia=coerce_skip_dia_thresholds(
                                    failed_spec.get("skip_dia")
                                    or failed_spec.get("skip_dia_conf")
                                ),
                                dia_diff=failed_spec.get("dia_diff"),
                                size_class_by_name=self._class_size_class_index,
                            ),
                            cn_index=self._cn_display_index,
                        )
                ...
                continue
            emit_entry = route_entry if isinstance(route_entry, dict) else None
            results.extend(
                self._emit_detect_route_result(
                    work,
                    route_entry=emit_entry,
                    route_key=route_key_s,
                    root_id=root_id,
                    filtered_out=filtered_out,
                )
            )
    ...
    def _try_recover_entry_via_dia_topk(
        self,
        row_for_label: dict[str, Any],
        *,
        entry: dict[str, Any],
        thr_reason: str,
        root_cfg: dict[str, Any],
        dia_enabled: bool,
        effective_skip_dia: SkipDiaThresholds | None,
        effective_dia_diff: float | None,
        matched_route_key: str,
    ) -> tuple[dict[str, Any], dict[str, Any], str] | None:
        """
        父级门限失败时：仅 ``threshold_dia`` 尝试按 topk 改类恢复。

        成功返回 ``(row_work, entry, matched_route_key)``；否则 ``None``（调用方过滤）。
        """
        if thr_reason != "threshold_dia":
            return None
        new_entry = try_promote_cls_topk_for_dia(
            row_for_label,
            root_cfg=root_cfg,
        ...
        if new_entry is None or not _passes_entry_thresholds(
            row_for_label,
            new_entry,
            root_cfg,
            dia_enabled=dia_enabled,
            ...
            return None
        new_key = str(
            new_entry.get("infer_name") or matched_route_key
        ).strip() or matched_route_key
        return row_for_label, new_entry, new_key
    ...
    def _walk_route(
        ...
        *,
        ...
        defer_entry_thresholds: bool = False,
    ) -> list[dict[str, Any]]:
        ...
        def _leaf(
            ...
            *,
            ...
            if defer_entry_thresholds:
                out = dict(row_leaf)
                out[_DETECT_ROUTE_ENTRY_KEY] = route_entry
                out[_DETECT_ROUTE_KEY_KEY] = route_key
                return [out]
        ...
        if route_table is not None:
            ...
            cls_route_label = ""
            if cls_raw is not None:
                cls_merged = _coalesce_cls_node(
                    cls_raw, root_cfg=root_cfg_for_dia
                )
                cls_route_label = f"{route_path}.out[{cls_key}].models.cls"
            ...
            cls_first = bool(
                ...
                and effective_skip_dia.is_configured()
            )
            if (
                defer_entry_thresholds
                and cls_merged is not None
                and _is_enabled(cls_merged)
                and cls_merged.get("model")
            ):
                cls_first = True
            ...
            if defer_entry_thresholds:
                if row_work is row:
                    row_work = dict(row)
                row_work = _record_detect_pending_threshold(
                    row_work,
                    entry=entry,
                    skip_dia=effective_skip_dia,
                    dia_diff=effective_dia_diff,
            ...
            elif not _passes_entry_thresholds(
                row_work,
                entry,
                root_cfg,
                dia_enabled=dia_enabled,
                ...
                # 父级 detect_conf/dia 在分类前拦截时，仍补跑分类供绘图 / topk 改类
                if (
                    cls_merged
                    and _is_enabled(cls_merged)
                    and not cls_first
                    ...
                    if row2 is not None:
                        row_for_label = row2
                thr_reason = _threshold_filter_reason(
                    row_for_label,
                    entry,
                    root_cfg,
                    dia_enabled=dia_enabled,
                ...
                recovered = self._try_recover_entry_via_dia_topk(
                    row_for_label,
                    entry=entry,
                    thr_reason=thr_reason,
                    root_cfg=root_cfg,
                    dia_enabled=dia_enabled,
                    effective_skip_dia=effective_skip_dia,
                    effective_dia_diff=effective_dia_diff,
                    matched_route_key=matched_route_key,
                )
                if recovered is not None:
                    row_work, entry, matched_route_key = recovered
                else:
                    if filtered_out is not None:
                        filtered_out.append(
                            _finalize_filtered(
            ...
            if cls_merged is not None and _is_enabled(cls_merged):
                ...
                return self._walk_route(
                    ...
                    defer_entry_thresholds=defer_entry_thresholds,
                )
            return _leaf(
                row_work,
                route_entry=entry,
    ...
    def _run_detect_root(
        ...
        *,
        ...
        _clip_profiles, _clip_batch, _predict_clip = _predict_clip_kwargs_from_cfg(
            cfg, global_cfg=self.config
        )
        ...
        with self._phase_timer("detect"):
            rows = predictor.predict(
                ...
                **_predict_clip,
                ...
                cls_pad_square=sq["to_square"],
                cls_gray_binarize=bool(cfg.get("gray_binarize", False)),
                detect_pad_square=sq["to_square"],
                detect_pad_square_full_image=sq["to_square"],
                ...
                split_by_scale=bool(cfg.get("split_by_scale", False)),
                split_if_contain=int(cfg.get("split_if_contain", 2) or 0),
            )
            self._stamp_detect_row_edge_dia_meta(
                rows,
                edge_rule_enabled=_edge_rule_enabled,
                dia_enabled=_dia_enabled,
                edge_reject_distance=_edge_reject_distance,
                edge_reject_conf_threshold=_edge_reject_conf_threshold,
                edge_reject_cls_conf_threshold=_edge_reject_cls_conf_threshold,
                bin_dark_ratio_min=_bin_dark_ratio_min_eff,
            )
        ...
        iou_thr = resolve_detect_iou_same_class(cfg)
        big_ior_thr = resolve_big_in_big_ior(cfg)
        small_ior_thr = resolve_small_in_small_ior(cfg)
        small_in_big_ior_thr = resolve_small_in_big_ior(cfg)
        defer_thr = (
            iou_thr > 0.0
            or big_ior_thr > 0.0
            or small_ior_thr > 0.0
            or small_in_big_ior_thr > 0.0
        )
        self._prefill_trt_cls_for_rows(
            ...
            root_cfg=cfg,
            dia_enabled=_dia_enabled,
            skip_dia=resolve_skip_dia_thresholds(cfg),
            dia_diff=resolve_dia_diff(cfg),
            img_w=_img_w,
            force_cls=defer_thr,
        )
        results: list[dict[str, Any]] = []
            ...
            for row in rows:
                ...
                pending.extend(
                    self._walk_route(
                        ...
                        defer_entry_thresholds=defer_thr,
                    )
                )
            if defer_thr:
                pending = self._apply_detect_geometry_post_filters(
                    pending,
                    root_id=root_id,
                    iou_thr=iou_thr,
                    big_ior_thr=big_ior_thr,
                    small_ior_thr=small_ior_thr,
                    small_in_big_ior_thr=small_in_big_ior_thr,
                    filtered_out=filtered_out,
                )
                results.extend(
                    self._finish_detect_rows_after_cls_merge(
                        pending,
                        root_id=root_id,
                        filtered_out=filtered_out,
                    )
    ...
    def _run_segment_root(
        ...
        *,
        ...
        _img_w = int(image_bgr.shape[1]) if getattr(image_bgr, "shape", None) is not None else 0
        _dia_enabled = _resolve_dia_enabled(cfg, _img_w)
        _, _, _predict_clip = _predict_clip_kwargs_from_cfg(
            cfg, global_cfg=self.config
        )
        _seg_root_t0 = time.perf_counter()
        with self._phase_timer("seg"):
            _seg_infer_t0 = time.perf_counter()
            rows = predictor.predict(
                image_bgr,
                **_predict_clip,
                padding=sq["to_square"],
                pad_full_image_to_square=sq["to_square"],
            ...
            _seg_infer_t1 = time.perf_counter()
            _seg_raw_count = len(rows)
            for row in rows:
                if isinstance(row, dict):
                    normalized = ensure_row_has_bbox_fields(row)
            ...
            _seg_post_t1 = time.perf_counter()

        logging.info(
            "分割 YOLO 推理完成: root=%s raw_instances=%d 耗时=%.3fs",
            root_id,
            _seg_raw_count,
            _seg_infer_t1 - _seg_infer_t0,
        )
        logging.info(
            "分割后处理完成: root=%s passed=%d 耗时=%.3fs (filter/dia)",
            root_id,
            len(rows),
            _seg_post_t1 - _seg_infer_t1,
        )
        ...
        _prefill_t1 = time.perf_counter()
        logging.info(
            "分割 cls 预填完成: root=%s rows=%d 耗时=%.3fs (job 收集+批量推理)",
            root_id,
            len(rows),
            _prefill_t1 - _prefill_t0,
        )
        ...
        _route_t1 = time.perf_counter()
        logging.info(
            "分割路由遍历完成: root=%s rows=%d results=%d 耗时=%.3fs",
            root_id,
            len(rows),
            len(results),
            _route_t1 - _route_t0,
        ...
        logging.info(
            "分割根节点耗时汇总: root=%s yolo=%.3fs post=%.3fs prefill=%.3fs route=%.3fs total=%.3fs",
            root_id,
            _seg_infer_t1 - _seg_infer_t0,
            _seg_post_t1 - _seg_infer_t1,
            _prefill_t1 - _prefill_t0,
            _route_t1 - _route_t0,
            _route_t1 - _seg_root_t0,
        )
        return results
        ...
        *,
        ...
        try:
            ...
            for root_id, cfg in self._roots.items():
                ...
                mtype = str(cfg.get("model_type", "detect")).strip().lower()
                pipeline_type = pipeline_type_of(mtype)
                if not pipeline_type:
                    continue
                root_tasks.append((root_id, cfg, pipeline_type, root_image, roi_circle))
            ...
            with self._phase_timer("post"):
                ...
                all_results = apply_relative_size_rerank(
                    all_results,
                    dia_index=self._class_dia_index,
                    cn_index=self._cn_display_index,
                    cfg=self._relative_size_rerank_cfg,
                    filtered_out=filtered_acc,
                    finalize_filtered=_finalize_filtered,
                    image_size=(
                        int(image_bgr.shape[1]),
                        int(image_bgr.shape[0]),
                    ),
                )
                all_results = self._apply_black_classes_post(
                    all_results, filtered_acc=filtered_acc
                )
                all_results = self._apply_cls_ood_post(
                    all_results, filtered_acc=filtered_acc
                )
    ...
    def release(self) -> None:
        ...
        clear_yolo_cache()
        try:
            from script.predict.model_detect_onnx import clear_onnx_detect_cache

            clear_onnx_detect_cache()
        except Exception:
            pass
...
def _eval_species_identity_name(r: dict[str, Any]) -> str:
    """过滤/粗类改写后，仍用于和 GT 比对的物种名（与框上中文名一致）。"""
    top1 = _extract_top1_cls_name(r)
    if (
        top1
        and not _is_other_class_name(top1)
        and top1.lower() not in ("insect", "unknown")
        and top1 != "昆虫"
    ):
        return top1
    for key in ("viz_name", "cn_name", "cls_name_top1"):
        val = str(r.get(key) or "").strip()
        if (
            val
            and not _is_other_class_name(val)
            and val.lower() not in ("insect", "unknown")
            and val != "昆虫"
        ):
            return val
    return ""
...
def _is_eval_coarse_class_name(name: str | None) -> bool:
    s = str(name or "").strip()
    if not s:
        return True
    if _is_other_class_name(s):
        return True
    return s.lower() in ("insect", "unknown") or s == "昆虫"


def _iter_eval_pred_class_names(
    r: dict[str, Any],
    *,
    cn_index: dict[str, str] | None = None,
) -> list[str]:
    """校验比对用的预测类名候选：优先框上中文显示名，再退回路由/top1。"""
    ordered: list[str] = []
    for val in (
        str(r.get("viz_name") or "").strip(),
        str(r.get("cn_name") or "").strip(),
        _label_display_name(r, cn_index=cn_index),
        _eval_species_identity_name(r),
        str(r.get("name") or "").strip(),
        str(r.get("cls_name") or "").strip(),
        str(r.get("cls_name_top1") or "").strip(),
        str(r.get("infer_name") or "").strip(),
        _extract_top1_cls_name(r),
    ):
        s = str(val or "").strip()
...
def _eval_pred_matches_gt(
    ...
    *,
    ...
    """统计与画图共用：任一候选类名与 GT 等价即 TP。"""
    gt = str(gt_name or "").strip()
    if not gt:
        return False
    for pred_cls in _iter_eval_pred_class_names(row, cn_index=cn_index):
        if is_class_match(
            pred_cls,
            gt,
            merge,
...
def _eval_row_pred_class_name(
    ...
    *,
    ...
    """校验用预测类名：优先框上显示名，避免 insect/other 路由名与 GT 中文对不上。"""
    if use_cn_name:
        return _label_display_name(r, cn_index=cn_index)
    if use_infer_name:
        return str(r.get("infer_name") or r.get("name") or "unknown")
    names = _iter_eval_pred_class_names(r, cn_index=cn_index)
    for s in names:
        if not _is_eval_coarse_class_name(s):
            return s
    return names[0] if names else "unknown"


def _eval_row_is_ignored(
    row: dict[str, Any],
    ignore_set: frozenset[str] | set[str] | None,
    merge: dict[str, list[str]] | None,
    *,
    label_alias_map: dict[str, str] | None = None,
) -> bool:
    """任一候选类名（路由名或框上中文）命中忽略清单则整框不参与校验。"""
    if not ignore_set:
        return False
    names = _iter_eval_pred_class_names(row)
    if not names:
        names = [str(row.get("cls_name") or row.get("name") or "")]
    return any(
        is_eval_ignored_class(nm, ignore_set, merge, label_alias_map=label_alias_map)
        for nm in names
    )


def _eval_pred_stat_class_name(
    row: dict[str, Any],
    merge: dict[str, list[str]] | None,
    label_alias_map: dict[str, str] | None,
) -> str:
    """按类统计 / 比赛识别数：优先报出 ``name``（与 XML ``<name>`` 一致）；仅报出框计入。"""
    name = str(row.get("name") or "").strip()
    if name:
        return normalize_class_name(
            name, merge, label_alias_map=label_alias_map
        )
    return normalize_class_name(
        _eval_row_pred_class_name(row),
        merge,
        label_alias_map=label_alias_map,
    )
...
def _result_row_to_pred_dict(
    ...
    *,
    ...
    if _row_has_cls_conf(r):
        out["cls_conf"] = _row_cls_conf(r)
    for k in (
        "name",
        "cls_name_top1",
        "cls_conf_top1",
        ...
        "viz_name",
        "cn_name",
        COMPETITION_ORIG_NAME_KEY,
    ):
        if k in r:
            out[k] = r[k]
    if "cls_conf_orig" not in out:
        topk = r.get("cls_topk") or r.get("cls_top3") or []
        if topk and isinstance(topk[0], dict) and topk[0].get("conf_orig") is not None:
            try:
                out["cls_conf_orig"] = float(topk[0].get("conf_orig") or 0.0)
            except (TypeError, ValueError):
                pass
        elif _row_has_cls_conf(r):
            out["cls_conf_orig"] = _row_cls_conf(r)
...
def _is_dia_size_filtered_row(r: dict[str, Any]) -> bool:
    """因对角线/尺寸门限进入 filtered 的实例（含 ``threshold_dia`` / ``dia`` / ``size``）。"""
    reason = str(r.get("filter_reason") or r.get("cls_other_reason") or "")
    return _is_dia_threshold_filter_reason(
        reason, dia_enabled=bool(r.get("dia_enabled", True))
    )


def _row_has_eval_bbox(r: dict[str, Any]) -> bool:
    """过滤框是否带可参与 IoU 匹配的有效 bbox。"""
    loc = r.get("location")
    if isinstance(loc, (list, tuple)) and len(loc) >= 4:
        try:
            x1, y1, x2, y2 = (int(loc[0]), int(loc[1]), int(loc[2]), int(loc[3]))
...
def _is_filtered_geom_match_row(r: dict[str, Any]) -> bool:
    """
    可进入评估「仅几何匹配」pred 池的 filtered 行。

    凡是带有效框、会被画出的过滤实例都参与匹配：图上名字已对上却因
    ``report_tier`` / ``in_big`` / ``threshold_*`` 等白名单漏掉时会误计漏报。
    未匹配的 filtered 仍不计 FP（``match_only_other``）。

    竞赛单类抑制（``competition_single_class_count``）不进评估池：真正不报出、
    不抬识别数；仍由 XML 导出路径单独写出。
    """
    if _is_competition_single_class_filtered(r):
        return False
    if not _row_has_eval_bbox(r):
        return False
    if bool(r.get("filtered")):
        return True
    reason = str(r.get("filter_reason") or r.get("cls_other_reason") or "").strip()
    if reason:
        return True
    return _is_filtered_other_row(r) or _is_dia_size_filtered_row(r)
...
def _build_eval_pred_pool(
    ...
    """
    构建验证用 pred 池：保留 results，并追加部分 filtered 实例仅参与几何匹配。

    追加范围见 ``_is_filtered_geom_match_row``（带有效框的 filtered 均参与）。

    返回 (pred_rows, match_only_other_flags, filtered_geom_indices)。
    ``match_only_other=True`` 的 pred 若未匹配 GT 不计 FP；匹配后按 ``is_class_match`` 计 TP 或类型错。
    ``filtered_geom_indices`` 为上述 filtered 在 ``filtered`` 列表中的下标。
    """
    preds = [_result_row_to_pred_dict(r) for r in results]
    match_only_other = [False] * len(preds)
    filtered_geom_indices: list[int] = []
    for fi, r in enumerate(filtered or []):
        if not _is_filtered_geom_match_row(r):
            continue
        preds.append(_result_row_to_pred_dict(r))
        match_only_other.append(True)
        filtered_geom_indices.append(fi)
    return preds, match_only_other, filtered_geom_indices
...
def save_pascal_voc_xml_for_results(
    ...
    *,
    ...
    ``filtered`` 中与评估几何匹配池一致的实例（other / 尺寸对角线过滤等）一并写出，
    并带 ``filter_reason``（见 ``_is_filtered_geom_match_row``）。
    竞赛单类抑制框不进评估池，但仍写出并带 ``filter_reason``。
    """
    out_p = Path(out_dir)
    ...
    for r in filtered or []:
        if not (
            _is_filtered_geom_match_row(r)
            or (
                _is_competition_single_class_filtered(r)
                and _row_has_eval_bbox(r)
            )
        ):
            continue
        row = _result_row_to_pred_dict(
            r,
            use_infer_name=not use_cn_name,
            use_cn_name=use_cn_name,
            cn_index=cn_index,
...
def _accumulate_predict_all_filename_gt_validation(
    ...
    *,
    ...
    fuzzy_only_wildcard: bool = False,
    tier_equivalence: ClassTierEquivalence | None = None,
    eval_ignore_classes: frozenset[str] | None = None,
    ...
    ignore_set = eval_ignore_classes or frozenset()
    if not gt_name or is_metric_ignored_other(gt_name, merge):
        return [], set(), preds
    if is_eval_ignored_class(
        gt_name, ignore_set, merge, label_alias_map=label_alias_map
    ):
        return [], set(), preds
    ...
    pred_eval_idx = [
        ...
        and not is_eval_ignored_class(
            str(r.get("cls_name", "") or ""),
            ignore_set,
            merge,
            label_alias_map=label_alias_map,
        )
    ...
    for i in pred_eval_idx:
        ...
        if is_class_match(
            ...
            if (
                pred_norm
                and not _is_other_class_name(pred_norm)
                and not _is_eval_coarse_class_name(pred_norm)
            ):
                _inc_stat_by_cls(stat_by_cls, gt_norm, "report_tp", 1)
        else:
...
def _annotate_filename_gt_eval_tags(
    ...
    *,
    ...
    ignore_set = eval_ignore_classes or frozenset()
    if is_eval_ignored_class(
        gt_name, ignore_set, class_merge, label_alias_map=label_alias_map
    ):
        return []
    for r in results_for_draw:
        pred_cls = str(r.get("name", "") or "")
        if is_eval_ignored_class(
            pred_cls, ignore_set, class_merge, label_alias_map=label_alias_map
        ):
            continue
...
def _accumulate_predict_all_validation(
    ...
    *,
    ...
    被过滤的实例仍参与与 GT 的几何匹配：匹配成功计 TP/类型错，避免 GT 误计漏报。
    比赛识别数 ``pred`` **只计报出框**（``results``）；filtered 无论是否匹配均不抬识别数、
    未匹配也不计 FP（与竞赛改 other / 阈值滤框「未报出」一致）。
    匹配优先级：① 报出框（``results``）优于 filtered 仅几何匹配框；② 同类优于异类；③ 几何分高者优先。
    ...
    def _is_ignored_name(name: str) -> bool:
        return is_eval_ignored_class(
            name, ignore_set, merge, label_alias_map=label_alias_map
        )

    def _pred_skip(r: dict, *, for_match_only_other: bool = False) -> bool:
        if _eval_row_is_ignored(
            r, ignore_set, merge, label_alias_map=label_alias_map
        ):
            return True
        name = str(r.get("cls_name", "") or "")
        if for_match_only_other:
            return False
        return is_metric_ignored_other(name, merge)
    ...
    pred_eval_idx = [
        i
        for i, r in enumerate(preds)
        if not _pred_skip(r, for_match_only_other=match_only_other[i])
    ]
    ...
    if not gts:
        ...
        for i in fp_idx:
            pn = _eval_pred_stat_class_name(preds[i], merge, label_alias_map)
    ...
    for i in pred_eval_idx:
        ...
        _inc_stat_by_cls(
            stat_by_cls,
            _eval_pred_stat_class_name(preds[i], merge, label_alias_map),
            "pred",
            1,
        )
    ...
    for i in range(len(preds)):
        if match_only_other[i]:
            ...
            continue
        pred_norm = _eval_pred_stat_class_name(preds[i], merge, label_alias_map)
        if i not in matched_p:
            stat_total["fp"] += 1
            _inc_stat_by_cls(stat_by_cls, pred_norm, "fp", 1)
        else:
            ...
            if _eval_pred_matches_gt(
                ...
                stat_total["tp"] += 1
                _inc_stat_by_cls(stat_by_cls, gt_norm, "tp", 1)
                # 比赛识别数：仅报出框，且报出 name（非 other/粗类）与 GT 同类
                if (
                    not match_only_other[i]
                    and pred_norm
                    and not _is_other_class_name(pred_norm)
                    and not _is_eval_coarse_class_name(pred_norm)
                    and is_class_match(
...
def _annotate_validation_eval_tags(
    ...
    *,
    ...
    """为 results / 仅几何匹配的 filtered 写入 eval_tag，并返回未匹配的漏报 GT 框列表。"""
    ...
    def _ignored(name: str) -> bool:
        return is_eval_ignored_class(
            name, ignore_set, class_merge, label_alias_map=label_alias_map
        )

    def _row_ignored(row: dict[str, Any]) -> bool:
        return _eval_row_is_ignored(
            row, ignore_set, class_merge, label_alias_map=label_alias_map
        )

    def _tag_class_match(row: dict[str, Any], gt_name: str) -> None:
        if _eval_pred_matches_gt(
            row,
            gt_name,
            class_merge,
            ...
            row["eval_tag"] = "tp"
            return
        row["eval_tag"] = "cls_err"
        row["eval_gt_name"] = gt_name

    for pi in range(n_result_preds):
        if _row_ignored(results_for_draw[pi]):
            continue
        if pi not in mset:
        ...
        _tag_class_match(results_for_draw[pi], gt_name)
    ...
    if filtered_for_draw is not None:
        for oi, fi in enumerate(filtered_other_indices):
            ...
            if gj is None or gj < 0 or gj >= len(gts):
                continue
            gt_name = str(gts[gj].get("name", "") or "")
            if _row_ignored(filtered_for_draw[fi]) or _ignored(gt_name):
                continue
            _tag_class_match(filtered_for_draw[fi], gt_name)
...
def _eval_tag_border_bgr(eval_tag: str) -> tuple[int, int, int] | None:
    """校验框颜色：有 eval_tag 时返回 TP/类型错/FP/FN 色，与是否关注类无关。"""
...
def _label_display_name(
    ...
    *,
    ...
    """绘图/XML 中文类名：检测拼音查 ``out.cn_name`` / ``cn_index``，跳过 ``unknown``→未知。"""
    for key in _iter_class_keys_for_cn_lookup(r):
        resolved = _resolve_cn_display_for_class(
            key, cn_index=cn_index, cls_cfg=cls_cfg
        )
    ...
    if viz:
        if cn_index:
            ...
            if hit:
                return hit
            resolved = _resolve_cn_display_for_class(
                viz, cn_index=cn_index, cls_cfg=cls_cfg
            )
    ...
    if cn:
        return cn
    keys = _iter_class_keys_for_cn_lookup(r)
    if keys:
        return keys[0]
    return str(r.get("name") or "unknown").strip() or "unknown"


def _row_has_draw_conf(row: dict[str, Any]) -> bool:
    """预测/过滤框带置信度字段；漏报 GT 通常没有。"""
    return any(row.get(k) is not None for k in ("det_conf", "cls_conf", "conf", "score"))


def _format_det_cls_conf_text(row: dict[str, Any]) -> str:
    """检出分；有分类分时为 ``0.45/0.98``，检测直出仅 ``0.45``。"""
    det_conf = _row_detect_conf(row)
    if not _row_has_cls_conf(row):
        return f"{det_conf:.2f}"
    cls_conf = _effective_cls_conf_for_display(row)
    return f"{det_conf:.2f}/{cls_conf:.2f}"


def _format_clear_box_label(
    row: dict[str, Any], *, cn_index: dict[str, str] | None = None
) -> str:
    """清爽标签：``隐翅虫0.45/0.98``（检出/分类）；无分类分时 ``隐翅虫0.45``；无置信度时只绘类名。"""
    name = _label_display_name(row, cn_index=cn_index)
    if not _row_has_draw_conf(row):
        return name
    return f"{name}{_format_det_cls_conf_text(row)}"


def _resolve_drawn_box_label(
    row: dict[str, Any],
    full: str,
    *,
    draw_label_clear: bool = False,
    eval_ignore_classes: frozenset[str] | None = None,
    cn_index: dict[str, str] | None = None,
    class_merge: dict[str, list[str]] | None = None,
    label_alias_map: dict[str, str] | None = None,
) -> str:
    """清爽模式绘类名+双置信度；评估忽略类未开清爽时只留类别名。"""
    if draw_label_clear:
        return _format_clear_box_label(row, cn_index=cn_index)
    if is_eval_ignored_class(
        str(row.get("name") or row.get("infer_name") or ""),
        eval_ignore_classes,
        class_merge,
        label_alias_map=label_alias_map,
    ):
        return _label_display_name(row, cn_index=cn_index)
    return full
...
def _format_result_label(
    ...
    conf_part = _format_det_cls_conf_text(r)
    ...
    if show_source:
        ...
        if src:
            return (
                f"{display} {conf_part}{dist_part}{in_big_part}"
...
def _humanize_other_filter_reason(
    ...
    *,
    ...
    if r == "other_cls_none":
        return "分类器无结果"
    if r.startswith("other_cls_ood:"):
        body = r[len("other_cls_ood:") :]
        nm = body.split("(", 1)[0].strip()
        disp = _resolve_cn_display_for_class(nm, cn_index=cn_index) if nm else nm
        return f"分类OOD垃圾桶→其他 top1={disp}"
...
def _format_filter_reason_for_label(
    ...
    *,
    ...
    if reason == "big_in_big_ior":
        ratio = r.get("big_in_big_ior_ratio")
        try:
            if ratio is not None:
                return f"big_in_big_ior:{float(ratio):.2f}"
        except (TypeError, ValueError):
            pass
        return "big_in_big_ior"
    if reason == "small_in_small_ior":
        ratio = r.get("small_in_small_ior_ratio")
        try:
            if ratio is not None:
                return f"small_in_small_ior:{float(ratio):.2f}"
        except (TypeError, ValueError):
            pass
        return "small_in_small_ior"
    if reason == "small_in_big_ior":
        ratio = r.get("small_in_big_ior_ratio")
        try:
            if ratio is not None:
                return f"small_in_big_ior:{float(ratio):.2f}"
        except (TypeError, ValueError):
            pass
        return "small_in_big_ior"
    if reason == "relative_size_no_candidate":
        return "relative_size_no_candidate"
    if reason == COMPETITION_FILTER_REASON:
        return "竞赛单类头数不足"
...
def _format_draw_result_label(
    ...
    *,
    ...
    ``draw_label_errors_only=False`` 时 TP 绘「中文类名 det/cls」（至多两位小数）；
    cls_err/fp/fn 等仍用完整评估标签。无 ``eval_tag`` 时回退推理标签（含置信度等）。
    """
...
def draw_results(
    ...
    *,
    ...
    ``draw_label_errors_only=False`` 时校验 TP 标签绘「中文类名 det/cls」；错误框仍保留完整评估标签。
    ``draw_label_errors_only=True`` 时校验 TP 仍画框但不画标签，仅 cls_err/fp/fn 等保留文字标签。
    ``draw_label_clear=True`` 时实际绘制的文字为 ``隐翅虫0.45/0.98``（类名+检出/分类）；
    漏报 GT 无置信度时只绘类名。不改是否画字。
    ``eval_ignore_classes`` 命中的类同样只绘类别名（不必开清爽模式）。
    ...
    def _box_label_text(row: dict[str, Any], full: str) -> str:
        return _resolve_drawn_box_label(
            row,
            full,
            draw_label_clear=draw_label_clear,
            eval_ignore_classes=eval_ignore_classes,
            cn_index=cn_index,
            class_merge=class_merge,
            label_alias_map=label_alias_map,
        )
    ...
    if eval_boxes and draw_bbox:
        for b in eval_boxes:
            ...
            eval_color = _eval_tag_border_bgr(str(b.get("eval_tag") or ""))
    ...
    if draw_filter and filtered_results:
        for r in filtered_results:
            ...
            eval_color = _eval_tag_border_bgr(eval_tag) if eval_tag else None
...
def _first_cls_node_cfg(config: dict[str, Any] | None) -> dict[str, Any] | None:
    """首个嵌套分类节点（含 ``model`` 与 ``cls_conf``），供体型门限读取。"""
...
def _demote_row_cls_conf_for_resume(
    ...
    *,
    ...
    """
    续跑时按当前 JSON 重放分类 top1 门限与 OOD；需过滤则返回 filtered 行，否则 None。

    与线上一致：先尝试 dia topk 改类，再判 ``cls_conf``，再判 ``p_dump``/``p_near``/``min_near``（受 ``near_switch`` 约束）。
    """
    work = dict(row)
    if pipeline is not None:
        rel_cfg = getattr(pipeline, "_relative_size_rerank_cfg", None) or {}
        pp = _postprocess_block(getattr(pipeline, "config", None) or {})
        cls_dia_diff = rel_cfg.get("dia_diff")
        if cls_dia_diff is None:
            cls_dia_diff = _relative_size_cfg_value(
                pp, "dia_diff", "relative_size_dia_diff", 0.3
            )
        if cls_dia_diff is None:
            cls_dia_diff = 0.3
        try:
            cls_dia_diff_f = float(cls_dia_diff)
        except (TypeError, ValueError):
            cls_dia_diff_f = 0.3
        cls_raw = _first_cls_node_cfg(getattr(pipeline, "config", None)) or {}
        if not isinstance(cls_raw, dict):
            cls_raw = {}
        root_like = {"dia_switch": pp.get("dia_switch", True)}
        cls_for_dia = _coalesce_cls_node(cls_raw, root_cfg=root_like)
        try_promote_cls_topk_for_dia(
            work,
            root_cfg={},
            dia_enabled=_resolve_dia_enabled(cls_for_dia, 0, fallback_cfg=root_like),
            dia_diff=cls_dia_diff_f,
            dia_index=getattr(pipeline, "_class_dia_index", None) or {},
            entry_index=getattr(pipeline, "_class_route_entry_index", None) or {},
            cand_conf_min=float(rel_cfg.get("cand_conf_min", 0.01) or 0.01),
            cn_index=getattr(pipeline, "_cn_display_index", None),
        )
        row.update(work)

    top1 = str(work.get("cls_name") or work.get("name") or "").strip()
    if not top1 or _is_other_class_name(top1):
        return None
    if not _row_has_cls_conf(work):
        return None
    try:
        conf = float(work.get("cls_conf", 0.0) or 0.0)
    except (TypeError, ValueError):
        conf = 0.0
    thr = resolve_effective_cls_conf_threshold(
        alg,
        top1,
        global_cls_conf,
        cls_cfg=_first_cls_node_cfg(
            getattr(pipeline, "config", None) if pipeline is not None else None
        ),
        size_class_by_name=(
            getattr(pipeline, "_class_size_class_index", None)
            if pipeline is not None
            else None
        ),
    )
    if thr is not None and conf <= float(thr):
        ...
        return det
    dump_names = (
        resolve_black_class_names(getattr(pipeline, "config", None) or {})
        if pipeline is not None
        else frozenset()
    )
    cn_to_key = (
        _get_cn_to_class_key_index(pipeline) if pipeline is not None else None
    )
    ood_work = dict(work)
    if try_demote_cls_ood_dump_near(
        ood_work,
        ...
        cn_to_key=cn_to_key,
        near_switch=resolve_near_switch(
            getattr(pipeline, "config", None) if pipeline is not None else None
        ),
    ):
        reason = str(ood_work.get("cls_other_reason") or "other_cls_ood")
        ood_work["name"] = "other"
        ood_work["filter_reason"] = reason
        ood_work["filtered"] = True
        return ood_work
    return None
...
def _reapply_resume_postprocess_filters(
    ...
    *,
    ...
    与线上一致：dia topk 改类 → 分类 ``cls_conf`` 门限 → ``report_tier`` 报出类过滤。
    """
    if not rows:
        return [], []
    ...
    filtered: list[dict[str, Any]] = []
    for row in rows:
        # xml 中已带 filter_reason / filtered 的实例直接归入 filtered，不再当报出框重放
        if bool(row.get("filtered")) or str(row.get("filter_reason") or "").strip():
            fr = dict(row)
            fr["filtered"] = True
            if not str(fr.get("filter_reason") or "").strip():
                fr["filter_reason"] = str(fr.get("cls_other_reason") or "filtered")
            filtered.append(fr)
            continue
        ...
        demoted = _demote_row_cls_conf_for_resume(
            norm, alg, global_cls_conf, pipeline=pipeline
        )
    ...
    kept = pipeline._apply_black_classes_post(kept, filtered_acc=filtered)
...
def _save_predict_all_type_confusion_crops(
    *,
    ...
    n_result_preds: int | None = None,
    eval_ignore_classes: frozenset[str] | None = None,
) -> None:
    """类型匹配切图（tp / cls_err / 漏检）：写入 eval_metrics/all/type_confusion_crops*。"""
    ...
    if gt_source == "filename":
        _save_type_confusion_crops_for_filename_gt(
            ...
            ignore_classes=eval_ignore_classes,
        )
        return
    if not gts:
        return
...
class _DemoPostConfig:
    ...
    eval_fuzzy_only_wildcard: bool
    eval_ignore_classes: frozenset[str]
    save_type_confusion_crops: bool
    standard_eval_subdir: str
    output_dir: str
    save_image: bool
    ...
    # 竞赛模式阶段一：只写 XML，跳过校验/画图/LS/LabelMe
    xml_only: bool = False
...
def _accumulate_other_filter_reason_stats(
    ...
    for r in filtered or []:
        ...
        if not _is_other_filter_reason(fr):
            continue
        if fr.startswith("other_cls_conf:") or fr.startswith("other_cls_ood:"):
            key = fr.split("(", 1)[0]
        else:
            key = fr
...
class _DemoPostShared:
    ...
    def _postprocess_locked(self, cfg: _DemoPostConfig, job: _DemoImageJob) -> None:
        ...
        if cfg.xml_only:
            ...
            if cfg.output_xml and img is not None:
            ...
            if job.skip_inference:
                line += f"跳过推理: {rel} -> {len(results)} 个目标"
            else:
                line += f"{rel} -> {len(results)} 个目标"
                line += f" 推理={infer_cost:.3f}s"
            line += " （竞赛模式：先写XML，暂缓画图）"
            if post_cost >= 0.001:
                line += f" 写XML={post_cost:.3f}s"
            if job.phase_line:
            ...
            if gts:
                ...
                if gt_source == "filename":
                    matches, matched_p, pred_rows = (
                        _accumulate_predict_all_filename_gt_validation(
                            ...
                            eval_ignore_classes=cfg.eval_ignore_classes,
                        )
                    )
                ...
                if self.std_cm is not None:
                    if gt_source == "filename":
                        _std_eval_collect_confusion_filename_gt(
                            ...
                            fuzzy_only_wildcard=cfg.eval_fuzzy_only_wildcard,
                            tier_equivalence=cfg.tier_equivalence,
                            ignore_classes=cfg.eval_ignore_classes,
                        )
                    else:
                        _std_eval_collect_confusion(
                            self.std_cm,
                ...
                if cfg.save_type_confusion_crops and gts:
                    _save_predict_all_type_confusion_crops(
                        ...
                        n_result_preds=len(results) if gt_source == "xml" else None,
                        eval_ignore_classes=cfg.eval_ignore_classes,
                    )
        ...
        if will_draw_image:
            ...
            draw_results(
                ...
                draw_label_clear=cfg.draw_label_clear,
                eval_ignore_classes=cfg.eval_ignore_classes,
...
def run_predict_all_demo(
    *,
    ...
    ROOT_IDS: Any,
    SAVE_IMAGE: Any,
    DRAW_PICTURE_AGAING: Any,
    DRAW_BBOX: Any,
    DRAW_POLYGON: Any,
    DRAW_VALIDATION_FOCUS: Any,
    DRAW_EVAL_FP_NON_FOCUS: Any,
    DRAW_LABEL: Any,
    DRAW_LABEL_CLEAR: Any,
    DRAW_LABEL_ERRORS_ONLY: Any,
    LABEL_FONT_SIZE: Any,
    SHOW_SOURCE_IN_LABEL: Any,
    SAVE_IMAGE_SCALE: Any,
    OUTPUT_XML: Any,
    OUTPUT_XML_CN_NAME: Any,
    OUTPUT_XML_DUAL_CONF: Any,
    OUTPUT_LABELME: Any,
    LABELME_COPY_IMAGE: Any,
    ENABLE_VALIDATION: Any,
    IGNORE_CLASSES: Any,
    MIN_INSECT_COUNT: Any,
    VAL_BOX_MATCH_METRIC: Any,
    VAL_GEOM_THRESHOLD: Any,
    ENABLE_STANDARD_EVAL: Any,
    STANDARD_EVAL_SUBDIR: Any,
    CLEAN_OUTPUT_BEFORE_RUN: Any,
    SKIP_IF_OUTPUT_EXISTS: Any,
    SAVE_TYPE_CONFUSION_CROPS: Any,
    SORT_STAT_BY_ACC: Any,
    CLASS_MERGE_TO_GROUPS: Any,
    EVAL_INSECT_WILDCARD: Any,
    EVAL_INSECT_WILDCARD_STRICT: Any,
    ENABLE_MASK_RATE_FILTER: Any,
    POSTPROCESS_PIPELINE: Any,
    POSTPROCESS_WORKERS: Any,
    ENABLE_LS_INGEST: Any,
    LS_INGEST_URL: Any,
    LS_INGEST_DRY_RUN: Any,
    LS_INGEST_SKIP_FILTERED: Any,
    LS_INGEST_SKIP_OTHER: Any,
    LS_INGEST_INCLUDE_FILTERED: Any,
    LS_INGEST_CROP_FROM_BBOX: Any,
    LS_INGEST_PAD_SQUARE: Any,
    LS_EXPORT_SEG_TRANSPARENT_PNG: Any,
    CLEAN_LS_EXPORT_BEFORE_RUN: Any,
    LS_INGEST_BATCH_SIZE: Any,
    LS_INGEST_JPEG_QUALITY: Any,
    LS_INGEST_CHOICE_FROM_NAME: Any,
    LS_INGEST_CHOICE_TO_NAME: Any,
    PHASE_PROFILE: Any,
) -> None:
    """批量 Demo：按 ``__main__`` 开关跑校验/画图/导出；无函数定义，仅编排调用。"""
    ...
    if not pipeline._enable_mask_rate_filter:  # noqa: SLF001
        logging.info("mask_rate 填充率过滤已关闭（mask_rate_switch=false）")
    if ENABLE_VALIDATION:
    ...
    eval_ignore_classes: frozenset[str] = frozenset()
    eval_min_insect_count = 0
    if ENABLE_VALIDATION and validation_focus.run_model == "shengchan":
        eval_ignore_classes = build_eval_focus_set(
            IGNORE_CLASSES,
            merge=class_merge_eval,
            label_alias_map=eval_label_alias_map,
        )
        eval_min_insect_count = max(0, int(MIN_INSECT_COUNT or 0))
    ...
    try:
        ...
        if ENABLE_VALIDATION:
            ...
            if validation_focus.run_model == "baipai":
                ...
                print(
                    ...
                    "同时统计不含鉴定0项与含鉴定0项（鉴定0识别≥1按0计）两种总分"
                )
                if IGNORE_CLASSES:
                    print(
                        f"  生产忽略类: {', '.join(str(x) for x in IGNORE_CLASSES)}"
                        "（不比对、不计入；标签仅类别名）"
                    )
                if eval_min_insect_count > 0:
                    print(
                        f"  生产平均分鉴定数下限: {eval_min_insect_count}"
                        "（低于此头数不进算术平均）"
                    )
            if validation_focus.report_classes:
                print(
        ...
        competition_mode = resolve_competition_mode_cfg(pipeline.config)
        competition_on = bool(competition_mode.enable)
        if competition_on:
            print(
                "竞赛模式: 已开启 "
                f"(single_class_count={competition_mode.single_class_count})；"
                "先写 XML，整批按 name 汇总低头数改 other，再重绘/校验"
            )
        ...
        post_cfg_phase1 = (
            dataclass_replace(post_cfg, xml_only=True) if competition_on else post_cfg
        )
        pending_jobs: list[_DemoImageJob] = []
        ...
        def _submit_post(job: _DemoImageJob, cfg: _DemoPostConfig) -> None:
            if competition_on:
                with pending_lock:
                    pending_jobs.append(job)
            if post_executor is not None:
                post_executor.submit(post_shared.postprocess, cfg, job)
            else:
                post_shared.postprocess(cfg, job)
        ...
        for idx, img_path in enumerate(image_files, 1):
            ...
            if skip_if_output_exists and predict_all_image_complete_for_skip(
                ...
                try:
                    ...
                    _submit_post(job, post_cfg_phase1 if competition_on else post_cfg)
                    continue
        ...
        if competition_on and pending_jobs and post_shared is not None:
            # 整批 XML 汇总（只认无 filter 的 <name>），再就地改 other，最后重载 job
            xml_paths = [
                Path(j.out_dir) / f"{Path(j.rel.name).stem}.xml" for j in pending_jobs
            ]
            suppress = collect_competition_suppress_names_from_xmls(
                xml_paths,
                single_class_count=competition_mode.single_class_count,
            )
            n_changed = 0
            for xp in xml_paths:
                n_changed += rewrite_competition_other_in_pred_xml(
                    xp, suppress_names=suppress
                )
            for job in pending_jobs:
                xp = Path(job.out_dir) / f"{Path(job.rel.name).stem}.xml"
                if xp.is_file():
                    try:
                        rows = parse_pascal_voc_pred_objects(xp)
                    except Exception:
                        logging.warning(
                            "竞赛重载 XML 失败，保留内存结果: %s", xp, exc_info=True
                        )
                    else:
                        results_r: list[dict[str, Any]] = []
                        filtered_r: list[dict[str, Any]] = []
                        for r in rows:
                            if bool(r.get("filtered")) or str(
                                r.get("filter_reason") or ""
                            ).strip():
                                filtered_r.append(r)
                            else:
                                results_r.append(r)
                        job.results = results_r
                        job.filtered = filtered_r
                job.img = None
            if suppress:
                sample = ", ".join(sorted(suppress)[:12])
                more = f" 等{len(suppress)}类" if len(suppress) > 12 else ""
                print(
                    f"竞赛模式: 整批 XML 按 name 汇总，单类头数<"
                    f"{competition_mode.single_class_count} "
                    f"改 other {len(suppress)} 类、{n_changed} 框"
                    f"（{sample}{more}）；重绘 / 校验"
                )
            else:
                print(
                    "竞赛模式: 整批 XML 无单类头数不足的报出类；重绘 / 校验"
                )
            post_cfg_phase2 = dataclass_replace(post_cfg, draw_picture_again=True)
            if POSTPROCESS_PIPELINE:
                post_executor = ThreadPoolExecutor(
                    max_workers=max(1, int(POSTPROCESS_WORKERS)),
                    thread_name_prefix="demo-post2",
                )
            for job in pending_jobs:
                img2 = cv2.imread(str(job.img_path))
                if img2 is None:
                    print(
                        f"[{job.idx}/{job.total}] 竞赛阶段二重读失败，跳过: {job.img_path}"
                    )
                    continue
                job.img = img2
                if post_executor is not None:
                    post_executor.submit(post_shared.postprocess, post_cfg_phase2, job)
                else:
                    post_shared.postprocess(post_cfg_phase2, job)
            if post_executor is not None:
                post_executor.shutdown(wait=True)
                post_executor = None
    ...
    if ENABLE_VALIDATION and (
        ...
        counting_summary = compute_competition_counting_summary(
            ...
            ignore_classes=eval_ignore_classes,
            min_insect_count=eval_min_insect_count,
        )
        ...
        if ENABLE_STANDARD_EVAL and std_cm is not None:
            ...
            if SAVE_TYPE_CONFUSION_CROPS:
                print(
                    f"类型匹配切图: {eval_root}/all/type_confusion_crops/ "
                    f"与 .../type_confusion_crops_by_pred/（含 tp、cls_err、漏检 pred=__miss__、"
                    f"多余检出 gt=__extra_pred__）"
                )
...
if __name__ == "__main__":
    # mac上运行代码
    # /Users/shunyaoyin/miniconda310/miniconda3/envs/yolo11/bin/python3 /Users/shunyaoyin/Documents/code/ai-company/insect/script/predict_all.py
    # unbuntu上运行代码
    # nohup /home/shunyao/miniconda310/envs/yolo11/bin/python3 /data/script/predict_all.py > predict.log &
    print("predict_all.py start")
    # 某些环境（如被其他模块/运行器提前配置过 logging）会导致 basicConfig 不生效；
    # 这里使用 force=True 确保脚本直跑时能看到 INFO 日志。
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(levelname)s %(message)s",
        force=True,
    )
    logging.info("predict_all.py start")

    CONFIG_PATH = INSECT_ALG_ALL_JSON_REL
    # 如果是macos
    if platform.system() == "Darwin":
        # INPUT_PATH = "/Volumes/shunyao-h1/训练数据/测试集/大虫检出测试集"
        # INPUT_PATH = "/Volumes/shunyao-h1/训练数据/测试集/大虫测试数据集"
        # INPUT_PATH = "/Volumes/shunyao-h1/训练数据/训练集/bj-0509-0511"
        # INPUT_PATH = "/Volumes/shunyao-h1/训练数据/测试集/大虫测试集泛化"
        # INPUT_PATH = "/Volumes/shunyao-h1/训练数据/测试集/hebing_ceshi1"
        INPUT_PATH = "/Volumes/shunyao-h1/训练数据/测试集/北京设备全标注"
        INPUT_PATH = "/Volumes/shunyao-h1/训练数据/测试集/问题测试集"
        INPUT_PATH = "/Volumes/shunyao-h1/训练数据/测试集/稻飞虱漏"
        # INPUT_PATH = "/Volumes/shunyao-h1/训练数据/测试集/6-18yindu拍摄稻飞虱测试"
        # INPUT_PATH = "/Volumes/shunyao-h1/训练数据/测试集/临时测试"
        # INPUT_PATH = "/Volumes/shunyao-h1/训练数据/北京比赛/照片圆盘"
        INPUT_PATH = "/Volumes/shunyao-h1/测试数据/北京比赛/模拟摆拍试卷"
        # INPUT_PATH = "/Volumes/shunyao-h1/测试数据/北京比赛/北京设备全标注早期"
        # INPUT_PATH = "/Volumes/shunyao-h1/比赛数据/2026辛集正式比赛/标本"
        INPUT_PATH = "/Volumes/shunyao-h1/比赛数据/2026辛集正式比赛/田间比赛对比"
        # INPUT_PATH = "/Volumes/shunyao-h1/测试数据/新镜头摆拍/7月21日新增摆拍标本/新镜头"
        # INPUT_PATH = "/Volumes/shunyao-h1/测试数据/云南摆拍-分类训练"
        # INPUT_PATH = "/Volumes/shunyao-h1/测试数据/问题测试/重复多个框检测"
        # INPUT_PATH = "/Volumes/shunyao-h1/测试数据/问题测试/二化螟漏"
        # INPUT_PATH = "/Users/shunyaoyin/Downloads/稻飞虱漏.jpg"

    else:
        # INPUT_PATH = "/data/model/dataset/9月新/辛集市田间比赛"
        INPUT_PATH = "/data/samples/田间比赛对比"
    # 摆拍
    # OUTPUT_DIR = INPUT_PATH + "-3.13.4-3.11.7"
    # OUTPUT_DIR = INPUT_PATH + "-3.13.13-3.8.11"
    # OUTPUT_DIR = INPUT_PATH + "-1.5.4-3.11.8"
    OUTPUT_DIR = INPUT_PATH + "-2.0.5-3.12.2-match"
    # OUTPUT_DIR = INPUT_PATH + "-2.0.5-3.12.2-close"

    
    ROOT_IDS: list[str] | None = None

    # 关闭时不绘制也不保存可视化图片，仅返回/打印检测结果（XML 另由 OUTPUT_XML 控制）
    SAVE_IMAGE = True
    # True：增量续跑时预测 xml 已存在仍重绘结果图（ENABLE_VALIDATION 时对照 GT xml 标注校验框）
    # False：仅当结果图文件不存在时才绘制（续跑且图已存在则跳过重绘，仍重算校验统计）
    DRAW_PICTURE_AGAING = False
    DRAW_BBOX = True
    DRAW_POLYGON = True
    # 未在 alg 配置 draw_filter 的根模型不绘 filtered；
    # True 时即使 draw_filter=false，仍绘制 TOP1+TOP2 关注类的 filtered（深灰框 +「过滤:原因」标签）
    DRAW_VALIDATION_FOCUS = True
    # 评估可视化：False=仅画 TOP1+TOP2 关注类相关校验框；True=非关注类 FP/FN/类型错等也画（灰框弱化）
    DRAW_EVAL_FP_NON_FOCUS = True
    # False 时不绘制框旁文字标签，仍保留 bbox / polygon 轮廓
    DRAW_LABEL = True
    # True 时清爽模式：文字标签绘「类名+检出/分类置信度」，如 隐翅虫0.45/0.98
    DRAW_LABEL_CLEAR = True
    # False 时校验 TP 标签仅绘中文类名；cls_err/fp/fn 等仍保留完整评估标签
    # True 时校验 TP 仍画框但不画标签，仅 cls_err/fp/fn 等保留文字标签（便于定位问题框）
    DRAW_LABEL_ERRORS_ONLY = False
    # 标签字号：None 时按图幅自适应（约 20–56px）；中文依赖系统字体（PingFang 等）
    LABEL_FONT_SIZE: int | None = None
    SHOW_SOURCE_IN_LABEL = True
    # 可视化结果图保存缩放：None 不缩放；0.5/0.25/0.125 分别缩至 1/2、1/4、1/8（仅 SAVE_IMAGE 落盘前）
    SAVE_IMAGE_SCALE: float | None = 1

    # 关闭时行为与改前一致；打开时在输出目录写入 Pascal VOC bbox xml（仅 bbox，不含 polygon）
    # 一并写入 other/尺寸对角线等「可评估几何匹配」的 filtered，并带 filter_reason
    # 增量续跑（SKIP_IF_OUTPUT_EXISTS）要求必须为 True
    OUTPUT_XML = True
    # 关闭时 xml 的 <name> 使用 infer_name（与外部测试集/拼音一致）；打开时优先 insect_alg_all 的 cn_name 索引
    OUTPUT_XML_CN_NAME = True
    # 关闭时 xml 不含置信度字段（与改前一致）；打开时每个 object 写入 det_conf / cls_conf
    OUTPUT_XML_DUAL_CONF = True
    # 打开时在输出目录写入 LabelMe JSON（polygon；无 mask 时退化为 bbox 四角），可直接 labelme <OUTPUT_DIR> 校对
    OUTPUT_LABELME = False
    # LabelMe 导出时是否将原图复制到与 json 同目录（推荐 True，便于 LabelMe 批量打开）
    LABELME_COPY_IMAGE = False


    # 关闭时不读标注、不统计；打开时对照同目录 .xml 做几何匹配与按类汇总；
    # 无 xml 时若文件名含已知中文名或拼音（如 灰飞虱.jpg），则假定图内报出框均应判为该种类并按框统计
    ENABLE_VALIDATION = True
    # 仅生产模式生效：清单内标注与推理不比对、不计入统计/总分；可视化标签只绘类别名（中文名即可）
    IGNORE_CLASSES = ["划蝽", "德国蜉金龟"]
    # 仅生产模式生效：鉴定头数低于此值的类不参与算术平均；<=0 关闭
    MIN_INSECT_COUNT = 3
    # 校验子配置（仅 ENABLE_VALIDATION=True 时生效）
    VAL_BOX_MATCH_METRIC = "iou"  # "iou" | "ior"
    VAL_GEOM_THRESHOLD = 0.25
    ENABLE_STANDARD_EVAL = True
    STANDARD_EVAL_SUBDIR = "eval_metrics"
    # 重新执行前清空 OUTPUT_DIR（含 eval_metrics 切图/CSV），避免与上次评估产物混合
    CLEAN_OUTPUT_BEFORE_RUN = False
    # 增量续跑：跳过已完整处理的图（须 CLEAN_OUTPUT_BEFORE_RUN=False 且 OUTPUT_XML=True）
    SKIP_IF_OUTPUT_EXISTS = True
    SAVE_TYPE_CONFUSION_CROPS = True
    SORT_STAT_BY_ACC = True
    CLASS_MERGE_TO_GROUPS: dict[str, list[str]] | None = None
    EVAL_INSECT_WILDCARD = True
    # True：insect/other 等粗类仅彼此通配；具体物种（如小菜蛾）不与 insect/other 互匹为 TP
    # False：保持旧口径，insect:['*'] 可与任意具体类互匹
    EVAL_INSECT_WILDCARD_STRICT = True

    # None：跟随 predict_cfg.mask_rate_switch；True/False 覆盖 JSON
    ENABLE_MASK_RATE_FILTER = None

    # True：推理（GPU）与校验/画图/导出（CPU）流水线并行；单卡推荐 workers=1
    POSTPROCESS_PIPELINE = True
    POSTPROCESS_WORKERS = 1

    # ===== 调测开关：将分类框图上报 LS 标注系统（默认关闭；关闭时行为与改前一致）=====
    # 打开后复用 script/ls_seg_classification_ingest.py 的裁剪与上报实现；不影响推理参数与算法配置。
    ENABLE_LS_INGEST = False
    # LS_INGEST_URL 取值二选一：
    #   - http(s):// 开头 → 标注系统完整 ingest 地址（含路径中的 ingest_token），上报行为不变；
    #   - 其它（如本地目录路径）→ 把分类框图按类别分文件夹落盘到该目录（不上报、不联网）。
    # 为空时回退环境变量 LS_INGEST_URL（仍按 URL 模式）。
    # LS_INGEST_URL = "http://8.137.33.38:48080/api/data-sources/ingest/ingest_1780037493645_83085d2c384e4/"
    # 保存本地
    LS_INGEST_URL = INPUT_PATH + "-cls"
    # True 仅组装任务不真正 POST（调试用）
    LS_INGEST_DRY_RUN = False
    # 跳过 filter=True 实例 / 跳过 cls_name=other 实例
    LS_INGEST_SKIP_FILTERED = False
    LS_INGEST_SKIP_OTHER = False
    # True 时把被过滤实例也纳入上报（自动置 skip_filtered=False，依赖 collect_filtered 收集到的 filtered）
    LS_INGEST_INCLUDE_FILTERED = True
    # 上报裁剪：True=外接框矩形；False=分割多边形掩码（仅分割来源有效）
    LS_INGEST_CROP_FROM_BBOX = False
    # 裁剪是否补成正方形（与分类推理 cls_pad_square 一致）：
    #   - LS 上报 / 白底 JPG：白边补方；
    #   - 本地透明 PNG：透明补方。
    # True 时导出的分割图与「检测/分类推理时的裁剪」几何基本一致（多边形外扩 + 正方形）。
    LS_INGEST_PAD_SQUARE = True
    # 仅本地目录导出生效：分割图（CROP_FROM_BBOX=False）输出带 alpha 透明通道的 PNG（多边形外透明）；
    # False 则回退为白底 JPG。bbox 裁剪不受影响，始终 JPG。
    LS_EXPORT_SEG_TRANSPARENT_PNG = True
    # 本地分类框图导出目录；重新执行前清空（仅本地目录模式）
    CLEAN_LS_EXPORT_BEFORE_RUN = True
    ...
    run_predict_all_demo(
        CONFIG_PATH=CONFIG_PATH,
        INPUT_PATH=INPUT_PATH,
        OUTPUT_DIR=OUTPUT_DIR,
        ROOT_IDS=ROOT_IDS,
        SAVE_IMAGE=SAVE_IMAGE,
        DRAW_PICTURE_AGAING=DRAW_PICTURE_AGAING,
        DRAW_BBOX=DRAW_BBOX,
        DRAW_POLYGON=DRAW_POLYGON,
        DRAW_VALIDATION_FOCUS=DRAW_VALIDATION_FOCUS,
        DRAW_EVAL_FP_NON_FOCUS=DRAW_EVAL_FP_NON_FOCUS,
        ...
        VAL_BOX_MATCH_METRIC=VAL_BOX_MATCH_METRIC,
        VAL_GEOM_THRESHOLD=VAL_GEOM_THRESHOLD,
        ENABLE_STANDARD_EVAL=ENABLE_STANDARD_EVAL,
        STANDARD_EVAL_SUBDIR=STANDARD_EVAL_SUBDIR,
        CLEAN_OUTPUT_BEFORE_RUN=CLEAN_OUTPUT_BEFORE_RUN,
        SKIP_IF_OUTPUT_EXISTS=SKIP_IF_OUTPUT_EXISTS,
        SAVE_TYPE_CONFUSION_CROPS=SAVE_TYPE_CONFUSION_CROPS,
        SORT_STAT_BY_ACC=SORT_STAT_BY_ACC,
        CLASS_MERGE_TO_GROUPS=CLASS_MERGE_TO_GROUPS,
        EVAL_INSECT_WILDCARD=EVAL_INSECT_WILDCARD,
        EVAL_INSECT_WILDCARD_STRICT=EVAL_INSECT_WILDCARD_STRICT,
        ENABLE_MASK_RATE_FILTER=ENABLE_MASK_RATE_FILTER,
        ...
        LS_INGEST_PAD_SQUARE=LS_INGEST_PAD_SQUARE,
        LS_EXPORT_SEG_TRANSPARENT_PNG=LS_EXPORT_SEG_TRANSPARENT_PNG,
        CLEAN_LS_EXPORT_BEFORE_RUN=CLEAN_LS_EXPORT_BEFORE_RUN,
        LS_INGEST_BATCH_SIZE=LS_INGEST_BATCH_SIZE,
        LS_INGEST_JPEG_QUALITY=LS_INGEST_JPEG_QUALITY,
        LS_INGEST_CHOICE_FROM_NAME=LS_INGEST_CHOICE_FROM_NAME,
        LS_INGEST_CHOICE_TO_NAME=LS_INGEST_CHOICE_TO_NAME,
