...
from script.predict_worker_pool import (
    InferenceProcessPool,
    configure_stdio_logging,
    isolate_current_process_from_cuda,
)

# 延迟绑定：``predict_all`` 会 import torch/torchvision。run_count>1 时须先隐藏主进程 GPU。
InsectPredictAll = None
draw_results = None
load_image_bgr_from_ref = None
load_insect_alg_all = None
collect_images = None
resolve_competition_mode_cfg = None
collect_competition_suppress_names_from_xmls = None
rewrite_competition_other_in_pred_xml = None
save_pascal_voc_xml_for_results = None
parse_pascal_voc_pred_objects = None


def _bind_predict_all() -> None:
    """延迟导入 ``predict_all``（会拉取 torch）。须在主进程隐藏 GPU 之后调用。"""
    global InsectPredictAll, draw_results, load_image_bgr_from_ref, load_insect_alg_all
    global collect_images, resolve_competition_mode_cfg
    global collect_competition_suppress_names_from_xmls, rewrite_competition_other_in_pred_xml
    global save_pascal_voc_xml_for_results, parse_pascal_voc_pred_objects
    if InsectPredictAll is not None:
        return
    from script.predict_all import (
        InsectPredictAll as _ipa,
        collect_images as _ci,
        collect_competition_suppress_names_from_xmls as _ccs,
        draw_results as _dr,
        load_image_bgr_from_ref as _lir,
        load_insect_alg_all as _lia,
        resolve_competition_mode_cfg as _rcm,
        rewrite_competition_other_in_pred_xml as _rco,
        save_pascal_voc_xml_for_results as _spx,
    )
    from script.predict_size_validate_lib import (
        parse_pascal_voc_pred_objects as _ppv,
    )

    InsectPredictAll = _ipa
    draw_results = _dr
    load_image_bgr_from_ref = _lir
    load_insect_alg_all = _lia
    collect_images = _ci
    resolve_competition_mode_cfg = _rcm
    collect_competition_suppress_names_from_xmls = _ccs
    rewrite_competition_other_in_pred_xml = _rco
    save_pascal_voc_xml_for_results = _spx
    parse_pascal_voc_pred_objects = _ppv
...
def _html_init_kwargs(**kwargs: Any) -> dict[str, Any]:
    """只保留当前 Gradio ``gr.HTML`` 支持的参数，避免旧版 ``TypeError``。"""
    try:
...
def _service_gate_html() -> str:
    """服务就绪遮罩 DOM（不含 ``<script>``；Gradio 会剥离 HTML value 内脚本）。"""
    return """
...
# 经 Blocks(head=...) 注入；勿写入 gr.HTML value（会被 sanitizer 去掉）。
_SERVICE_GATE_JS = f"""
(function() {{
  if (window.__insectGateInstalled) return;
  window.__insectGateInstalled = true;
  ...
  window.insectWaitForGate = function(fn, tries) {{
    tries = tries === undefined ? 80 : tries;
    const gate = gateEl();
    if (gate) {{
      fn(gate);
      return;
    ...
    if (tries <= 0) return;
    setTimeout(function() {{ window.insectWaitForGate(fn, tries - 1); }}, 50);
  }};
  ...
  window.insectShowGate = function(opts) {{
    opts = opts || {{}};
    window.insectWaitForGate(function(gate) {{
  ...
  window.insectCheckOnLoad = async function() {{
    window.insectSetPageInteractive(false);
    window.insectWaitForGate(async function() {{
      try {{
        const data = await window.insectFetchReady();
        if (data) {{
          window.insectHideGate();
...
_SERVICE_GATE_HEAD = f"<script>{_SERVICE_GATE_JS}</script>"


_PAGE_LOAD_GATE_JS = f"""
() => {{
  {_SERVICE_GATE_JS}
...
def _log_predict_perf_config(cfg: dict[str, Any]) -> None:
    ...
    logger.info(
        ...
        "clip_profiles_enable=%s trt_switch=%s mask_rate_switch=%s",
...
def allocate_batch_result_dir(input_dir: Path) -> Path:
    """在输入目录同级分配 ``{名}_结果1``、``_结果2``…（已存在则递增）。"""
    parent = input_dir.parent
    stem = input_dir.name
    n = 1
    while True:
        cand = parent / f"{stem}_结果{n}"
        if not cand.exists():
            return cand
        n += 1


def pick_local_directory(title: str = "选择图片目录") -> str:
    """
    弹出系统目录对话框，返回路径；取消/不可用时返回空串。

    **禁止**在 Gradio 回调线程里用 tkinter：macOS 要求 NSWindow 在主线程，
    否则进程直接 abort（NSInternalInconsistencyException）。改为子进程：
    Darwin ``osascript`` / Linux ``zenity`` 或 ``kdialog``。
    """
    import shutil
    import subprocess

    title_safe = str(title or "选择目录").replace('"', "").replace("\\", "")
    try:
        if sys.platform == "darwin":
            script = (
                f'set p to choose folder with prompt "{title_safe}"\n'
                "POSIX path of p"
            )
            proc = subprocess.run(
                ["osascript", "-e", script],
            ...
            if proc.returncode != 0:
                return ""
            return str(proc.stdout or "").strip().rstrip("/")
        ...
        if sys.platform.startswith("linux"):
            ...
            if shutil.which("kdialog"):
                proc = subprocess.run(
                    ["kdialog", "--getexistingdirectory", ".", "--title", title_safe],
                    capture_output=True,
                    text=True,
                    timeout=300,
                    check=False,
                ...
                if proc.returncode != 0:
                    return ""
                return str(proc.stdout or "").strip()
            logger.info("目录对话框不可用：未找到 zenity/kdialog，请粘贴路径")
            return ""

        logger.info("目录对话框未支持当前平台 %s，请粘贴路径", sys.platform)
        return ""
    except Exception as exc:  # pragma: no cover
        logger.info("目录对话框失败（请粘贴路径）: %s", exc)
        return ""


def _split_pred_xml_rows(
    rows: list[dict[str, Any]],
) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    """预测 XML 行拆成保留 / 过滤两类（与 predict_all 竞赛重载口径一致）。"""
    results: list[dict[str, Any]] = []
    filtered: list[dict[str, Any]] = []
    for r in rows:
        if bool(r.get("filtered")) or str(r.get("filter_reason") or "").strip():
            filtered.append(r)
        else:
            results.append(r)
...
class GradioPredictAllApp:
    def __init__(
        ...
        *,
        ...
        self._parent_cuda_visible: str | None = None
        self._parent_cuda_isolated = False
        if peek_run_count(self.config_path) > 1:
            self._parent_cuda_visible = isolate_current_process_from_cuda()
            self._parent_cuda_isolated = True
            logger.info(
                "[启动] run_count>1：主进程隐藏 GPU（CUDA_VISIBLE_DEVICES=''），"
                "避免与 worker 各占一份显存；worker 将恢复可见设备=%s",
                self._parent_cuda_visible
                if self._parent_cuda_visible is not None
                else "(未设置)",
            )
        _bind_predict_all()
        ...
        if self._run_count > 1 and not self._parent_cuda_isolated:
            logger.warning(
                "[启动] run_count=%d 但启动前未隔离主进程 GPU（peek 与加载结果不一致）",
                self._run_count,
            )
        if self._parent_cuda_isolated and self._run_count <= 1:
            logger.warning(
                "[启动] 已隔离主进程 GPU 但 run_count=%d，单进程推理将看不到 CUDA",
                self._run_count,
            )
        ...
        if self._run_count > 1:
            ...
            self._process_pool = InferenceProcessPool(
                ...
                cuda_visible_devices=self._parent_cuda_visible,
                restore_cuda_visible=self._parent_cuda_isolated,
            )
        else:
            logger.info(
        ...
        if self._run_count > 1:
            logger.info(
                "[推理设备] run_count=%d：主进程不加载模型、不初始化 CUDA，"
                "推理由 worker 执行（device=%s）",
                self._run_count,
    ...
    def shutdown(self) -> None:
        ...
        if not self._parent_cuda_isolated:
            try:
                import torch
    ...
    # ----- 本地目录批量检测（compitition 仅此处生效） -----

    def pick_input_dir(self, current: str = "") -> str:
        """「选择目录」按钮：填入服务端本地路径；取消则保留原值。"""
        path = pick_local_directory("选择待检测图片目录")
        if path:
            logger.info("[批量] 已选择输入目录: %s", path)
            return path
        return str(current or "")

    def pick_output_dir(self, current: str = "") -> str:
        path = pick_local_directory("选择结果输出目录")
        if path:
            logger.info("[批量] 已选择输出目录: %s", path)
            return path
        return str(current or "")
    ...
    def run_batch_dir(
        ...
        *spec_values: Any,
        ...
        """
        目录批量检测：写结果图 + 预测 XML 到目标目录。

        ``postprocess.compitition`` 仅在此路径生效；单图 ``run`` / API 不走竞赛改写。
        """
        in_raw = str(input_dir or "").strip()
        if not in_raw:
            yield "请先填写或选择输入目录（服务端本地路径）。"
            return
        in_p = Path(in_raw).expanduser()
        if not in_p.is_dir():
            yield f"输入路径不是目录或不存在：`{in_p}`"
            return
        out_raw = str(output_dir or "").strip()
        if out_raw:
            out_p = Path(out_raw).expanduser()
        else:
            out_p = allocate_batch_result_dir(in_p)
        try:
            out_p.mkdir(parents=True, exist_ok=True)
        except OSError as e:
            yield f"无法创建输出目录 `{out_p}`：{e}"
            return

        try:
            cfg = self.build_effective_config(list(spec_values))
        except Exception as e:  # pragma: no cover
            logger.exception("[批量] 构建生效配置失败")
            yield f"配置解析失败：{e}"
            return
        ...
        if not enabled_roots:
            yield "没有启用任何根模型，请在下方至少启用一个。"
            return

        use_pool = self._process_pool is not None
        predict_cfg = self.base_config if use_pool else cfg
        if use_pool and not self._config_matches_base(cfg):
            logger.warning(
                "[批量] 页面算法参数与默认配置不完全一致，"
                "推理仍使用 worker 默认配置"
            )

        competition_mode = resolve_competition_mode_cfg(predict_cfg)
        competition_on = bool(competition_mode.enable)
        input_root, image_files = collect_images(str(in_p))
        if not image_files:
            yield f"目录下未找到图片：`{in_p}`"
            return
        ...
        if not use_pool:
            try:
                pipeline = self.get_pipeline(predict_cfg, device)
                cn_index = dict(getattr(pipeline, "_cn_display_index", {}) or {})
            except Exception as e:
                logger.exception("[批量] 获取管线失败")
                yield f"管线初始化失败：{e}"
                return

        comp_line = (
            f"竞赛模式 **开**（single_class_count={competition_mode.single_class_count}）"
            if competition_on
            else "竞赛模式 **关**（单图 API 始终不走竞赛）"
        )
        yield (
            f"开始批量：输入 `{in_p}` → 输出 `{out_p}` ｜ "
            f"共 **{len(image_files)}** 张 ｜ {comp_line}"
        )
        logger.info(
            "=== [批量] 开始 === in=%s out=%s n=%d competition=%s thr=%s",
            in_p,
            out_p,
            len(image_files),
            competition_on,
            competition_mode.single_class_count,
        )
        ...
        for idx, img_path in enumerate(image_files, 1):
            ...
            try:
                ...
                err_n += 1
                logger.exception("[批量] 推理失败 %s", img_path)
                yield (
                    f"[{idx}/{total}] 推理失败 `{rel}`：{e} ｜ "
                    f"成功 {ok_n} / 跳过 {skip_n} / 失败 {err_n}"
                )
                continue
            ...
            try:
                save_pascal_voc_xml_for_results(
                    ...
                    rel_path=rel,
                    cn_index=cn_index,
                    include_dual_conf=True,
                    filtered=filtered,
                )
            except Exception as e:
                err_n += 1
                logger.exception("[批量] 写 XML 失败 %s", img_path)
                yield (
                    f"[{idx}/{total}] 写 XML 失败 `{rel}`：{e} ｜ "
                    f"成功 {ok_n} / 跳过 {skip_n} / 失败 {err_n}"
                )
                continue
            ...
            if competition_on:
                ...
                try:
                    ...
                    err_n += 1
                    logger.exception("[批量] 绘制失败 %s", img_path)
                    yield (
                        f"[{idx}/{total}] 绘制失败 `{rel}`：{e} ｜ "
                        f"成功 {ok_n} / 跳过 {skip_n} / 失败 {err_n}"
                    )
                    continue
            ok_n += 1
            if idx == 1 or idx == total or idx % 5 == 0:
                phase = "写XML" if competition_on else "推理+绘制"
                yield (
                    f"[{idx}/{total}] {phase} `{rel}` "
                    f"（保留 {len(results)} / 过滤 {len(filtered)}）｜ "
                    f"成功 {ok_n} / 跳过 {skip_n} / 失败 {err_n}"
                )

        n_changed = 0
        suppress: frozenset[str] = frozenset()
        if competition_on and pending:
            yield (
                f"竞赛阶段：汇总 {len(pending)} 张 XML，"
                f"单类头数 < {competition_mode.single_class_count} 改 other …"
            )
            xml_paths = [
                Path(j["out_dir"]) / f"{Path(j['rel'].name).stem}.xml" for j in pending
            ]
            ...
            for job in pending:
                ...
                if xp.is_file():
                    try:
                        ...
                        job["results"], job["filtered"] = _split_pred_xml_rows(rows)
                    except Exception:
                        logger.warning(
                ...
                try:
                    ...
                    err_n += 1
                    logger.exception("[批量] 竞赛重绘失败 %s", job["img_path"])
                    yield f"竞赛重绘失败 `{job['rel']}`：{e}"

        elapsed = time.perf_counter() - t_all
        suppress_info = ""
        if competition_on:
            if suppress:
                sample = ", ".join(sorted(suppress)[:8])
                more = f" 等{len(suppress)}类" if len(suppress) > 8 else ""
                suppress_info = (
                    f"\n- 竞赛改 other：**{len(suppress)}** 类、**{n_changed}** 框"
                    f"（{sample}{more}）"
                )
            else:
                suppress_info = "\n- 竞赛：无单类头数不足的报出类"
        ...
        summary = (
            f"**批量完成**（耗时 {elapsed:.1f}s）\n"
            f"- 输入：`{in_p}`\n"
            f"- 输出：`{out_p}`\n"
            f"- 成功 **{ok_n}** / 跳过 **{skip_n}** / 失败 **{err_n}** / 共 {total}\n"
            f"- {comp_line}"
            f"{suppress_info}"
        )
        logger.info(
            "=== [批量] 结束 === out=%s ok=%d skip=%d err=%d elapsed=%.1fs",
            out_p,
            ok_n,
            skip_n,
            err_n,
    ...
    def _draw_batch_image(
        ...
        *,
        ...
        draw_results(
            ...
            label_font_size=font_px,
            show_source_in_label=show_source,
            filtered_results=job["filtered"],
    ...
    def predict_api(self, image: np.ndarray | str | None) -> dict[str, Any]:
        ...
        注意：``postprocess.compitition`` **不**对本接口生效（无整批口径）。
        """
        use_pool = self._process_pool is not None
        if image is None:
        ...
        # 单图 API 明确跳过竞赛整批改写（即使配置 enable=true）
        _comp = resolve_competition_mode_cfg(self.base_config)
        if _comp.enable:
            logger.debug(
                "[API] /predict 忽略 postprocess.compitition（仅目录批量生效）"
            )
        if use_pool:
            if isinstance(image, np.ndarray):
                return {
    ...
    def predict_insect_api(self, url: str) -> dict[str, Any]:
        ...
        if _comp.enable:
            logger.debug(
                "[API] /insect_3_predict 忽略 postprocess.compitition（仅目录批量生效）"
            )
        try:
            t0 = time.perf_counter()
            results = self._dispatch_predict(
    ...
    def build_ui(self) -> gr.Blocks:
        ...
        with gr.Blocks(
            ...
            head=_SERVICE_GATE_HEAD,
        ) as demo:
            gr.HTML(**_html_init_kwargs(value=_service_gate_html(), container=False))
            ...
            with gr.Column(elem_id="insect-main-panel"):
                ...
                with gr.Accordion("本地目录批量检测", open=False):
                    _comp_cfg = resolve_competition_mode_cfg(self.base_config)
                    gr.Markdown(
                        "对**服务端本地目录**整批推理并绘制到目标目录；"
                        "输出留空则自动生成 `{输入目录名}_结果1`、`_结果2`…。"
                        f"当前配置竞赛模式："
                        f"**{'开' if _comp_cfg.enable else '关'}**"
                        f"（`single_class_count={_comp_cfg.single_class_count}`）；"
                        "仅本批量生效，**不影响**下方单图「运行识别」与 API。"
                    )
                    with gr.Row():
                        batch_input_in = gr.Textbox(
                            label="输入目录（服务端路径）",
                            placeholder="/data/samples/某卷",
                            scale=4,
                        )
                        batch_pick_in_btn = gr.Button("选择目录", scale=0)
                    with gr.Row():
                        batch_output_in = gr.Textbox(
                            label="输出目录（可选，留空自动 _结果N）",
                            placeholder="留空 → 输入名_结果1 / _结果2 …",
                            scale=4,
                        )
                        batch_pick_out_btn = gr.Button("选择输出", scale=0)
                    batch_run_btn = gr.Button(
                        "开始批量检测",
                        variant="primary",
                        elem_id="insect-batch-run-btn",
                    )
                    batch_summary_out = gr.Markdown("")
                    batch_pick_in_btn.click(
                        fn=self.pick_input_dir,
                        inputs=[batch_input_in],
                        outputs=[batch_input_in],
                    )
                    batch_pick_out_btn.click(
                        fn=self.pick_output_dir,
                        inputs=[batch_output_in],
                        outputs=[batch_output_in],
                    )
...
if __name__ == "__main__":
    ...
    # mac本地启动
    # /Users/shunyaoyin/miniconda310/miniconda3/envs/yolo11/bin/python3 /Users/shunyaoyin/Documents/code/ai-company/insect/script/predict_all_gradio.py
    ...
    INFER_DEVICE: str | None = None # "cuda:0"
