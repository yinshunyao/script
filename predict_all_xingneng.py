...
def _collect_enabled_out_leaf_class_keys(node: Any) -> set[str]:
    ...
    跳过 ``enable=false`` 的路由项；嵌套 ``models.cls.enable=false`` 时其 ``cls.out``
    叶类不视为启用（与 ``_collect_disabled_report_classes_from_out_node`` 对称）。
    """
    keys: set[str] = set()
    if not isinstance(node, dict):
        return keys
...
def _collect_disabled_report_classes_from_out_node(node: Any) -> set[str]:
    ...
    for cls_key, entry in out_table.items():
        ...
        if isinstance(cls_cfg, dict):
            if not _is_enabled(cls_cfg):
                keys.update(_collect_out_leaf_class_keys(cls_cfg))
            else:
                cls_out = cls_cfg.get("out")
                if isinstance(cls_out, dict) and cls_out:
                    for leaf_key, leaf_entry in cls_out.items():
...
def resolve_disabled_report_class_names(
    ...
    中 ``enable=false`` 的叶节点（含 ``infer_name``）。再经 ``class_tier_aliases`` 扩展。
...
def resolve_report_allowed_class_names(
    ...
    含路由 key 与 ``infer_name``。自动排除 ``out`` 树中 ``enable=false`` 的类
...
def _merge_root_cfg(
    ...
    ``postprocess`` 中的 ``background_classes``、``class_tier_aliases``（类型等价、可传递）为全局报出/验证配置，不并入单根推理配置。
