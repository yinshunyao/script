...
def _alg_sub_from_out_entry(entry: dict[str, Any]) -> dict[str, Any]:
    ...
    if "size_class" in entry and entry["size_class"] is not None:
        raw_size = str(entry.get("size_class") or "").strip().lower()
        if raw_size in ("small", "large"):
            sub["size_class"] = raw_size
    if "dia" in entry and entry["dia"] is not None:
        sub["dia"] = entry["dia"]
    for ood_key in ("p_dump", "p_near"):
        if ood_key in entry and entry[ood_key] is not None:
            try:
                sub[ood_key] = float(entry[ood_key])
            except (TypeError, ValueError):
                continue
    if "min_near" in entry and entry["min_near"] is not None:
        try:
            n = int(entry["min_near"])
        except (TypeError, ValueError):
            n = 0
        if n > 0:
            sub["min_near"] = n
    if "near_cls" in entry:
        raw_near = entry.get("near_cls")
        if raw_near is None:
            sub["near_cls"] = []
        elif isinstance(raw_near, str):
            name = raw_near.strip()
            sub["near_cls"] = [name] if name else []
        elif isinstance(raw_near, (list, tuple)):
            sub["near_cls"] = [
                str(x).strip() for x in raw_near if str(x or "").strip()
            ]
        else:
            sub["near_cls"] = []
    return sub
...
def xyxy_from_seq(seq: Any) -> tuple[float, float, float, float] | None:
    """把长度为 4 的序列（list/tuple/ndarray）解析成浮点 ``(x1,y1,x2,y2)``。"""
...
def row_location_to_bbox(row: dict[str, Any]) -> tuple[int, int, int, int] | None:
    ...
    xy = xyxy_from_seq(row.get("location"))
    if xy is None and all(k in row for k in ("x1", "y1", "x2", "y2")):
...
def normalize_in_big_by(value: Any) -> str:
    """
    ``in_big_by``：``mask``（默认，像素前景 mask）或 ``bbox``（小虫检测框矩形，不提取前景）。
    """
    v = str(value or "mask").strip().lower()
    if v in ("bbox", "box"):
        return "bbox"
    return "mask"


def _bbox_in_big_ratio_parts(
    bbox: tuple[int, int, int, int],
    ...
    """
    bbox 模式：小虫检测框整体作为矩形，不读图像、不提取前景 mask。

    占比 = 大虫 polygon 与小虫 bbox 相交像素数 / 小虫 bbox 面积；
    相交为 0 时返回 ``(None, ...)``（不判误报）。
    """
    if len(polygon) < 3:
    ...
    black_local = np.ones((bh, bw), dtype=bool)
    intersect_bool = big_mask_bool
    union_bool = black_local | big_mask_bool
    intersect_pixels = int(np.count_nonzero(intersect_bool))
    if intersect_pixels <= 0:
        return None, None, black_local, big_mask_bool, union_bool
    crop = np.full((bh, bw, 3), 255, dtype=np.uint8)
    return (
        intersect_pixels / small_area,
        crop,
        black_local,
        big_mask_bool,
        union_bool,
...
def compute_bbox_in_big_ratio(
    bbox: tuple[int, int, int, int],
    polygon: list[list[int]],
) -> float | None:
    """小虫 bbox 与大虫 polygon 相交面积占小虫 bbox 面积的比例（``in_big_by=bbox``）。"""
    ratio, _, _, _, _ = _bbox_in_big_ratio_parts(bbox, polygon)
    return ratio
...
def compute_binarize_black_in_big_ratio(
    ...
    *,
    ...
    ``in_big_by=bbox``：小虫 bbox 矩形与大虫 polygon 相交面积 / 小虫 bbox 面积。
    """
    mode = normalize_in_big_by(in_big_by)
    if mode == "bbox":
        return compute_bbox_in_big_ratio(bbox, polygon)
...
def max_binarize_black_in_big_ratio_parts(
    ...
    *,
    ...
    for big_row in big_rows:
        ...
        if mode == "bbox":
            ratio, crop, black_local, big_mask_bool, union_bool = _bbox_in_big_ratio_parts(
                bbox, poly
            )
        else:
...
def collect_in_big_debug_parts_for_row(
    ...
    *,
    ...
    ratio, parts = max_binarize_black_in_big_ratio_parts(
        small_row, big_rows, image_bgr, fill_holes=fill_holes, in_big_by=in_big_by
    )
    ...
    if mode == "bbox":
        ...
        if bh <= 0 or bw <= 0:
            return ratio, None
        black_local = np.ones((bh, bw), dtype=bool)
        overlap_bool = np.zeros((bh, bw), dtype=bool)
        crop = None
...
def max_binarize_black_in_big_ratio(
    ...
    *,
    ...
    """小虫行相对多张大虫 polygon 的最大 in_big 占比。"""
