...
from script.predict.model_detect_factory import create_detector
...
class PredictSize:
    ...
    def __init__(self, detect_model_path, size_config_path,
                 ...
                 detect_model_type: str | None = None,
        ...
        :param detect_model_type: 检测后端，``detect``（YOLO）或 ``onnx``
        :param class_names: ONNX 检测类别名列表或 classes.txt 路径（仅 onnx；YOLO 用权重内 names）
        """
        ...
        self.detector = create_detector(
            detect_model_path,
            model_type=detect_model_type or "detect",
    ...
    def _union_merge_kept_with_drop(keep: dict, drop: dict) -> None:
        ...
        try:
            ...
            pass
        try:
            if "cls_conf" in keep or "cls_conf" in drop:
                keep["cls_conf"] = max(
                    float(keep["cls_conf"]) if keep.get("cls_conf") is not None else 0.0,
                    float(drop["cls_conf"]) if drop.get("cls_conf") is not None else 0.0,
                )
        except Exception:
            pass
    ...
    def _draw_results(
        ...
        *,
        ...
        for r in results:
            ...
            # 标签文字：先检出置信度；有分类模型时再写分类置信度
            if cls_conf is None:
                label = f"{cls_name} det:{float(det_conf or 0.0):.2f}"
            else:
                label = (
                    f"{cls_name} det:{float(det_conf or 0.0):.2f} "
                    f"cls:{float(cls_conf or 0.0):.2f}"
                )
            edge_label = None
    ...
    def predict(self, image, clip_size=2500, overlap_size=800,
        ...
        :param split_by_scale: 多尺度检测时是否启用跨尺度大框误检抑制
        :param split_if_contain: 大框含另一尺度小框个数门限
        ...
        detections = self.detector.predict(
            ...
            split_by_scale=bool(split_by_scale),
            split_if_contain=int(split_if_contain),
        )
        ...
        for det in detections:
            ...
            if self._filter_by_size(det, diag_filter_range=diag_filter_range):
                if self.classifier is not None:
                    ...
                    # 无分类模型：保留检测类名与 detect conf，不写 cls_conf
                    det["cls_name"] = det.get("class_name", "unknown")
                    det.pop("cls_conf", None)
                    det["cls_topk"] = []
                    det["cls_top3"] = []
            else:
                # 不在尺寸范围内 -> 标记过滤，不跑分类
        ...
        if edge_reject_distance is not None and edge_reject_distance > 0:
            for det in final_results:
                ...
                det_low = float(det.get("conf", 0.0) or 0.0) < det_conf_threshold
                if det.get("cls_conf") is None:
                    if det_low:
                        det["filter"] = True
                    continue
                if det_low or float(det.get("cls_conf", 0.0) or 0.0) < float(
                    edge_reject_cls_conf_threshold
                ):
                    det["filter"] = True
...
def write_pascal_voc_xml(
    ...
    *,
    ...
    include_dual_conf: True 时在 object 下写入 det_conf；有分类结果时再写 cls_conf / cls_conf_orig
    （``cls_conf`` 为过滤归一化后，``cls_conf_orig`` 为模型原始置信度）。无分类模型时不写 cls_conf。
    若行含非空 ``filter_reason``，额外写入 ``filter_reason`` 与 ``filtered=1``。
    """
    ...
    for r in results:
        ...
        if include_dual_conf:
            ...
            ET.SubElement(obj, "det_conf").text = _fmt_conf(det_conf)
            _cls_raw = r.get("cls_conf")
            if _cls_raw is not None:
                try:
                    cls_conf = float(_cls_raw)
                except (TypeError, ValueError):
                    cls_conf = None
                if cls_conf is not None:
                    ET.SubElement(obj, "cls_conf").text = _fmt_conf(cls_conf)
                    cls_conf_orig = r.get("cls_conf_orig")
                    if cls_conf_orig is None:
                        cls_conf_orig = cls_conf
                    else:
                        try:
                            cls_conf_orig = float(cls_conf_orig)
                        except (TypeError, ValueError):
                            cls_conf_orig = cls_conf
                    ET.SubElement(obj, "cls_conf_orig").text = _fmt_conf(cls_conf_orig)
        filter_reason = str(r.get("filter_reason") or "").strip()
        if filter_reason:
            ET.SubElement(obj, "filter_reason").text = filter_reason
            ET.SubElement(obj, "filtered").text = "1"
        competition_orig = str(r.get("competition_orig_name") or "").strip()
        if competition_orig:
            ET.SubElement(obj, "competition_orig_name").text = competition_orig
        ...
        if isinstance(cls_topn, list) and len(cls_topn) >= 2:
            ...
            for ent in cls_topn:
                ...
                ET.SubElement(item, "conf").text = _fmt_conf(conf_v)
                if ent.get("conf_orig") is not None:
                    try:
                        orig_v = float(ent.get("conf_orig") or 0.0)
                    except (TypeError, ValueError):
                        orig_v = conf_v
                    ET.SubElement(item, "conf_orig").text = _fmt_conf(orig_v)
