...
_INSECT_ROOT = Path(__file__).resolve().parents[1]
_CLASS_MAP_JSON_CANDIDATES = (
    _INSECT_ROOT / "train" / "train_ba" / "class_map.json",
    _INSECT_ROOT / "train" / "rtdetrv2_pytorch" / "class_map.json",
)
# 别名键不在 insect_info/out 时，推理目录也可能没有 class_map。
# 层级别名两侧中文（标注可能写任一侧）。仅当对应拼音已在 class_tier_aliases 连通分量里才并入。
_TIER_ALIAS_ZH_FALLBACK: dict[str, str] = {
    "毒隐翅虫": "duyinchichong",
    "隐翅虫": "yinchichong",
    "步甲": "bujia",
    "中华星步甲": "zhonghualanbujia",
    "中华婪步甲": "zhonghualanbujia",
    "歧角螟": "qijiaoming",
    "榄绿歧角螟": "lanlvqijiaoming",
    "蟋蟀": "xishuai",
    "小棺头蟋蟀": "xiaoguantouxishuai",
    "叩甲": "koujia",
    "锥尾叩甲": "zhuiweikoujia",
}
...
@lru_cache(maxsize=1)
def _load_class_map_zh_to_pinyin() -> dict[str, str]:
    """检测训练 class_map：中文标注名 → 拼音。文件缺失则空表。"""
    for path in _CLASS_MAP_JSON_CANDIDATES:
        if not path.is_file():
            continue
        try:
            ...
            logging.warning("加载 class_map 失败 %s: %s", path, e)
            continue
        classes = raw.get("classes") if isinstance(raw, dict) else None
        if not isinstance(classes, dict):
            continue
        out: dict[str, str] = {}
        for zh, rec in classes.items():
            if not isinstance(rec, dict):
                continue
            py = str(rec.get("pinyin") or "").strip()
            zh_s = str(zh or "").strip()
            if zh_s and py:
                out[zh_s] = py
        if out:
...
def _zh_names_for_pinyin_keys(pinyin_keys: set[str]) -> dict[str, str]:
    """别名成员的中文名 → 拼音 key（class_map / insect_info / 兜底）。"""
    keys = {str(k).strip() for k in pinyin_keys if str(k).strip()}
    if not keys:
        return {}
    out: dict[str, str] = {}
    for zh, py in _TIER_ALIAS_ZH_FALLBACK.items():
        if py in keys and zh:
            out[zh] = py
    for zh, py in _load_class_map_zh_to_pinyin().items():
        if py in keys and zh:
            out[zh] = py
    try:
...
def _zh_names_from_alg_out(
    alg_config: dict[str, Any] | None,
    pinyin_keys: set[str],
) -> dict[str, str]:
    """``out.cn_name`` → 拼音（仅别名成员）。insect_info 缺条目时仍能并入两侧中文。"""
    keys = {str(k).strip() for k in pinyin_keys if str(k).strip()}
    if not keys or not isinstance(alg_config, dict):
        return {}
    display: dict[str, str] = {}
    _collect_eval_class_display_from_alg_node(alg_config.get("models"), display)
    out: dict[str, str] = {}
    for py in keys:
        zh = str(display.get(py) or "").strip()
        if zh:
            out[zh] = py
    return out


def _zh_names_for_tier_aliases(aliases: dict[str, Any] | None) -> dict[str, str]:
    keys: set[str] = set()
    if isinstance(aliases, dict):
        for key, value in aliases.items():
            ...
            if v:
                keys.add(v)
    return _zh_names_for_pinyin_keys(keys)
...
class ClassTierEquivalence:
    ...
    JSON 写成 ``A: B`` **不是**把 A 单向改写成 B。标注/预测可能是任一侧拼音或对应中文
    （如步甲↔中华星步甲），两种中文都必须能互匹。每条 kv 只表示可互换；多组经并查集传递
    （``A~B``、``B~C`` ⇒ ``A~C``）。统计合并时 ``value`` 侧为 preferred canonical，
    不写入 ``label_alias_map``，以免覆盖另一侧中文。
    ``extra_names`` 把中文标注名并入已有拼音成员的连通分量，不新增无关类。
    """
    ...
    def from_tier_aliases(
        ...
        if isinstance(extra_names, dict) and parent:
            known = set(parent)
            for zh, py in extra_names.items():
                z = str(zh or "").strip()
                p = str(py or "").strip()
                if z and p and p in known:
                    _union(z, p)
                    known.add(z)
        remapped: dict[str, str] = {}
        for pref in stat_canonical.values():
            p = str(pref or "").strip()
            if p:
                remapped[_find(p)] = p
        return cls(_parent=parent, _stat_canonical=remapped)
...
def build_class_tier_equivalence(
    ...
    return ClassTierEquivalence.from_tier_aliases(tier_aliases, extra_names=extra_names)
    ...
    *,
    ...
    if isinstance(aliases, dict):
        for key, value in aliases.items():
            ...
            if v:
                member_keys.add(v)
    extra.update(_zh_names_from_alg_out(alg_config, member_keys))
    return build_class_tier_equivalence(aliases, extra_names=extra)
...
def build_eval_label_alias_map(
    *,
    ...
    ``postprocess.class_tier_aliases`` 的 key→value **不**写入本表（避免覆盖 canonical）；
    别名成员的中文名 setdefault 到拼音 **key 自身**。类型等价见 ``ClassTierEquivalence`` /
    ...
    if isinstance(alg_config, dict):
        ...
        if isinstance(postprocess, dict):
            for zh, py in _zh_names_for_tier_aliases(
                postprocess.get("class_tier_aliases")
                if isinstance(postprocess.get("class_tier_aliases"), dict)
                else None
            ).items():
                mapping.setdefault(zh, py)
...
def is_eval_ignored_class(
    raw: str,
    ignore_classes: frozenset[str] | set[str] | None,
    merge: dict[str, list[str]] | None,
    *,
    label_alias_map: dict[str, str] | None = None,
) -> bool:
    """忽略清单命中（原名或 normalize 后）：不比对、不计入校验指标。"""
    if not ignore_classes:
        return False
    s = str(raw or "").strip()
    if not s:
        return False
    if s in ignore_classes:
        return True
    canon = normalize_class_name(s, merge, label_alias_map=label_alias_map)
    return bool(canon) and canon in ignore_classes


def _skip_eval_metric_class(
    raw: str,
    merge: dict[str, list[str]] | None,
    *,
    ignore_classes: frozenset[str] | set[str] | None = None,
    label_alias_map: dict[str, str] | None = None,
) -> bool:
    if is_metric_ignored_other(raw, merge):
        return True
    return is_eval_ignored_class(
        raw, ignore_classes, merge, label_alias_map=label_alias_map
    )
...
def parse_pascal_voc_pred_objects(xml_path: str | Path) -> list[dict[str, Any]]:
    ...
    for obj in root.findall("object"):
        ...
        fr_el = obj.find("filter_reason")
        if fr_el is not None and (fr_el.text or "").strip():
            row_out["filter_reason"] = fr_el.text.strip()
            row_out["filtered"] = True
        else:
            filtered_el = obj.find("filtered")
            if filtered_el is not None and str(filtered_el.text or "").strip() in (
                "1",
                "true",
                "True",
            ):
        ...
        if orig_el is not None and (orig_el.text or "").strip():
            row_out["competition_orig_name"] = orig_el.text.strip()
...
# --------------------------------------------------------------------------- #
# report_tp：仅报出框且报出 name 与 GT 同类的正确匹配（比赛「识别数」用）
_STAT_BY_CLS_KEYS = ("gt", "pred", "tp", "fp", "fn", "cls_err", "report_tp")


def build_eval_focus_set(
    names: list[str] | set[str] | frozenset[str] | None,
    *,
    merge: dict[str, list[str]] | None = None,
    label_alias_map: dict[str, str] | None = None,
    tier_equivalence: ClassTierEquivalence | None = None,
) -> frozenset[str]:
    """将关注类名列表展开为匹配用集合（含原始拼音、normalize 后的名、层级别名 canonical 与中文成员）。"""
    out: set[str] = set()
    for n in names or []:
        raw = str(n or "").strip()
        if not raw:
        ...
        if tier_equivalence is not None:
            for cand in (raw, canon):
                if not cand:
                    continue
                rolled = tier_equivalence.canonical_stat_name(cand)
                if rolled:
                    out.add(rolled)
                out.update(tier_equivalence.expand_names([cand]))
...
def _std_eval_collect_confusion(
    ...
    *,
    ...
    if not gts:
        for p in preds:
            if _skip_eval_metric_class(
                str(p.get("cls_name", "") or ""), class_merge_to_groups, **skip_kw
            ):
                continue
    ...
    for i, j, _ in mlist:
        if _skip_eval_metric_class(
            str(preds[int(i)].get("cls_name", "") or ""),
            class_merge_to_groups,
            **skip_kw,
        ):
            continue
        if _skip_eval_metric_class(
            str(gts[int(j)].get("name", "") or ""),
            class_merge_to_groups,
            **skip_kw,
        ):
            continue
...
def _std_eval_collect_confusion_filename_gt(
    ...
    *,
    ...
    if _skip_eval_metric_class(gt_raw, class_merge_to_groups, **skip_kw):
        return
    gn = confusion_matrix_stat_label(
        gt_raw,
        class_merge_to_groups,
    ...
    eval_preds = [
        p
        for p in preds
        if not _skip_eval_metric_class(
            str(p.get("cls_name", "") or ""), class_merge_to_groups, **skip_kw
        )
    ]
    if not eval_preds:
...
def _save_type_confusion_crops_for_filename_gt(
    ...
    *,
    ...
    """文件名 GT：按匹配结果落盘（tp / cls_err / 整图漏检 pred=__miss__）。"""
    gt_raw = str(gt_name or "").strip()
    skip_kw = {
        "ignore_classes": ignore_classes,
        "label_alias_map": label_alias_map,
    }
    if not gt_raw or _skip_eval_metric_class(gt_raw, class_merge_to_groups, **skip_kw):
        return
    ...
    if not eval_preds:
        stem = _type_miss_crop_stem(rel_path, 0)
...
_TYPE_CONFUSION_EXTRA_GT = _STD_EVAL_BG_ROW  # FP：预测框无几何匹配 GT
...
def _type_extra_pred_crop_stem(rel_path: Path, pi: int, pred: dict) -> str:
    det_conf = float(pred.get("conf", 0.0) or 0.0)
    cls_conf = float(pred.get("cls_conf", det_conf) or det_conf)
    return f"{rel_path.stem}__pi{pi:03d}__extra__det{det_conf:.3f}__cls{cls_conf:.3f}.jpg"
...
def _save_type_confusion_crops_for_branch(
    ...
    *,
    ...
    """
    按匹配结果切图落盘（索引方式同 predict_cls_validate_from_xml）：
    - 几何已匹配（tp / cls_err）：type_confusion_crops/gt=.../pred=...
    - 几何漏检（GT 无匹配预测框）：.../pred=__miss__
    - 多余检出（最终 results 中预测框无匹配 GT）：.../gt=__extra_pred__

    ``n_result_preds`` 为 ``len(results)``：后置 other / 分类未过阈等 filtered 实例
    仅参与几何匹配与类型错切图，未匹配时不落 FP 切图（与评估 FP 统计一致）。
    """
    if not gts:
        return
    skip_kw = {
    ...
    mlist = list(matches or [])
    matched_g = {int(j) for _, j, _ in mlist}
    matched_p = {int(pi) for pi, _, _ in mlist}
    ...
    fp_upper = int(n_result_preds) if n_result_preds is not None else len(preds)
    for pi in range(len(preds)):
        if pi in matched_p:
            continue
        if pi >= fp_upper:
        ...
        stem = _type_extra_pred_crop_stem(rel_path, pi, p)
        _write_type_confusion_crop_pair(
            crop,
            base_gt=base_gt,
            base_pred=base_pred,
            gn=_TYPE_CONFUSION_EXTRA_GT,
            pn=pn,
            stem=stem,
        )
