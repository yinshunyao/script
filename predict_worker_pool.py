...
def isolate_current_process_from_cuda() -> str | None:
    """
    将当前进程对 GPU 不可见（``CUDA_VISIBLE_DEVICES=""``）。

    必须在本进程首次 ``import torch`` 之前调用。返回调用前的
    ``CUDA_VISIBLE_DEVICES``（未设置时为 ``None``），供 worker 恢复。
    """
...
def restore_cuda_visible_devices(saved: str | None) -> None:
    """恢复 ``CUDA_VISIBLE_DEVICES``；``saved is None`` 表示原本未设置，则删除该变量。"""
    if saved is None:
...
def _worker_main(
    ...
    restore_cuda_visible: bool = False,
) -> None:
    """子进程入口：加载 ``InsectPredictAll`` 并循环处理推理任务。"""
    if restore_cuda_visible:
        restore_cuda_visible_devices(cuda_visible_devices)
    _ensure_insect_root_on_syspath()
    _configure_worker_logging()
    wlog = logging.getLogger(f"insect.worker.{worker_id}")
    if restore_cuda_visible:
        wlog.info(
            "worker-%d 已恢复 CUDA_VISIBLE_DEVICES=%s",
            worker_id,
            os.environ.get("CUDA_VISIBLE_DEVICES", "(未设置)"),
        )
    pipeline = None
    try:
...
class InferenceProcessPool:
    ...
    主进程若已 ``isolate_current_process_from_cuda``，须传 ``restore_cuda_visible=True``
    以及隔离前的 ``cuda_visible_devices``，否则 worker 继承空的 ``CUDA_VISIBLE_DEVICES``
    会在 CPU 上加载模型。
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._restore_cuda_visible = bool(restore_cuda_visible)
