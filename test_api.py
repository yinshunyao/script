...
def collect_image_urls_from_dir(image_dir: str, *, recursive: bool = False) -> list[str]:
    """遍历目录收集图片，返回服务端绝对路径列表（请求体原样发送）。"""
    root = Path(image_dir)
    if not root.is_dir():
        raise ValueError(f"TEST_IMAGE_ALL 目录不存在或不是目录: {image_dir}")
    ...
    if not urls:
        raise ValueError(f"TEST_IMAGE_ALL 目录下未找到图片: {image_dir}")
    return urls


def resolve_test_image_urls(
    *,
    image_all_dir: str | None = None,
    ...
    1. ``image_all_dir``：遍历目录下所有图片（``TEST_IMAGE_ALL``）
    2. ``image_urls``：完整服务端绝对路径列表
    3. ``image_dir`` + ``image_names``：目录 + 文件名列表
    4. ``image_url``：单张完整路径
    """
    if image_all_dir:
        urls = collect_image_urls_from_dir(image_all_dir, recursive=recursive)
    elif image_urls:
    ...
    if not urls:
        raise ValueError(
            "请设置 TEST_IMAGE_ALL，或 TEST_IMAGE_URLS，"
            "或 TEST_IMAGE_DIR + TEST_IMAGE_NAMES，或 TEST_IMAGE_URL"
        )
    return urls
...
"""
    ...
    # 方式四（最高优先级）：遍历目录下所有图片；设 None 或 "" 则禁用
    TEST_IMAGE_ALL: str | None = "/data/models/模拟试卷/"
    # 是否递归子目录（仅 TEST_IMAGE_ALL 生效）
    TEST_IMAGE_ALL_RECURSIVE = False

    # 多张批量时默认精简输出；单张或需调试时可设 True 打印完整 JSON
    _has_batch = bool(TEST_IMAGE_ALL) or len(TEST_IMAGE_URLS) > 1 or sum(
        1 for n in TEST_IMAGE_NAMES if n.strip()
    ) > 1
    VERBOSE = not _has_batch
    # 并发数：1=顺序请求（测单张延迟）；>1 仅压测排队，单 GPU 服务总墙钟不会明显下降
    CONCURRENCY = 4

    try:
        image_urls = resolve_test_image_urls(
            image_all_dir=TEST_IMAGE_ALL or None,
            image_dir=TEST_IMAGE_DIR,
            image_names=TEST_IMAGE_NAMES,
            image_urls=TEST_IMAGE_URLS or None,
            image_url=TEST_IMAGE_URL,
            recursive=TEST_IMAGE_ALL_RECURSIVE,
        )
